	...
	public void printTally(PrintWriter output) {
		...
		for (int i = 0; i < 3 && i < sortedLex.size(); i++) {
			...
			for (String id : temp) {
				...
				// 2 courses of action depending on whether input is gold std
				// .auto parses or C&C .auto parses
				if (idInfo.length == 2) {
				...
				output.println("<li>");
				output.println(id);
				output.println(" <a href=file://" + ccgbankHome + "/original/data/HTML/" + dir
						+ "/" + idLink + " target=" + sentNo + ">");
