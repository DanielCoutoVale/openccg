...
public class InfoHelper {
	...
	/**
	 * Read BBN NE aux file corresponding to the WSJ Section provided as
	 * argument.
	 */
	...
	/**
	 * Read quotes aux file corresponding to the WSJ Section provided as
	 * argument.
	 */
	...
	/**
	 * Read PTB aux file (sbj, fn-tag and tpc info) corresponding to the WSJ
	 * Section provided as argument.
	 */
	...
	public String getBBNClass(String header, String lex, String pos, String cat, int nodeInd) {
		...
		if (pos.startsWith("NN") || pos.startsWith("RB") || pos.startsWith("JJ")
				|| pos.startsWith("VB") || pos.equals("CD") || lex.equals("%") || pos.equals("$"))
			relFlag = true;
		...
		for (String span : spanList) {
			...
			if (!inds[0].matches("[0-9]+") || !inds[1].matches("[0-9]+") || inds[0].equals("NA")
					|| inds[1].equals("NA"))
				continue;
			...
			if (nodeInd >= ind1 && nodeInd <= ind2) {
				...
				if (!semClass.startsWith("DATE") && !semClass.startsWith("TIME")
						&& !semClass.startsWith("ORDINAL") && !semClass.startsWith("QUANTITY")
						&& !semClass.startsWith("PERCENT") && !semClass.startsWith("MONEY"))
					accClasses = true;

				if (accClasses || pos.startsWith("NN") || pos.equals("CD") || lex.equals("%")
						|| pos.equals("$")) {
					String wordBit = "";
	...
	public String classReplace(String semClass, String lex, String wordBit) {
		...
		/*
		 * CITY-based classes if(wordBit.equals(lex)) retVal=semClass; else
		 * if(lex.contains(wordBit)) retVal=lex.replaceFirst(wordBit,semClass);
		 */
	...
	public String getBBNInfo(String header, String span, String words) {
		...
		if (bbnInfo.containsKey(key)) {
			...
			if (ccgWords.length > 0 && bbnWords.length > 1
					&& ccgWords.length == bbnWords.length - 1) {
	...
	public String id(String cat) {
		...
		if (idTally.size() == 1)
			...
			retVal = "later" + "_" + Integer.toString(idNum);
	...
	/**
	 * Given a lexical item, ascertain whether it is a legit original CCGbank
	 * word
	 **/
	public boolean isCCGWord(String word) {

		boolean retval = true;
		if (word.matches("\\p{Punct}|[\\.]+|(-lrb-)|(-rrb-)|(-lcb-)|(-rcb-)|(--)|(`)|(')|(``)|('')")
	...
	public void printBBNClasses() {
		...
		try {
			...
			System.out
					.println("Printing BBN classes used in the corpus to bbn-types.txt (for use in grammar.xml)");
