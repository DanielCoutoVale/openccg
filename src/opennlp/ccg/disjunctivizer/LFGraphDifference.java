...
/**
 * Represents the difference between two {@link LFGraph}s that characterizes
 * their difference as a set of edits: either {@link #inserts()},
 * {@link #deletes()}, or {@link #substitutions()}. These edits are determined
 * by a specified {@linkplain #getAlignment() alignment} between the phrases the
 * graphs are supposed to represent. All of the sets of edges returned by this
 * class, and the convenience maps build on top of them, are read-only.
 * Attempting to add or remove elements or keys from any of these (including via
 * any of the iterators) throws an {@link UnsupportedOperationException}.
 * <p>
 * This class also provides the convenience methods
 * {@link #insertsFor(LFVertex)} and {@link #deletesFor(LFVertex)}, for getting
 * just the inserts or deletes for a specified vertex. The convenience methods
 * {@link #substitutionsFor(LFEdge)} gets the set of substitutions correspoding
 * to a given edge, and {@link #substitutionsBySource()} and
 * {@link #substitutionsBySourceFor(LFEdge)} are similar methods that provide
 * maps whose keys are the source vertices of the substituted edges.
 * 
...
public class LFGraphDifference {
	...
	/**
	 * Creates a new graph difference between <tt>a</tt> and <tt>b</tt>, as
	 * determined by the specified alignment.
	 * 
	 ...
	 *            {@linkplain PhrasePosition#A A-position} indices are
	 *            understood to correspond to <tt>a</tt> and
	 *            {@linkplain PhrasePosition#B B-position} indices are
	 *            understood to correspond to <tt>b</tt>.
	 * @throws IllegalArgumentException If either graph is <tt>null</tt>, or if
	 *             the alignment is <tt>null</tt>.
	 */
	 ...
	 * @return The value of {@link #getA()} if <tt>position</tt> is
	 *         {@link PhrasePosition#A}, and the value of {@link #getB()}
	 *         otherwise.
	 */
	...
	/**
	 * Computes a hash code for this graph difference based on its graphs and
	...
	/**
	 * Tests whether this LF graph difference is equivalent to another by
	 * comparing their graphs and the alignment between them.
	 */
	@Override
	 ...
	 * 
	 * @return An LF graph difference whose {@linkplain PhrasePosition#A
	 *         A-position} graph is the value of this difference's
	 *         {@link #getB()}, whose {@linkplain PhrasePosition#B B-position}
	 *         graph is the value of this difference's {@link #getA()}, and
	 *         whose alignments are the {@linkplain Alignment#reverse() reverse}
	 *         of this difference's {@link #getAlignment()}.
	 */
	 ...
	 * 
	 * @return The set of edges in the {@linkplain PhrasePosition#A A-position}
	 *         graph that have an aligned {@linkplain LFEdge#getSource() source
	 *         vertex} and an unaligned {@linkplain LFEdge#getTarget() target
	 *         vertex}.
	 * 
	 ...
	 * 
	 * @return The set of edges in the {@linkplain PhrasePosition#B B-position}
	 *         graph that have an aligned {@linkplain LFEdge#getSource() source
	 *         vertex} and an unaligned {@linkplain LFEdge#getTarget() target
	 *         vertex}.
	 * 
	...
	Set<LFEdge> doDeletes(PhrasePosition keyPosition) {
		return Collections.unmodifiableSet(new FilteredLFEdgeSet(get(keyPosition).edgeSet(),
				new AlignedEdgeFilter(alignment.asMap(keyPosition).keySet(), SOURCE_ALIGNED,
						TARGET_UNALIGNED)));
	}
	 ...
	 * @return The subset of {@link #inserts()} whose
	 *         {@linkplain LFEdge#getSource() source} index is among the
	 *         {@linkplain Alignment#getTargets(Integer) targets} for the
	 *         specified vertex, or {@link Collections#EMPTY_SET} if none exist.
	 * 
	 * @see AlignedEdgeFilter
	 */
	...
	public Set<LFEdge> insertsFor(LFVertex vertex) {
		...
		return (indices.isEmpty()) ? Collections.EMPTY_SET : new FilteredLFEdgeSet(inserts(),
				new AlignedEdgeFilter(indices, SOURCE_ALIGNED));
	}
	 ...
	 * @return The subset of {@link #deletes()} whose
	 *         {@linkplain LFEdge#getSource() source vertex} is the specified
	 *         vertex.
	 * 
	 ...
	 * 
	 * @return The subset of the {@linkplain PhrasePosition#B B-position} 
	 *         graph's edges for which there exists an edge in the
	 *         {@linkplain PhrasePosition#A A-position} graph that meets the
	 *         following conditions:
	 *         <ol>
	 *         <li>The B edge's source is aligned to the A edge's source, but
	 *         the B edge's target is not.</li>
	 *         <li>The B edge's target is aligned to the A edge's target, but
	 *         the B edge's source is not.</li>
	 *         </ol>
	 * 
	 * @see CompositeFilter
	 */
	public Set<LFEdge> substitutions() {
		if (substitutions == null) {
			...
			for (LFEdge aEdge : a.edgeSet()) {
				Set<Integer> sMaps = alignment.getTargets(aEdge.getSource().getIndex()), tMaps = alignment
						.getTargets(aEdge.getTarget().getIndex());

				if (!sMaps.isEmpty() && !tMaps.isEmpty()) {
					if (sourceFilter == null) {
						sourceFilter = new AlignedEdgeFilter(sMaps, SOURCE_ALIGNED,
								TARGET_UNALIGNED);
						targetFilter = new AlignedEdgeFilter(tMaps, TARGET_ALIGNED,
								SOURCE_UNALIGNED);
	 ...
	 * @return The subset of {@link #substitutions()} whose source is aligned to
	 *         the edge's source and whose target is aligned to the edge's
	 *         target, or {@link Collections#EMPTY_SET} if none exist.
	 */
	@SuppressWarnings("unchecked")
	public Set<LFEdge> substitutionsFor(LFEdge edge) {
		Set<Integer> srcMapsTo = alignment.getTargets(edge.getSource().getIndex()), trgMapsTo = alignment
				.getTargets(edge.getTarget().getIndex());

		return (srcMapsTo.isEmpty() || trgMapsTo.isEmpty()) ? Collections.EMPTY_SET : Collections
				.unmodifiableSet(new FilteredLFEdgeSet(substitutions(),
						new CompositeFilter<LFEdge>(
								new AlignedEdgeFilter(srcMapsTo, SOURCE_ALIGNED),
	 ...
	 * 
	 * @return A map whose keys are the source vertices in the set of
	 *         {@link #substitutions()} and whose values are the edges whose
	 *         {@linkplain LFEdge#getSource() source vertex} is the same as the
	 *         corresponding key. If there are no substitutions,
	 *         {@link Collections#EMPTY_MAP} is returned.
	 *         <p>
	 *         Note that the returned map is read-only, that is, both its
	 *         {@link Map#put(Object, Object)} method and its
	 *         {@linkplain Map#entrySet() entry set}'s iterator's
	 *         {@link Iterator#remove()} method throw an
	 *         {@link UnsupportedOperationException}. Also, the members of the
	 *         returned map's entry set are immutable, so that their
	 *         {@link Entry#setValue(Object)} methods also throw an
	...
	public Map<LFVertex, Set<LFEdge>> substitutionsBySource() {
		...
		return subs.isEmpty() ? Collections.EMPTY_MAP : Collections
				.unmodifiableMap(new FilteredMap<LFVertex, Set<LFEdge>>(new SourceView(),
	 ...
	 * 
	 * @param edge The edge to get substitutions for.
	 * @return The subset of {@link #substitutionsBySource()} in which the keys
	 *         are aligned to the specified edge's source and the associated
	 *         values' targets are aligned to the specified edge's target. Since
	 *         this map is based on the one returned by
	 *         {@link #substitutionsBySource()}, it is also read-only, and the
	 *         same stipulations apply to it.
	 * 
	...
	public Map<LFVertex, Set<LFEdge>> substitutionsBySourceFor(LFEdge edge) {
		...
		return subsBySource.isEmpty() ? subsBySource
				: new SubstitutedSourceView(subsBySource, edge);
	}
					...
					return new Iterator<Entry<LFVertex, FilteredLFEdgeSet>>() {
						...
						public Entry<LFVertex, FilteredLFEdgeSet> next() {
							...
							return new SimpleImmutableEntry<LFVertex, FilteredLFEdgeSet>(src,
									new FilteredLFEdgeSet(subs, new VertexMatchFilter(src,
											SOURCE_MATCH)));
						}

						@Override
						public void remove() { // subs.iterator() should be
												// read-only, but just in case
	...
	class SubstitutedSourceView extends AbstractMap<LFVertex, Set<LFEdge>> {
		...
		class EntrySet extends AbstractSet<Entry<LFVertex, Set<LFEdge>>> {
			...
			Set<Entry<LFVertex, Set<LFEdge>>> entries() {
				if (entries == null) {
					entries = new FilteredSet<Entry<LFVertex, Set<LFEdge>>>(sourceView.entrySet(),
			...
			public Iterator<Entry<LFVertex, Set<LFEdge>>> iterator() {
				return new Iterator<Entry<LFVertex, Set<LFEdge>>>() {
					...
					public Entry<LFVertex, Set<LFEdge>> next() {
						...
						return new SimpleImmutableEntry<LFVertex, Set<LFEdge>>(e.getKey(),
					...
					@Override
					public void remove() { // source view is already read-only,
											// so just in case
						throw new UnsupportedOperationException();
					}
				};
