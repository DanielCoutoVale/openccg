 ...
 * Instances can be configured to switch handling of
 * {@link LFGraphDifference#inserts()}, {@link LFGraphDifference#deletes()} and
 * {@link LFGraphDifference#substitutions()} on or off. By default, all three
 * are performed.
 * <p>
 * Disjunctivizers can be re-used, so that all of the boolean parameters
 * {@link #setProcessingDeletes(boolean)},
 * {@link #setProcessingInserts(boolean)}, and
 * {@link #setProcessingSubstitutions(boolean)} can be modified in between calls
 * to {@link #buildDisjunctiveLFFor(LFGraphDifference)}. When these parameters
 * are changed, the disjunctive LF built will change as well. The
 * {@link Document} used to create disjunctive LF elements (and the elements
 * within them) can be configured as well, either at creation or via
...
public class Disjunctivizer {
	...
	/**
	 * Creates a new disjunctivizer that will use the specified document to
	 * create the elements in its generated disjunctive LFs.
	 * 
	...
	/**
	 * Creates a new disjunctivizer that will use the specified document to
	 * create the elements in its generated disjunctive LFs, with the specified
	 * parameters dictating whether to process inserts, deletes, and
	 * substitutions.
	 * 
	 * @param document The document to use for creating elements, attributes,
	 *            nodes, etc.
	 * @param processingInserts Whether to process
	 *            {@link LFGraphDifference#inserts()}.
	 * @param processingDeletes Whether to process
	 *            {@link LFGraphDifference#deletes()}.
	 * @param processingSubstitutions Whether to process
	 *            {@link LFGraphDifference#substitutions()}.
	 */
	public Disjunctivizer(Document document, boolean processingInserts, boolean processingDeletes,
	 ...
	 * 
	 * @param document The document that will be used while building disjunctive
	 *            LF elements.
	 */
	...
	/**
	 * Returns whether this disjunctivizer processes
	 * {@link LFGraphDifference#inserts()}.
	 */
	...
	/**
	 * Sets whether this disjunctivizer processes
	 * {@link LFGraphDifference#inserts()}.
	 */
	...
	/**
	 * Builds a disjunctive LF based on the specified graph difference. The
	 * shape of the returned element will change depending on whether inserts,
	 * deletes, or substitutions are being processed.
	 * 
	 * @param graphDifference The graph difference to use for building the
	 *            disjunctive LF.
	 * @return A recursively build disjunctive LF based on this disjunctivizer's
	 *         graph difference.
	 * @throws IllegalArgumentException If <tt>graphDifference</tt> is
	 *             <tt>null</tt>.
	 */
	public Element buildDisjunctiveLFFor(LFGraphDifference graphDifference) {
		...
		if (disjunctiveLF == null || !this.graphDifference.equals(graphDifference)) {
			...
			for (LFVertex p : graphDifference.a.highestLFAncestors()) {
				disjunctiveLF.appendChild(createDisjunctiveElement(new DLFContext(A, p,
						disjunctiveLF)));
			}
		}
	...
	private void findForeignAlignedSubgraphRoots() {
		...
		while (rootIterator.hasNext()) {
			...
			if (!foreignAlignedSubgraphRoots.containsKey(vertex)) { // already
																	// encountered?
				if (mappings.containsKey(vertex.getIndex())) { // aligned?
					LFGraphIterator subgraphIterator = new LFGraphIterator(graphDifference.b,
							vertex);

					while (subgraphIterator.hasNext()) { // map whole subgraph
															// to the aligned
															// vertex
	...
	private Element createDisjunctiveElement(DLFContext context) {
		...
		Set<LFVertex> locals = localContext.getVertices(LOCAL_ANCESTOR), visited = localContext
				.getVertices(VISITED);
		...
		if (locals.contains(localContext.vertex)
			...
			Set<LFEdge> outgoingEdges = graph.containsVertex(localContext.vertex) ? graph
					.outgoingEdgesOf(localContext.vertex) : Collections.EMPTY_SET;

			if (outgoingEdges.isEmpty()) { // leaf?
				setPredicateName(localContext);
			} else {
				if (!imported && processingSubstitutions) { // do substitutions,
															// if applicable
	...
	private void processInserts(DLFContext context) {
		...
		for (LFEdge ins : graphDifference.insertsFor(context.vertex)) {
			...
			if (!foreignAlignedSubgraphRoots.containsKey(ins.getTarget())) {
				...
				importedVertices.add(ins.getTarget()); // remember that inserted
														// vertex is foreign
	...
	private void processSubstitutions(DLFContext context) {
		for (LFEdge outgoing : context.getGraph().outgoingEdgesOf(context.vertex)) {
			if (graphDifference.substitutionsFor(outgoing).isEmpty()) { // no
																		// substitution(s)
																		// for
																		// this
																		// edge?
	...
	private void processSubstitutedEdge(DLFContext context, LFEdge outgoing) {
		...
		Map<LFVertex, Set<LFEdge>> subsBySource = graphDifference
				.substitutionsBySourceFor(outgoing);
		...
		for (LFVertex subSource : subsBySource.keySet()) {
			...
			// find the edges matching the outgoing edge's label, and the
			// vertices with different predicates
			// from the outgoing edge's target vertex
			FilteredLFEdgeSet identicals = new FilteredLFEdgeSet(
					subEdges,
					new MembershipFilter<LFEdge>(context.getGraph().outgoingEdgesOf(context.vertex)));
			Set<LFEdge> matchingLabels = new FilteredLFEdgeSet(subEdges, new LabelMatchFilter(
					outgoing.getLabel()));
			Set<LFVertex> differentPredicates = new FilteredSet<LFVertex>(new FilteredLFEdgeSet(
					matchingLabels, predicateFilter).targetView(), new VisitedFilter<LFVertex>());

			// deal with the edges with matching labels separately from other
			// substitutions
			subEdges.removeAll(matchingLabels);
			differentPredicates.removeAll(identicals.targetView());
			subEdges.removeAll(identicals);

			if (subEdges.isEmpty() && differentPredicates.isEmpty()) { // no
																		// substitutions
																		// to
																		// make
				...
				if (!differentPredicates.isEmpty()) { // handle matching labels
														// but different
														// predicates
	...
	private void processDifferentPredicates(DLFContext context, LFEdge outgoing,
		...
		if (!terminal) {
			terminal = !new FilteredSet<LFVertex>(differentPredicates, new TerminalFilter(
					graphDifference.b)).isEmpty();
		}
		...
		if (terminal) {
			...
			if (differentPredicates.size() == 1) { // if we're here at all, it's
													// at least non-empty
				processSingletonDifferentPredicate(ctxt, outgoing, differentPredicates.iterator()
						.next());
			} else {
	...
	private void processSingletonDifferentPredicate(DLFContext context, LFEdge outgoing,
		...
		// generate the target element, but do not propagate changes to tracked
		// vertices
		...
		// if we get here, they all were aliased: use generated target element
		// instead
	...
	private void processSubstitutedEdges(DLFContext context, LFEdge outgoing,
			Set<LFEdge> substituedEdges) {
		LFEdgeLabel label = outgoing.getLabel();
		boolean singleton = substituedEdges.size() == 1; // can't be empty if we
															// get here
	...
	private void processSubstitutedSimilarTarget(DLFContext context, LFEdge outgoing) {
		...
		for (LFVertex subSource : subsBySource.keySet()) {
			...
			if (!similarTargetEdges.isEmpty()) {
				if (similarTargetEdges.size() > 1) { // more than one similar
														// target?
					System.err.println("more than one similar target edge for " + ctxt.vertex
							+ ": " + similarTargetEdges); // TODO figure out
															// what to do about
															// this
				}
	...
	private void processSubstitutedPredicates(DLFContext context, LFEdge outgoing) {
		...
		if (predicate != null) {
			Set<LFVertex> alternates = new FilteredSet<LFVertex>(graphDifference
					.substitutionsBySourceFor(outgoing).keySet(),
			...
			if (alternates.isEmpty()) { // the simple case, no other predicates
										// involved
				setPredicateName(context);
	...
	private void fixOptions(DLFContext context, LFEdgeLabel label) {
		...
		for (int j = 0; j < ncs.getLength(); j++) {
			...
			if (c != null && c.getNodeType() == Node.ELEMENT_NODE
					&& c.getNodeName().equals(OPTIONAL_TAG)) {
					...
					if (r != null && r.getNodeType() == Node.ELEMENT_NODE
						...
						if (label.getName().equals(re.getAttribute(NAME_ATTR))) {
							...
							if (d != null && d.getNodeType() == Node.ELEMENT_NODE
								...
								break; // don't try to remove more than once,
										// throws DOMException
							}
						}
					}
	...
	private void assimilateAttributes(DLFContext context, LFVertex one, LFVertex two) {
		...
		Map<Mode, Proposition> oneAttrs = new HashMap<Mode, Proposition>(one.getAttributeMap()), twoAttrs = new HashMap<Mode, Proposition>(
				two.getAttributeMap());
		...
		} else if (twoAttrs.isEmpty()) { // some attributes remain for first
											// vertex
	...
	class DLFContext {
		...
		DLFContext(PhrasePosition graphPosition, LFVertex vertex, Element parent) {
			this(graphPosition, vertex, parent, new EnumMap<VertexType, Set<LFVertex>>(
					VertexType.class));
		}

		DLFContext(PhrasePosition graphPosition, LFVertex vertex, Element parent,
				Map<VertexType, Set<LFVertex>> vertices) {
		...
		DLFContext copy(boolean copyVertices) {
			return copyVertices ? copyWithVertexMask(VertexType.values()) : new DLFContext(
					graphPosition, vertex, parent, vertices);
		}
