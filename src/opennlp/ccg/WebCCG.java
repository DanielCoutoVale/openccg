...
/**
 * An interface for use with a higher-level web interface. This should provide
 * as simple an interface onto parsing as possible, with its output in a format
 * that can be easily handled by a CGI program or similar. Called as
 *
 * webccg [-showall] [-showderivs] [-showsem] [-visualize FILE] GRAMMARDIR
 * 
 * -showall shows all parses rather than just the first one. -showderivs shows
 * the derivation history of each parse. -showsem shows the logical form of each
 * parse. -visualize output a visualization of the parses into FILE (in PNG
 * format).
 *
 * @author Ben Wing
