...
/**
 * A class for extracting generic features from derivations, inspired by those
 * in the C&C-07 normal form model.
 * 
...
public class SyntacticFeatureExtractor implements FeatureExtractor {
		...
		// non-terminal
			...
			if (inputs.length == 1) {
				...
				if (includeDistFeats) {
					...
					currentDistW = null;
					currentDistP = null;
					currentDistV = null; // also reset current distances
					inc(distExtractors);
				}
			}
		}
	...
	/**
	 * Returns the distance in intervening words as 0w, 1w, 2w or 3w (for 3 or
	 * more).
	 */
	protected String distWords() {
		if (currentDistW != null)
			return currentDistW;
		setCurrentWords();
		int dist = Math.abs(currentHeadIndex - currentSibHeadIndex) - 1;
		switch (dist) {
		case 0:
			return currentDistW = "0w";
		case 1:
			return currentDistW = "1w";
		case 2:
			return currentDistW = "2w";
		default:
			return currentDistW = "3w";
		}
	}

	/**
	 * Returns the distance in intervening punctuation marks as 0p, 1p, 2p or 3p
	 * (for 3 or more).
	 */
	protected String distPuncts() {
		if (currentDistP != null)
			return currentDistP;
		setCurrentWords();
		...
		switch (count) {
		case 0:
			return currentDistP = "0p";
		case 1:
			return currentDistP = "1p";
		case 2:
			return currentDistP = "2p";
		default:
			return currentDistP = "3p";
		}
	}
	...
	/**
	 * Returns the distance in intervening verbs as 0v, 1v, or 2v (for 2 or
	 * more).
	 */
	protected String distVerbs() {
		if (currentDistV != null)
			return currentDistV;
		setCurrentWords();
		int min = Math.min(currentHeadIndex, currentSibHeadIndex);
		int max = Math.max(currentHeadIndex, currentSibHeadIndex);
		...
		switch (count) {
		case 0:
			return currentDistV = "0v";
		case 1:
			return currentDistV = "1v";
		default:
			return currentDistV = "2v";
		}
	}

	/**
	 * Returns whether a word is a verb. The default implementation tests for a
	 * POS tag beginning with V.
	 */
	...
	private List<TrieMap.KeyExtractor<String>> lexcat_word() {
		...
		retval.add(new TrieMap.KeyExtractor<String>() {
			public String getKey() {
				return currentSign.getWordForm();
			}
		});
		return retval;
	}
	...
	private void add_supertag(List<TrieMap.KeyExtractor<String>> retval) {
		...
		retval.add(new TrieMap.KeyExtractor<String>() {
			public String getKey() {
				return currentSign.getSupertag();
			}
		});
	}
	...
	private void add_unary_rule(List<TrieMap.KeyExtractor<String>> retval) {
		...
		retval.add(new TrieMap.KeyExtractor<String>() {
			public String getKey() {
				return currentInputs[0].getSupertag();
			}
		});
	}
	...
	private void add_binary_rule(List<TrieMap.KeyExtractor<String>> retval) {
		...
		retval.add(new TrieMap.KeyExtractor<String>() {
			public String getKey() {
				return currentInputs[1].getSupertag();
			}
		});
	}
	...
	private void add_lex_word(List<TrieMap.KeyExtractor<String>> retval) {
		retval.add(new TrieMap.KeyExtractor<String>() {
			public String getKey() {
				return currentSign.getLexHead().getWordForm();
			}
		});
	}
	...
	private void add_lex_pos(List<TrieMap.KeyExtractor<String>> retval) {
		retval.add(new TrieMap.KeyExtractor<String>() {
			public String getKey() {
				return currentSign.getLexHead().getPOS();
			}
		});
	}
	...
	private void add_sibling_word(List<TrieMap.KeyExtractor<String>> retval) {
		retval.add(new TrieMap.KeyExtractor<String>() {
			public String getKey() {
				return currentSibling.getLexHead().getWordForm();
			}
		});
	}
	...
	private void add_sibling_pos(List<TrieMap.KeyExtractor<String>> retval) {
		retval.add(new TrieMap.KeyExtractor<String>() {
			public String getKey() {
				return currentSibling.getLexHead().getPOS();
			}
		});
	}
