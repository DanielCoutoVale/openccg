...
/**
 * A class for extracting total lexical log probabilities from a supertagger as
 * a feature. The class may also be used as a sign scorer.
 * 
...
public class LexLogProbFeatureExtractor implements FeatureExtractor, SymbolScorer {
	...
	/**
	 * Recursively gets lex log prob total for the given sign, if not already
	 * present.
	 */
	protected float getLexLogProb(Symbol sign, boolean complete) {
		// check for stored log prob
		SupertaggerAdapter.LexLogProb lexlogprob = (SupertaggerAdapter.LexLogProb) sign
				.getData(SupertaggerAdapter.LexLogProb.class);
		if (lexlogprob != null)
	...
	protected FeatureVector lexLogProbVector(float logprob) {
		...
		if (lexlogprobFeature != null)
			retval.add(lexlogprobFeature, logprob);
		return retval;
	}

	/**
	 * Returns a score for the given sign and completeness flag; specifically,
	 * returns the lex log prob total for the sign.
	 */
