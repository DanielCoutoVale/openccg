...
public final class AtomCat extends AbstractCat implements TargetCat {
	...
	/**
	 * Flag indicating whether this cat is a fragment with completion status
	...
	/**
	 * Constructor which creates an atomic category with the given type and
	 * feature structure.
	 */
	...
	/**
	 * Constructor which creates an atomic category with the given type, feature
	 * structure and LF.
	 */
	...
	public Element toXml() {
		...
		if (!_featStruc.isEmpty() || _featStruc.getIndex() > 0) {
			if (_featStruc instanceof GFeatStruc) { // only supporting
													// GFeatStruc per xml
													// construction
	...
	public Category copy() {
		AtomCat retval = new AtomCat(type, _featStruc.copy(), (_lf == null) ? null
				: (LF) _lf.copy());
	...
	public Object fill(Substitution s) throws UnifyFailure {
		AtomCat $ac = new AtomCat(type, (FeatureStructure) _featStruc.fill(s), (_lf == null) ? null
				: (LF) _lf.fill(s));
		return $ac;
	}
	...
	public String getSupertag() {
		...
		sb.append(type);
		if (_featStruc != null)
			sb.append(_featStruc.getSupertagInfo());
		if (sb.length() == 0)
			_supertag = "UnknownCat";
		else
	...
	public boolean equalsNoLF(Object obj, TObjectIntHashMap varMap, TObjectIntHashMap varMap2) {
		...
		if (_featStruc != null && ac._featStruc == null) {
			return false;
		}
		if (_featStruc == null && ac._featStruc != null) {
			return false;
		}
		if (!type.equals(ac.type)) {
			return false;
		}
		if (_featStruc != null) {
			if (_featStruc instanceof GFeatStruc) {
				if (!((GFeatStruc) _featStruc).equals(ac._featStruc, varMap, varMap2)) {
					return false;
				}
			} else { // nb: would be nice to get rid of this case
				if (!_featStruc.equals(ac._featStruc)) {
					return false;
				}
			}
		}
	...
	/**
	 * Returns whether this category is a fragment category; returns true iff
	 * the type of this cat is "frag".
	 */
	public boolean isFragment() {
		return type.equals("frag");
	}
}
