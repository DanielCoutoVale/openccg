...
public abstract class AbstractCat implements Category, Serializable {
	...
	/**
	 * Constructor which retrieves the LF from the XML element and flattens it
	 * to a conjunction of elementary predications (or a single one).
	 */
	...
	/**
	 * Adds an XML element for the LF, if any, to the given catElt. Uses
	 * {@link HyloHelper#toXml(LF)}.
	 */
	public void toXml(Element catElt) {
		if (_lf != null)
			catElt.addContent(HyloHelper.getInstance().toXml(_lf));
	}

	// during deserialization, intern computed supertag, and ensure varmap
	// recomputed
	private void readObject(java.io.ObjectInputStream in) throws IOException,
			ClassNotFoundException {
		in.defaultReadObject();
		if (_supertag != null)
			_supertag = _supertag.intern();
		_varMap = null;
	}
	...
	/**
	 * This will return a TeX formatted representation for a category. If
	 * toTeX() is not implemented for this category, the normal toString()
	...
	public abstract Object unify(Object u, Substitution sub) throws UnifyFailure;
	 ...
	 * order. The implementation calls hashCodeNoLF(varMap) and hashCode(varMap)
	 * on the LF.
	 */
	public int hashCode() {
		// NB: caching of the hash code has been turned off to avoid problems
		// with stale values;
		...
		if (_lf != null) {
			_hashCode += _lf.hashCode(_varMap);
		}
		return _hashCode;
	}

	/**
	 * Returns a hash code for this category ignoring the LF. The hash code
	 * handles equivalence up to variable names as long as features and
	 * predicates are in the same order. The implementation calls
	 * hashCodeNoLF(varMap).
	 */
	 ...
	 * same order. The implementation calls equalsNoLF(obj, varMap, varMap2).
	 */
	...
	private boolean equals(Object obj, boolean checkLF) {
		...
		if (!equalsNoLF(obj, _varMap, ac._varMap))
			return false;
		if (checkLF && _lf != null && !_lf.equals(ac._lf, _varMap, ac._varMap)) {
			return false;
		}
		return true;
	}
	 ...
	 */
	public abstract boolean equalsNoLF(Object obj, TObjectIntHashMap varMap,
