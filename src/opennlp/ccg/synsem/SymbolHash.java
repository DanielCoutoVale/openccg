...
public class SymbolHash extends THashSet {
	...
	/**
	 * Hashing strategy that uses Sign's surfaceWordHashCode and
	...
	public Set<Symbol> asSymbolSet() {
		return (Set<Symbol>) this;
	}

	/**
	 * Adds a sign, keeping the one with lower derivational complexity if there
	 * is an equivalent one there already; returns the old sign if it was
	 * displaced, the new sign if there was no equivalent old sign, or null if
	 * the sign was not actually added.
	 */
	...
	/**
	 * Compares signs by their derivation complexity, lists of words, then
	 * (somewhat desperately) cat hash codes.
	 */
	...
	public static int compareTo(List<Association> words1, List<Association> words2) {
		...
		while (i < words1.size() || i < words2.size()) {
			...
			if (i == words2.size())
				return 1;
			Association w1 = words1.get(i);
			Association w2 = words2.get(i);
