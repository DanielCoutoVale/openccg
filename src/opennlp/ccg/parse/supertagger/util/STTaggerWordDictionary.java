...
public class STTaggerWordDictionary implements STTaggerDictionary, Serializable {
	...
	/*
	 * This object represents our dictionary. The String is the word we want to
	 * look up, and the Pair holds the word frequency count (= `a') and the
	 * Collection<String> of the CCG lex. cat's seen with that word (= `b').
	 */
	...
	/**
	 * This method implements the interface STTaggerDictionary by delegating to
	 * <code>getEntry(String, int)</code> (see below).
	 */
	 ...
	 * <code>String</code> word, only if that word appears at least `freq' times
	 * in the corpus.
	 * 
	 * @param key A <code>String</code> representing a particular word.
	 * @param freq An <code>int</code> specifying the number of times a word
	 *            should have occured before it is returned.
	 * @return A <code>String[]</code> containing all supertags (CCG lexical
	 *         categories) seen with the specified word. Returns
	 *         <code>null</code> if the word does not appear at least `freq'
	 *         times in the corpus from which the dictionary was created or if
	 *         the word does not appear at all. (N.B. Passing in an
	 *         <code>int</code> `freq' value of zero will elicit the same
	 *         behavior as passing in a `freq' value of 1.)
	 */
	@SuppressWarnings("unchecked")
	public Collection<String> getEntry(String word, int freq) {
		if (word == null)
			return null; // mww: extra null check
	 ...
	 * entry for a particular <code>String</code> representing a word.
	 *
	 * @param key A <code>String</code> representing a particular word.
	 ...
	 *         that represent the word -> (freq, { ... supertags ...}) mappings
	 *         in the dictionary.
	 */
	public Iterator<Pair<String, Pair<Integer, Collection<String>>>> getMappings() {
		...
		while (keyset.hasNext()) {
			tempS = keyset.next();
			preRes.add(new Pair<String, Pair<Integer, Collection<String>>>(tempS, this.dict
					.get(tempS)));
		}
	...
	/**
	 * A method for getting the number of times a word was seen in the training
	 * data with which this dictionary was created.
	 * 
	 * @param word A <code>String</code> representing the word in question.
	 * @return An <code>int</code> count of this word's frequency in the corpus
	 *         with which this dictionary was created.
	 */
	@SuppressWarnings("unchecked")
