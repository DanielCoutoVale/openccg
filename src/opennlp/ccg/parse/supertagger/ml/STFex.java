...
public class STFex implements FeatureExtractor {
	...
	/**
	 * An object containing a ConditionalProbabilityTable that can give a prior
	 * distribution over all known supertags given a POS-tagged word.
	 * 
	...
	public void useMultiPOS(boolean trueOrFalse) {
		useMultiPOS = trueOrFalse;
	}

	/**
	 * Extracts an <code>ArrayList<String></code> representing the contextual
	 * predicates (features) of a line of (tokenised) text (each
	 * <code>String</code> represents the predicates that fire for a word in the
	 * line). Each resulting <code>String</code> will have the form: cp1 cp2 ...
	 * cpK
	 * 
	 * @param sentence A <code>List<Word></code> of feature bundles.
	 * @param wordIndex An <code>int</code> giving the location of the word to
	 *            be tagged.
	 * @return A <code>Collection<Pair<String,Double>></code> of real-valued
	 *         feature activations for the word at index <tt>wordIndex</tt>
	 */
	public Collection<Pair<String, Double>> getFeatures(Map<Integer, TaggedWord> sentence,
		...
		if (useMultiPOS) {
			for (Pair<Double, String> tg : current.getPOSTagging())
				result.add(new Pair<String, Double>(curP + tg.b, tg.a));
		} else {
		...
		if (useMultiPOS && prev != Constants.OOB) {
			for (Pair<Double, String> tg : prev.getPOSTagging())
				result.add(new Pair<String, Double>(prevP + tg.b, tg.a));
		} else {
		...
		if (useMultiPOS && prevPrev != Constants.OOB) {
			for (Pair<Double, String> tg : prevPrev.getPOSTagging())
				result.add(new Pair<String, Double>(prevPrevP + tg.b, tg.a));
		} else {
		...
		if (useMultiPOS && next != Constants.OOB) {
			for (Pair<Double, String> tg : next.getPOSTagging())
				result.add(new Pair<String, Double>(nextP + tg.b, tg.a));
		} else {
		...
		if (useMultiPOS && nextNext != Constants.OOB) {
			for (Pair<Double, String> tg : nextNext.getPOSTagging())
				result.add(new Pair<String, Double>(nextNextP + tg.b, tg.a));
		} else {
			result.add(new Pair<String, Double>(nextNextP + nextNext.getPOS(), activation));
		}

		// now for conjunctions of features: w-2w-1=..., w-1w+1=..., w+1w+2=...
		// (same for posp).
		// (i.e., bigram features over words and parts of speech and bigrams of
		// words and POSs that straddle the current token).
		...
		for (int j = 1; j < wds.length; j++) {
			...
			result.add(new Pair<String, Double>(lxfLabs[j - 1] + "|" + lxfLabs[j] + "="
					+ wds[j - 1].getForm() + "|" + wds[j].getForm(), activation));
			result.add(new Pair<String, Double>(posfLabs[j - 1] + "|" + posfLabs[j] + "="
					+ wds[j - 1].getPOS() + "|" + wds[j].getPOS(), activation));
			// also, if at the current word slot, add bigrams that straddle the
			// current word.
			if (j == 2) {
				result.add(new Pair<String, Double>(lxfLabs[j - 1] + "|" + lxfLabs[j + 1] + "="
						+ wds[j - 1].getForm() + "|" + wds[j + 1].getForm(), activation));
				result.add(new Pair<String, Double>(posfLabs[j - 1] + "|" + posfLabs[j + 1] + "="
						+ wds[j - 1].getPOS() + "|" + wds[j + 1].getPOS(), activation));
			}
		}

		// If the prior model is not null, extract a feature for the beta-best
		// (beta = 0.1) classes
		// predicted by the prior model (for all output classes -- supertags --
		// seen with this word's
		// POS).
		// Extract prior features from these.
		if (priorMod != null) {
			priorMod.computePriors(current.getWord());
			for (Pair<String, Double> priorClassActivationPair : priorMod
					.getRestrictedBetaBestPriors(current.getWord(), 0.1)) { // TODO:
																			// make
																			// beta
																			// parameterizable.
				double act = Math.log(priorClassActivationPair.b);
				String wd = current.getForm().intern(), pos = current.getPOS().intern();
				result.add(new Pair<String, Double>(priorST + priorClassActivationPair.a.intern(),
						act)); // log(prob)
				result.add(new Pair<String, Double>(wordPriorST
						+ priorClassActivationPair.a.intern() + "_" + wd, act)); // log(prob)
				result.add(new Pair<String, Double>(wordPOSPriorST
						+ priorClassActivationPair.a.intern() + "_" + wd + "_" + pos, act)); // log(prob)
				result.add(new Pair<String, Double>(POSPriorST
						+ priorClassActivationPair.a.intern() + "_" + pos, act)); // log(prob)
				result.add(new Pair<String, Double>(antiPriorST
						+ priorClassActivationPair.a.intern(), Math.log(1 - Math.exp(act)))); // log(1-prob)
				// TODO: come up with sensible "anti-prior" features that
				// simulate the filtering effect of the tagging dict.
			}
		}
		return result;
	}
	...
	public List<Collection<Pair<String, Double>>> getSentenceFeatures(
		...
		List<Collection<Pair<String, Double>>> res = new ArrayList<Collection<Pair<String, Double>>>(
				sentence.size());
	...
	// pass in a POS tagger config file, if we aren't only using gold POS tags
	// only.
	// input corpus is from stdin, output goes to stdout.
	public static void main(String[] args) {
		// we assume that the training data is being streamed in from stdin (no
		// parse IDs, just SRILM factor bundle lines),
