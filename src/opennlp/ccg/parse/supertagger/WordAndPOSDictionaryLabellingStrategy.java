...
/**
 * A `labelling strategy' for a CCG supertagger that restricts the output of the
 * model based on word and POS `tagging dictionaries' in the following way:
 *
 * if a word w occurs at least K times in training, the model's output is
 * constrained to the outcomes seen with w during training. If w did not occur
 * at least K times during training, the model's output is constrained to the
 * outcomes seen with w's POS tag during training. In the off chance that the
 * POS tag was not seen in training, the model's best prediction is used.
 *
...
public class WordAndPOSDictionaryLabellingStrategy implements LabellingStrategy, Supertagger {
	...
	// POS-specific multipliers to "tighten" or "loosen" up the tagging beam
	// width
	// ("beta") as needed. E.g., the beta for period/full stop might not need to
	// be
	// very permissive, while those for lexical verbs or some fancy punctuation
	// marks
	// might need to be.
	...
	public WordAndPOSDictionaryLabellingStrategy(STTaggerWordDictionary wd,
			STTaggerPOSDictionary pd, int K, MaxentModel mo, FeatureExtractor fexer) {
	...
	public WordAndPOSDictionaryLabellingStrategy(STTaggerWordDictionary wd,
			STTaggerPOSDictionary pd, int K, MaxentModel mo, FeatureExtractor fexer,
			String tagSequenceModel, Constants.TaggingAlgorithm alg) {
	...
	public WordAndPOSDictionaryLabellingStrategy(STTaggerWordDictionary wd,
		...
		try {
			...
			Logger.getLogger(WordAndPOSDictionaryLabellingStrategy.class.getName()).log(
	...
	public WordAndPOSDictionaryLabellingStrategy(STTaggerWordDictionary wd,
			STTaggerPOSDictionary pd, int K, MaxentModel mo) {
	...
	public void setMaxSearchBeam(int maxSearchBeam) {
		if (seqScorer != null)
			seqScorer.setSearchBeam(maxSearchBeam);
	}
	...
	/**
	 * A method that returns all labels given by the model that both (1) are
	 * assigned probability `p' s.t.: p>=(`beta'*<bestProb>), where `beta' is a
	 * factor passed in by the client of this method and where <bestProb> is the
	 * probability of the most probably outcome of the model and (2) (if the
	 * word (obtained from the <code>String[]</code> `context') has occured at
	 * least K times in training) are in the <code>STTaggerWordDictionary</code>
	 * under the entry for said word. If the word did not occur at least K times
	 * in training, the output set is constrained by a
	 * <code>STTaggerPOSDictionary</code>. In the off chance that a POS did not
	 * occur in the training data, the models predictions themselves are
	 * submitted to the `beta constraint'.
	 *
	 * @param context A <code>String[]</code> of contextual predicates (in the
	 *            maximum entropy modelling sense)
	 * @param mo A <code>MaxentModel</code>.
	 * @param beta A <code>double</code> specifying how close in probability all
	 *            returned outcomes must be.
	 * @return An <code>ArrayList<String></code> of labels that meet the above
	 *         constraints.
	 */
	public List<String> multitag(Association w, Collection<Pair<String, Double>> context,
			double beta) {
	 ...
	 * the best label multiplied by a factor `beta', given a model and a
	 * <code>String[]</code> of contextual predicates.
	 * 
	 * @param thisWord a <code>opennlp.ccg.lexicon.Word</code> representing the
	 *            current word being tagged.
	 * @param context A <code>Collection<Pair<String,Double>></code> of
	 *            contextual predicates (in the maximum entropy modelling sense)
	 *            with their corresponding activations (real-valued, hence the
	 *            <code>Double</code>).
	 * @param model A model for generating the base predictions.
	 * @param beta A positive <code>double</code> specifying how close to the
	 *            best label each label returned must be.
	...
	public List<Pair<Double, String>> multitagWithScores(Association thisWord,
			Collection<Pair<String, Double>> context, double beta) {
		...
		bestOutcomeProb = 0;
		// mww: changed to not always include front of list, as it may not meet
		// dict constraints
		ProbIndexPair temp;
		...
		Collection<String> wordPermittedOutcomes = (wd != null) ? this.wd.getEntry(word, this.K)
				: null;
		if (wordPermittedOutcomes != null && useWordDict) {
			...
			for (int ocInd = 0; ocInd < sortedOutcomes.length; ocInd++) {
				...
				if (wordPermittedOutcomes.contains(tempOutcome)) {
					...
					if (currentOutcomeProb >= (bestOutcomeProb * beta)) { // Beta
																			// constraint.
						...
						// Else, since our ProbIndexPair[] is sorted by
						// probablity, there will be no more
						// outcomes that make the (beta) cut.
						break;
					}
				} // If the word is not in the dictionary specified outcomes,
					// move along.
			...
			if (pos != null) {
				...
				} // mww: check for null pos

			}
			if (posPermittedOutcomes != null && usePOSDict) {
				// Get all beta-OK outcomes that are in the POS dictionary
				// entry.
				for (int ocInd2 = 0; ocInd2 < sortedOutcomes.length; ocInd2++) {
					...
					} // If the word is not in the dictionary specified
						// outcomes, move along.

				}
			} else {
				// Otherwise, just get all model predictions that meet the beta
				// constraint,
				...
				for (int ocInd3 = 0; ocInd3 < sortedOutcomes.length; ocInd3++) {
					...
					if (currentOutcomeProb >= (bestOutcomeProb * beta)) {
						// Made the cut-off, add the outcome.
						retVal.add(new Pair<Double, String>(temp.a,
								mo.getOutcome(temp.b.intValue())));
						// update max, for first selected outcome
						if (currentOutcomeProb > bestOutcomeProb) {
							bestOutcomeProb = currentOutcomeProb;
						}
						...
						// Else, since our ProbIndexPair[] is sorted by
						// probability, there will be no more
		...
		if (includeGold) {
			...
			if (!containsGold) {
				// if the gold-standard still isn't in there, it must not be
				// part of the tag set, add it with epsilon probability.
				// we're assuming that gold tags are needed for a training
				// routine that doesn't care about supertag probabilities
				// (as in Clark and Curran (2007)).
				// check to see whether we are in the log domain (by checking
				// for negative scores -- kind of a hack).
				retVal.add(new Pair<Double, String>((sortedOutcomes[0].a < 0) ? -99
						: 1.0112214926104486e-43, thisWord.getSupertag()));
			}
		}
		// *******************************************************************************************
		return retVal;
	...
	public List<List<Pair<Double, String>>> multitag(List<Association> sentence, double beta) {
		List<List<Pair<Double, String>>> results = new ArrayList<List<Pair<Double, String>>>(
				sentence.size());
		...
		// Iterate simultaneously through both the words and the contextual
		// features.
		...
		while (wds.hasNext() && ctxts.hasNext()) {
			...
			if (w.getFunctions() == null) {
				w = WordPool.createFullWord(w, w.getForm(), tagging.get(cursor).getPOSTagging()
						.get(0).b, w.getSupertag(), w.getEntityClass());
			}
			context = ctxts.next();
			if (seqScorer != null) {
				// increase the tag ambiguity (for re-scoring using
				// forward-backward).
				double newBeta = Math.min(beta * minMultiplier, beta / 8);
				if (beta < 0.00001) {
					newBeta = Math.min(beta * minMultiplier, beta / 2);
				}
				results.add(multitagWithScores(w, context, newBeta));
			} else {
				results.add(multitagWithScores(w, context, beta));
			}
			cursor++;
		}
		...
		if (seqScorer != null) {
			// rescore and filter. pass in input sentence (in case, e.g., we
			// have set the includeGold flag).
	...
	/**
	 * Return a beta-best filtered subset of the tags in each multitagging list
	 * (each multitagging list is assumed to be non-empty).
	 */
	private List<List<Pair<Double, String>>> betaBestFilter(
			List<List<Pair<Double, String>>> multitaggings, double beta,
			List<Association> inputSentence) {
		List<List<Pair<Double, String>>> res = new ArrayList<List<Pair<Double, String>>>(
				multitaggings.size());

		int wordIndex = 0;
		for (List<Pair<Double, String>> mtagging : multitaggings) {
			List<Pair<Double, String>> tempTagging = new ArrayList<Pair<Double, String>>(
					mtagging.size());
			Association thisWord = inputSentence.get(wordIndex);
			// set to a (possibly different, possibly less restrictive?) beta if
			// this POS has a beta multiplier set.
			...
			for (Pair<Double, String> tg : mtagging) {
				if (tg.a >= (possiblyNewBeta * best)
						|| (includeGold && tg.b.equals(thisWord.getSupertag()))) {
					tempTagging.add(tg);
				} else {
					if (!includeGold) { // if we're not still fishing for
										// gold...
	...
	/**
	 * Returns whether there are any less restrictive beta settings remaining in
	 * the sequence.
	 */
	...
	/**
	 * Returns whether there are any more restrictive beta settings remaining in
	 * the sequence.
	 */
	...
	/**
	 * Maps the given words to their predicted categories, so that the beta-best
	 * categories can be returned by calls to setWord and getSupertags.
	 */
	...
	/**
	 * Sets the current word to the one with the given index, so that the
	 * beta-best categories for it can be returned by a call to getSupertags.
	 */
	...
	/**
	 * Returns the supertags of the desired categories for the current lexical
	 * lookup as a map from supertags to contextual probabilities (or null to
	 * accept all).
	 */
	...
	/**
	 * A factory method to make a supertagger from a config file (see the sample
	 * config file:
	 * 
	...
	public static WordAndPOSDictionaryLabellingStrategy supertaggerFactory(String configFile) {
		...
		String[] pathKeys = { "priormodel", "priormodelvocab", "sequencemodel", "wdict", "posdict",
				"maxentmodel", "posconfig" };
		...
		String priorModS = opts.get("priormodel"), priorVocabS = opts.get("priormodelvocab"), seqModS = opts
				.get("sequencemodel"), wDictS = opts.get("wdict"), pDictS = opts.get("posdict"), firstKS = opts
				.get("firstk"), lastKS = opts.get("lastk"), maxentModS = opts.get("maxentmodel"), posConfigS = opts
				.get("posconfig"), betasS = opts.get("betas"), betaMults = opts
				.get("betamultipliers"), // POS-specific multipliers to
											// "tighten" or "loosen" up the
											// tagging beam width.
		...
		assert (priorModS != null && priorVocabS != null)
				|| (priorModS == null && priorVocabS == null) : "using prior model with no vocab file.";
		...
		STTaggerWordDictionary wD = (wDictS != null) ? new XMLWordDictionaryReader(new File(wDictS))
				.read() : null;
		STTaggerPOSDictionary pD = (pDictS != null) ? new XMLPOSDictionaryReader(new File(pDictS))
				.read() : null;
		...
		TaggingAlgorithm alg = (opts.get("taggingalgorithm") == null || opts
				.get("taggingalgorithm").equals("forward-backward")) ? TaggingAlgorithm.FORWARDBACKWARD
				: TaggingAlgorithm.FORWARD;
		MaxentModel mem = new ZLMEM(new File(maxentModS));
		// STTaggerWordDictionary wd,STTaggerPOSDictionary pd, int K,
		// MaxentModel mo, FeatureExtractor fexer,
		// String tagSequenceModel, Constants.TaggingAlgorithm alg, POSTagger
		// posTagger
		res = (pTagger != null) ? new WordAndPOSDictionaryLabellingStrategy(wD, pD, kay, mem, fex,
				seqModS, alg, pTagger) : new WordAndPOSDictionaryLabellingStrategy(wD, pD, kay,
				mem, fex, seqModS, alg);
		res.setK(kay);
		...
		res.setIncludeGold((opts.get("includegold") == null || opts.get("includegold").equals(
				"false")) ? false : true);

		// get POS-specific beta multipliers (as a string of <POS,double> pairs
		// -- all space delimited).
		if (betaMults != null) {
	...
	public static void main(String[] args) {
		...
		try {
			...
			String inputCorp = "<stdin>", output = "<stdout>", configFile = null;
			...
			WordAndPOSDictionaryLabellingStrategy stgger = WordAndPOSDictionaryLabellingStrategy
					.supertaggerFactory(configFile);

			// for each sentence, print out:
			// <s>
			// w1 <numPOSTags> <posTag1> ... <posTagK> <numSupertags>
			// <supertag1> ... <supertagL>
			// ...
			// wN <numPOSTags> <posTag1> ... <posTagM> <numSupertags>
			// <supertag1> ... <supertagU>
			// </s>
			for (List<Association> inLine : in) {
				...
				while (++cursor < taggedSent.size()) {
					...
					// print out number of POS tags, followed by tab-separated
					// probabilized POS tagging.
					out.write("\t" + posT.getPOSTagging().size());
					for (Pair<Double, String> pt : posT.getPOSTagging()) {
						out.write("\t" + pt.b + "\t" + pt.a);
					}
					// now print out number of and list of tab-separated,
					// probabilized supertags.
