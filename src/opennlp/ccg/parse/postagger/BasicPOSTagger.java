...
public class BasicPOSTagger extends POSTagger {
	...
	public List<TaggedWord> tagSentence(List<Association> sentence) {
		...
		// the prob-string taggings (to be filtered, etc. before adding them to
		// the taggings of the TaggedWord list).
		List<List<Pair<Double, String>>> taggings = new ArrayList<List<Pair<Double, String>>>(
				sentence.size());
		...
		for (Collection<Pair<String, Double>> fts : ftss) {
			...
			List<Pair<Double, Integer>> distroList = new ArrayList<Pair<Double, Integer>>(
					distro.length);
			ind = 0;
			...
			// widen beta a little bit (we're going to do some fwd-bwd rescoring
			// inp a minute, but we don't
			// want to do the fwd-bwd alg over ALL possible tags -- too
			// inefficient).
			...
			for (Pair<Double, Integer> outcome : distroList) {
				if ((outcome.a >= (widenedBeta * best))
						|| (includeGold && tagMod.getOutcome(outcome.b).equals(goldPOS))) {
					tagging.add(new Pair<Double, String>(outcome.a, tagMod.getOutcome(outcome.b)));
				} else {
					if (!includeGold) { // if not still potentially fishing for
										// a gold POS tag, then break (they're
										// in sorted order).
						break;
					}
				}
	...
	public static void main(String[] args) throws IOException {
		...
		try {
			...
			for (List<Association> inLine : inp) {
				...
				List<List<Pair<Double, String>>> sentTagging = new ArrayList<List<Pair<Double, String>>>(
						taggedSent.size());
