...
public abstract class POSTagger {
	...
	public void setIncludeGold(boolean includeGoldOrNot) {
		includeGold = includeGoldOrNot;
	}

	/**
	 * Filter the POS tags by the beta filter and return the (potentially)
	 * trimmed-down results. It is assumed that the tags of the tagged word are
	...
	public List<TaggedWord> betaBestFilter(List<TaggedWord> sentence) {
		...
		for (TaggedWord tw : sentence) {
			...
			List<Pair<Double, String>> tmpTagging = new ArrayList<Pair<Double, String>>(tw
					.getPOSTagging().size());
	...
	/**
	 * Build a POS tagger from a config file. A non-dummy config file might
	 * contain: ... # this is a comment taggerType=basic
	 * priorModel=/home/.../posprior/prior.flm # note that CaSE dOES Not matter
	 * for the key (but does for the value, e.g. a file name).
	 * PRIORmodelvocab=/home/.../posprior/vocab.voc # you can also repeat
	 * opions. the last one will take effect.
	 * priormodelvocab=/home/.../posprior/vocab2.voc
	 * 
	 * # did you see that empty line get ignored? # notice that spaces around
	 * the '=' get ignored, as well. maxentModel =
	 * /home/.../maxentmodels/myposmod.mod # this last must be an ARPA-formatted
	 * n-gram model over POS tags (7-grams work well).
	 * sequenceModel=/home/.../pos.lm # lastly, the tagging beam width (1.0
	 * means "single-best" -- i.e., a unitagger). beta=0.1
	 */
	public static POSTagger posTaggerFactory(String configFile) {
		...
		try {
			...
			TaggingAlgorithm alg = (opts.get("taggingalgorithm") == null || opts.get(
					"taggingalgorithm").equals("forward-backward")) ? TaggingAlgorithm.FORWARDBACKWARD
