...
public class POSPriorModel extends ConditionalProbabilityTable {
	...
	public double getPriorOf(String pos, String word) {
		attrVals.clear();
		Pair<String, String> surfaceForm = pairs.intern(new Pair<String, String>(WORD, word
				.intern()));
		attrVals.add(surfaceForm);
		Pair<String, String> partOfSpeech = pairs.intern(new Pair<String, String>(POS_TAG, pos
				.intern()));
	...
	/*
	 * added by DCE, to facilitate use in hypertagging Identical to above
	 * method, but accepts a String (name of EP) rather than a Word object.
	 */
		...
		try {
			...
			for (List<Association> inLine : in) {
				for (Association w : inLine) {
					String pos = POS_TAG + "-" + DefaultTokenizer.escape(w.getFunctions()), wform = WORD
