 ...
 * 
 * The inputs are "TaggedWord"s simply for consistency of interface. There
 * should be no tags assigned to the words (short, perhaps, TOBI tags or the
 * like).
 * 
...
public class POSTagFex implements FeatureExtractor {
	...
	/**
	 * Get a sentence of words' features for applying the tagger (i.e., not
	 * training mode).
	 */
	...
	/**
	 * Get the features for a word in context. training == true iff the output
	 * class is to be collected as well.
	 * 
	 * TODO: This and supertagger feature extractor (fex) should be merged into
	 * a more general, parameterizable sentence-level contextual feature
	 * extractor. (VERY todo-ish, though.)
	 */
	public Collection<Pair<String, Double>> getFeatures(Map<Integer, TaggedWord> sentence,
		...
		// these are in a separate sequence model (n-gram model over POS
		// sequences).

		// standard contextual features (word to the left, current word, word to
		// the right, etc.).
		...
		// add real-valued (activation = prior log-prob) features for each of
		// the beta-best prior
		...
		if (posPrior != null) {
			...
			for (Pair<Double, String> prior : priors) {
				if (prior.a > (beta * best)) {
					// add the features PPOS=<POSTAG>:<log-prob> and
					// PPOS_word=<POSTAG>_<wordForm>:<log-prob>.
					result.add(new Pair<String, Double>(priorF + "=" + prior.b, prior.a));
					result.add(new Pair<String, Double>(priorF + "_word" + "=" + prior.b + "_"
							+ wform, prior.a));
				} else {
					break;
				}
		...
		// if the word's length is > 4, then extract the 1-, 2-, 3- and
		// 4-character affixes.
		if (current.getForm().length() > 4) {
			...
			for (cursor = 0; cursor < 4; cursor++) {
				...
				result.add(new Pair<String, Double>(prefix + "=" + prefixes.toString(),
						Constants.one));
			}
			// suffixes.
			for (cursor = wdForm.length - 1; cursor >= wdForm.length - 5; cursor--) {
				suffixes.insert(0, wdForm[cursor]);
				result.add(new Pair<String, Double>(suffix + "=" + suffixes.toString(),
						Constants.one));
			}
		}

		// now do "contains hyphen", "contains number",
		// "contains uppercase letter" and contains fused NE connecter (_)
		// features.
		...
		if (current.getForm().contains(neConnecter)) {
	...
	/**
	 * Get the features for a sentence of words in context. training == true iff
	 * the output classes are to be collected as well.
	 */
	public List<Collection<Pair<String, Double>>> getSentenceFeatures(
			Map<Integer, TaggedWord> sentence, boolean training) {
		List<Collection<Pair<String, Double>>> result = new ArrayList<Collection<Pair<String, Double>>>(
				30);
	...
	public static void main(String[] args) throws IOException {
		...
		SRILMFactoredBundleCorpusIterator corp = new SRILMFactoredBundleCorpusIterator(
				input.equals("<stdin>") ? new BufferedReader(new InputStreamReader(System.in))
						: new BufferedReader(new FileReader(new File(input))));
		BufferedWriter out = new BufferedWriter(output.equals("<stdout>") ? new BufferedWriter(
				new OutputStreamWriter(System.out)) : new BufferedWriter(new FileWriter(new File(
				output))));
