...
public class DerivationHistory implements Serializable, Comparable<DerivationHistory> {
	...
	private String toString(int maxRuleLen) {
		...
		for (int i = (ruleName.length() + 2); i < maxRuleLen; i++) {
			sb.append(' ');
		}
		sb.append(_output.toString()).append('\n');
		// done
	...
	/**
	 * Returns the complexity of the derivation, as the sum of the number of
	 * steps, plus the number of composition or substitution steps, plus the
	...
	public boolean containsCycle() {
		if (_noHistory || _inputs.length != 1)
			return false;
	...
	public int compareTo(DerivationHistory dh) {
		...
		if (_noHistory)
			return 0;
		if (_inputs.length < dh._inputs.length)
			return -1;
		if (_inputs.length > dh._inputs.length)
			return 1;
		for (int i = 0; i < _inputs.length; i++) {
			int cmp = _inputs[i].getDerivationHistory().compareTo(
					dh._inputs[i].getDerivationHistory());
			if (cmp != 0)
				return cmp;
		}
		return 0;
