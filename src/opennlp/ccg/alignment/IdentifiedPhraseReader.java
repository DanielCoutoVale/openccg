...
/**
 * Phrase reader for phrases that have an identifier in addition to a line
 * number, usually formatted by markup like <blockquote>
 * 
 * <pre>
 * &lt;s snum=&quot;157&quot;&gt; ... &lt;/s&gt;
 * </pre>
 * 
 * </blockquote> Since the value of the identifier is not necessarily an
 * {@link Integer}, this class gives IDs as strings. The
 * {@linkplain #getPhraseIdentifierAttribute() phrase identifier attribute}
 * (here <code>snum</code>) and {@linkplain #getPhraseTag() phrase tag} (here
 * <code>s</code>) are configurable when instances are constructed.
 * 
...
public class IdentifiedPhraseReader extends PhraseReader {
	...
	/**
	 * Creates an identified phrase reader with the
	 * {@linkplain Alignments#DEFAULT_PHRASE_NUMBER_BASE default number base}.
	 * 
	...
	/**
	 * Creates an identified phrase reader with the
	 * {@linkplain Alignments#DEFAULT_PHRASE_TAG default phrase tag} and
	 * {@link Alignments#DEFAULT_PHRASE_IDENTIFIER_ATTRIBUTE default phrase
	...
	/**
	 * Creates an identified phrase reader for the specified phrase tag and
	 * identifier attribute.
	 * 
	 * @param phraseTag The phrase tag that will be used to parse identifiers.
	 * @param phraseIdentifierAttribute The attribute that will denote
	 *            identifiers when parsing phrases.
	 * @throws IllegalArgumentException if <tt>phraseIdentifierAttribute</tt> or
	 *             <tt>phraseTag</tt> is <tt>null</tt>.
	 */
	...
	/**
	 * Reads a phrase from the underlying input stream, first parsing the
	 * {@linkplain Phrase#getNumber() phrase's number} based on the
	 * {@linkplain #getPhraseTag() phrase tag} and
	 * {@linkplain #getPhraseIdentifierAttribute() identifier attribute} being
	 * used.
	 * 
	 * @return A phrase with the ID signaled in the input.
	 * @throws IOException If the underlying input contains ill-formated phrase
	 *             markup, or if no ID cannot be determined after parsing the
	 *             phrase markup.
	 */
	@Override
