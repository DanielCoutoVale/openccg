...
/**
 * Class for reading {@link Mapping}s from an underlying reader based on a
 * specified {@link MappingFormat}.
 * <p>
 * Instances of this class read mappings group by group, with
 * {@link #nextGroup()} called after reading the correct number of mappings for
 * the current group, as signaled by the {@link #canRead()} method.
 * <p>
 * The following code fragment illustrates the usage of this class: <blockquote>
 * 
 * <pre>
...
public class MappingReader extends FilterReader {
	 ...
	 * 
	 * @return The next {@link MappingGroup} found by reading from the
	 *         underlying reader.
	 * @throws IOException If a {@link ParseException} is encountered when
	 *             calling {@link MappingFormat#parseMapping(String)} based on
	 *             the underlying input, or if one is thrown by the underlying
	 *             reader. An IOException is also thrown if the number of
	 *             mappings in the {@linkplain MappingGroup#getLength() current
	...
	public MappingGroup nextGroup() throws IOException {
		...
		currentGroup = (newCount == 0) ? null : new MappingGroup(mappingQueue.peek().phraseNumber,
				newCount);
		...
		while (!eog) {
			...
			try {
				...
				throw new IOException(((currentGroup == null) ? "" : "group "
						+ currentGroup.phraseNumber + ": ")
						+ "problem formatting mapping "
						+ sb.toString()
						+ " at offset "
						+ pe.getErrorOffset() + ": " + pe.getMessage(), pe);
			}

			// if the format allows null IDs, use previous's running counter
			if (currentGroup == null) {
				Integer I = (a.phraseNumber == null) ? (previous == null) ? format.encodingScheme
						.getPhraseNumberBase().start : previous.phraseNumber + 1 : a.phraseNumber;
	...
	/**
	 * Tests whether mappings can be read from this reader without throwing an
	 * {@link IOException}.
	 * 
	 * @return true If there is a current mapping group and mappings remain to
	 *         be read from it.
	 * @see #nextGroup()
	 * @see #readMapping()
	 */
	...
	/**
	 * Overrides the superclass method to check first if any mappings are
	 * available.
	 * 
	 ...
	 * 
	 * @return true If it is guaranteed that a call to {@link #readMapping()}
	 *         will not block for input.
	 */
	@Override
	...
	/**
	 * Reads a mapping from the underlying reader, if one is
	 * {@linkplain #canRead() available}.
	 * 
	 * @return A mapping formatted by the {@linkplain #getFormat() format in
	 *         effect}.
	...
	/**
	 * Closes this reader, checking first if the correct number of mappings were
	 * read.
	 * 
	...
	void checkMappingCount() throws IOException {
		if ((currentGroup == null && mappingCount > 0)
			...
			throw new IOException(currentGroup == null ? "" : "group " + currentGroup.phraseNumber
					+ ": " + "mapping count does not match: expected "
					+ ((currentGroup == null) ? 0 : currentGroup.length) + ", but was "
					+ mappingCount);
		}
	}
}
