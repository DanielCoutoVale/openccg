...
/**
 * <p>
 * An edge is a tracker for a sign, ie a sign together with bitsets representing
 * its coverage of the input predicates and the semantic indices used, along
 * with lists of the active LF alts. It also has a completeness percentage and a
 * score, as well as its most specific incomplete LF chunk (if any). Edges are
 * created by an EdgeFactory and managed by a Chart.
 * </p>
 * <p>
 * A representative edge is an edge that represents (stands in for) other edges
 * with the same category during the chart generation process. A representative
 * edge has a list of alternative edges, which are assumed to share the same
 * category; it is considered disjunctive when there is more than one
 * alternative. Note that initially a representative edge will be in its list of
 * alternatives, but it can be removed during pruning. Finally, for the anytime
 * search, a representative edge can maintain a collection of successful
 * combinations, to avoid invoking the combinatory rules multiple times with the
 * same combinations of categories.
 * </p>
 *
...
public class Edge extends Tracker {
	...
	/**
	 * The edge, if any, that this edge is constructed from by marking optional
	 * bits as completed.
	 */
	protected Edge optCompletes = null;

	/** Constructor. */
	public Edge(Symbol sign, BitSet bitset, BitSet indices, float completeness, double score,
	...
	/**
	 * Returns whether this edge has completeness 1.0, ie, covers all the input
	 * preds.
	 */
	...
	/**
	 * Returns true iff this edge can combine with the given tracker without
	 * violating its LF chunk constraint (if any). Specifically, returns true
	 * when this edge has no incomplete chunk or the tracker is semantically
	 * empty; otherwise, returns true iff the incomplete chunk intersects with
	 * the tracker's coverage vector.
	 */
	public boolean meetsLfChunkConstraints(Tracker tracker) {
		if (incompleteLfChunk == null || tracker.bitset.isEmpty())
			return true;
	...
	/**
	 * Returns a hash code for this edge based on the surface words, ignoring
	 * the LF and ignoring the score.
	 */
	...
	/**
	 * Returns whether this edge equals the given object. (Alternatives are not
	 * considered.)
	 */
	...
	/**
	 * Returns whether this edge equals the given object based on the surface
	 * words, ignoring the LF and ignoring the score.
	 */
	...
	/**
	 * Returns a string for the edge in the format {completeness} [score]
	 * orthography :- category {bitset}.
	 */
	public String toString() {
		...
		// sbuf.append(indices + " ");
		if (showCompleteness) {
			sbuf.append("{" + nf2.format(completeness) + "} ");
		}
		...
		if (showBitset) {
			sbuf.append(' ').append(toString(bitset));
		}
