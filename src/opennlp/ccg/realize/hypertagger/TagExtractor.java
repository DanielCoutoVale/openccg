...
public abstract class TagExtractor {
	...
	protected class LfGraphNode {
		...
		protected String POS; // caches the part-of-speech tag for this node (or
								// GS postag)
		...
		/*
		 * public boolean isLexPred() { return
		 * HyloHelper.getInstance().isLexPred(data); }
		 */
	 ...
	 * ... Values are arbitrary strings, but a key can have multiple values.
	 * Thus, the values are actually of type ArrayList<String>.
	 * 
	 * @author espinosa
	 *
	 */
	protected class FeatureList extends HashMap<String, ArrayList<String>> {
		...
		protected void addFeature(String featureName, String value) {
			/*
			 * if(value == null) { return; } if(this.get(featureName) == null) {
			 * this.put(featureName.trim(), new ArrayList<String>()); }
			 * ArrayList<String> feats = this.get(featureName);
			 * feats.add(value.trim()); this.put(featureName.trim(), feats);
			 * numFeatures++;
			 */
		 ...
		 * for(Map.Entry<String,ArrayList<String>> e : es) { String fn =
		 * e.getKey(); for(String fv : e.getValue()) { this.addFeature(fn, fv);
		 * } } }
		 */
	...
	/**
	 * This method takes an LF and extracts its features, changing the internal
	 * state of this object accordingly. Other methods such as getSupertag() can
	 * then be called to obtain the tagger's prediction.
	 * 
	 * @param preds A logical form
	 * @throws FeatureExtractionException when the logical form cannot be
	 *             processed and the extracted features will not be meaningful
	 */
	public void setLF(List<SatOp> preds) throws FeatureExtractionException {
		...
		for (SatOp s : preds) {
			if (s == null) {
				throw new FeatureExtractionException(); // ??? why is it
														// sometimes null?
			}
		...
		for (SatOp s : preds) {
			...
			if (d.getArg() instanceof NominalVar) {
				...
				// System.err.println("found var: str = " +
				// d.getArg().toString());
				...
				ln.setSource(curNode);
				if (target != null) {
					// ??? why is it sometimes null? and if it is, should an
					// exception be thrown?
					target.setParentLink(ln);
					target.addMultiParent(curNode);
				} else {
					// System.err.println("TE: target was null: " +
					// d.getArg().toString());
				}
				if (curNode == null) {
					throw new FeatureExtractionException();
				...
				try {
					...
					// ??? this null must occur because there were no
					// attributes, but this
	...
	/**
	 * This method extracts features from a node in the graph and returns them
	 * as an array of strings. It's implemented slightly differently in
	 * HyperTagger and POSTagExtractor.
	 * 
	...
	/**
	 * @param predInfo
	 * @throws FeatureExtractionException This method stores gold standard
	 *             predicate info, which is expected to be in the following
	 *             format: predInfo :: (field<space>)+ field ::
	 *             wordId:supertag:POStag:predName
	 * 
	 *             The supertag and postag are expected to have been escaped by
	 *             Lexicon.DefaultTokenizer.escape(), and will be unescaped
	 *             during storage. This method throws a runtime error if the
	 *             predInfo string cannot be parsed.
	 * 
	 */
