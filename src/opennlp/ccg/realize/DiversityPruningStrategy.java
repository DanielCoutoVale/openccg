...
/**
 * Abstract n-best edge pruning strategy that keeps edges diversified according
 * to the notCompellinglyDifferent equivalence relation. The edges are clustered
 * into a ranked list of equivalence classes, which are sequentially sampled
 * until the limit n is reached to determine the edges to keep. If the
 * singleBestPerGroup flag is set, then a maximum of one edge per equivalence
 * class is retained.
 *
...
abstract public class DiversityPruningStrategy extends NBestPruningStrategy {
	/**
	 * Flag for whether to keep only the single best edge among those that are
	 * not compellingly different (defaults to false).
	 */
	 ...
	 * which should have equivalent categories and be sorted by score, from
	 * highest to lowest. In particular, prunes and returns the edges that fall
	 * below the N-best cutoff when the diversity strategy determined by
	 * notCompellinglyDifferent is applied. If the singleBestPerGroup flag is
	 * set, no more than one edge per group of equivalent ones will be returned.
	 */
	public List<Edge> pruneEdges(List<Edge> catEdges) {
		...
		if (!singleBestPerGroup && catEdges.size() <= CAT_PRUNE_VAL)
			return retval;
		...
		while (keepers.size() < CAT_PRUNE_VAL && (!singleBestPerGroup || counter < numGroups)) {
