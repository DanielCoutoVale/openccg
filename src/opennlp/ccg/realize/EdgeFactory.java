...
/**
 * The EdgeFactory is responsible for creating edges. A single edge factory
 * instance per realization request is assumed.
 *
...
public class EdgeFactory {
	...
	/**
	 * Preference key for whether to use feature licensing; if false, the simple
	 * lex feature is used for comparison purposes.
	 */
	...
	/**
	 * The rule instances, ie the type changing rules with instantiated
	 * semantics.
	 */
	...
	/**
	 * The LF alts, represented as a list of lists of alts (where each list of
	 * alts forms an exclusive disjunction).
	 */
	...
	/**
	 * Set of nominals whose phrases are marked for labeling in the output (with
	 * mark=+).
	 */
	...
	/**
	 * Flag for whether to use indexing. Setting retrieved from preferences;
	 * turned off when gluing fragments.
	 */
	...
	/**
	 * Flag for whether to debug category instantiation (defaults to false). If
	 * true, cases of complex categories whose outermost category is not
	 * instantiated with index nominals are reported to System.err. Note that
	 * realization is more efficient if such categories can be avoided in the
	 * grammar. Uncovered EPs after lex lookup are also reported to System.err.
	 */
	...
	/**
	 * Bit vector for EPs not covered by a lexical edge or rule instance; null
	 * if none.
	 */
	...
	/** Flag for whether to use relaxed relation matching. */
	// XXX tmp switch
	...
	public EdgeFactory(Grammar grammar, List<SatOp> preds, SymbolScorer signScorer,
		...
		if (useFeatureLicensing) {
			...
			// if feature licensing off, use simple lex feature for comparison
			// purposes
			featureLicenser = new FeatureLicenser(this,
	...
	/**
	 * Adds LF optional parts for any preds not covered by a lex item or rule
	 * instance, divided up by chunks, for use with fragment gluing or relaxed
	 * relation matching.
	 */
	public void addLFOptsForUncoveredPreds() {
		// see if all preds covered
		if (uncoveredEPs == null)
			return;
		...
		for (BitSet chunk : lfChunks) {
			if (opt.intersects(chunk)) {
				...
				optChunk.and(chunk);
				if (!lfOpts.contains(optChunk))
					lfOpts.add(optChunk);
				opt.andNot(optChunk);
			}
		}
		// add anything remaining
		if (!opt.isEmpty() && !lfOpts.contains(opt))
			lfOpts.add(opt);
		// ensure hasLfAltsOrOpts set
		hasLfAltsOrOpts = true;
		// TODO deal with lf alts too (may require sorting chunks and alts by
		// size)
	}

	/**
	 * Adds an LF optional part for each instantiated rule instance, for use
	 * with fragment gluing.
	 */
	...
	private BitSet uncoveredPreds() {
		...
		for (int i = retval.nextSetBit(0); i >= 0; i = retval.nextSetBit(i + 1)) {
			...
			if (HyloHelper.getInstance().isLexPred(pred)
					|| HyloHelper.getInstance().isAttrPred(pred)) {
				hasUncoveredPreds = true;
				break;
			}
		}
		// return
	...
	/**
	 * Makes an edge, computing the completeness percentage, sign score, and
	 * indices, and setting the most specific incomplete LF chunk (if any).
	 */
	...
	/**
	 * Makes an edge for the given alt sign from the given edge, after computing
	 * the sign's score.
	 */
	protected Edge makeAltEdge(Symbol altSign, Edge edge) {
		double score = signScorer.score(altSign, edge.complete());
		return new Edge(altSign, edge.bitset, edge.indices, edge.completeness, score,
	...
	public Edge makeJoinedEdge(Edge edge1, Edge edge2) {
		...
		boolean complete = (completeness == 1.0);
		double score = signScorer.score(sign, complete);
		return new Edge(sign, bitset, edge1.indices, completeness, score, edge1.activeLfAlts,
				edge1.incompleteLfChunk);
	}
	 ...
	 * updating the bitset for any completely covered alts. NB: If the given LF
	 * alts list is not the entire list, each alt is assumed to intersect.
	 */
	...
	/**
	 * Returns the active LF alts that result from combining the given ones, or
	 * null if these are incompatible. For alt sets in common, the combined alts
	 * consist of the intersection of these alt sets, or null if this
	 * intersection is empty. For alts sets not in common, the active alts are
	 * carried through unchanged.
	 */
	private List<List<Alt>> getCombinedLfAlts(List<List<Alt>> activeLfAlts1,
			List<List<Alt>> activeLfAlts2) {
		if (activeLfAlts1.isEmpty())
			return activeLfAlts2;
		if (activeLfAlts2.isEmpty())
			return activeLfAlts1;
		List<List<Alt>> retval = new ArrayList<List<Alt>>(activeLfAlts1.size()
				+ activeLfAlts2.size());
		...
		for (int i = 0; i < lfAlts.size(); i++) {
			...
			if (altSet1.get(0).altSet < i && it1.hasNext())
				altSet1 = it1.next();
			if (altSet2.get(0).altSet < i && it2.hasNext())
				altSet2 = it2.next();
			// check whether only one or the other has alt set i
			if (altSet1.get(0).altSet == i && altSet2.get(0).altSet != i)
				retval.add(altSet1);
			else if (altSet2.get(0).altSet == i && altSet1.get(0).altSet != i)
				retval.add(altSet2);
	...
	private void checkInstantiation(List<Edge> edges) {
		for (int i = 0; i < edges.size(); i++) {
			...
			if (noSemEdges.contains(edge))
				continue; // exempt uninstantiated no sem edges
	...
	private void addCatNominal(Object indexVal) {
		if (indexVal instanceof NominalAtom) {
			if (!catNominals.contains(indexVal)) {
				catNominals.add(indexVal);
			}
		}
	}
	...
	private void listPairedNominals() {
		for (int i = 0; i < preds.size(); i++) {
			SatOp pred = preds.get(i);
			if (!"tup".equals(HyloHelper.getInstance().getLexPred(pred)))
				continue;
			...
			for (int j = i + 1; j < preds.size(); j++) {
				...
				if (!tupNom.equals(HyloHelper.getInstance().getPrincipalNominal(predJ)))
					break;
			...
			if (!(nom1 instanceof NominalAtom))
				continue;
			if (!(nom2 instanceof NominalAtom))
				continue;
	...
	private void addBoundVarNominals() {
		for (int i = 0; i < preds.size(); i++) {
			SatOp pred = preds.get(i);
			String rel = HyloHelper.getInstance().getRel(pred);
			if (rel == null || !rel.equals("BoundVar"))
				continue;
			Nominal nom2 = HyloHelper.getInstance().getSecondaryNominal(pred);
			if (!(nom2 instanceof NominalAtom))
				continue;
			...
			for (int j = 0; j < preds.size(); j++) {
				SatOp predJ = preds.get(j);
				if (!nom2.equals(HyloHelper.getInstance().getPrincipalNominal(predJ)))
					continue;
				if (!"tup".equals(HyloHelper.getInstance().getLexPred(predJ)))
					continue;
				// if so, add paired items as bound vars too
				for (int k = j + 1; k < preds.size(); k++) {
					SatOp predK = preds.get(k);
					if (!nom2.equals(HyloHelper.getInstance().getPrincipalNominal(predK)))
						break;
					...
					if ("Item1".equals(relK) || "Item2".equals(relK)) {
						...
						if (!(nom2K instanceof NominalAtom))
							continue;
	...
	private boolean completesChunk(Edge edgeA, Edge edgeB) {
		if (edgeA.incompleteLfChunk != null) {
			...
			tmpBitSet.andNot(edgeA.bitset);
			tmpBitSet.andNot(edgeB.bitset);
	...
	// NB: assumes that preds are sorted by their principal nominals, with the
	// lex pred first
	private List<String> getCoartRels(int predIndex) {
		...
		for (int i = predIndex + 1; i < preds.size(); i++) {
			...
			if (!nom.equals(HyloHelper.getInstance().getPrincipalNominal(relPred)))
				break;
	...
	/**
	 * Creates and returns all initial edges. In particular, initializes all
	 * lexical edges that cover some of the input semantics; also initializes
	 * edges for semantically null lexical items, and initializes instances of
	 * type changing rules which introduce their own semantics. If a hypertagger
	 * is in place, only the beta-best edges are returned for each EP.
	 */
	public List<Edge> createInitialEdges() {
		...
		for (int i = 0; i < preds.size(); i++) {
			...
			// create initial and marked edges for each sign, updating feature
			// map
		...
		List<Edge> retval = new ArrayList<Edge>(initialEdges.size() + markedEdges.size()
		...
		if (uncoveredEPs != null && debugInstantiation) {
			System.err.println("Warning, uncovered preds after lex instantiation: "
					+ Edge.toString(uncoveredEPs));
		}

		// set opts for missing relations, if apropos
		if (useRelaxedRelationMatching)
			addLFOptsForUncoveredPreds();
	...
	private List<Edge> createInitialEdges(Symbol sign, int predIndex) {
		...
		for (Pair<Substitution, BitSet> inst : instantiations) {
			Substitution subst = inst.a;
			BitSet bitset = inst.b;
	...
	private List<RuleInstance> createRuleInstances(TypeChangingRule rule, int predIndex) {
		...
		for (Pair<Substitution, BitSet> inst : instantiations) {
			...
			TypeChangingRule newRule = new TypeChangingRule(filledArg, filledResult, rule.name(),
					rule.getFirstEP());
		...
		while (!remainingPreds.isEmpty() && remainingPreds.size() != prevSize) {
			...
			for (Iterator<SatOp> it = remainingPreds.iterator(); it.hasNext();) {
				...
				for (Pair<Substitution, BitSet> inst : prev) {
					Substitution s = inst.a;
					BitSet b = inst.b;
					if (matchingPredIndices.size() == 1) { // reuse current
															// instantiation
						...
						for (int matchingPredIndex : matchingPredIndices) {
							...
							if (checkAlts(b2)) {
								try { // unify
									...
									Pair<Substitution, BitSet> inst2 = new Pair<Substitution, BitSet>(
											s2, b2);
									retval.add(inst2);
								} catch (UnifyFailure uf) {
								}
	...
	/**
	 * Returns all edges that can be created by combining the given edges; if
	 * the collectCombos flag is true, the edges are updated with collected
	 * combos, and additional alt edges are made for the remaining alternative
	 * edges for the given first edge.
	 */
	public List<Edge> createNewEdges(Edge edge, Edge next, boolean collectCombos) {
		...
		if (useChunks) {
			if (!edge.meetsLfChunkConstraints(next) || !next.meetsLfChunkConstraints(edge))
				return Collections.emptyList();
		}
		...
		if (collectCombos && edge.altEdges.size() > 0) {
			...
			for (int i = 0; i < numNewEdges; i++) {
				...
				for (int j = 0; j < edge.altEdges.size(); j++) {
					...
					if (furtherEdge == edge)
						continue;
					Symbol[] signs = (rightward) ? new Symbol[] { next.sign, furtherEdge.sign }
							: new Symbol[] { furtherEdge.sign, next.sign };
					Symbol lexHead = (rightward == lefthead) ? next.sign.getLexHead()
							: furtherEdge.sign.getLexHead();
					Symbol altSign = Symbol.createDerivedSignWithNewLF(resultCat, signs, rule,
							lexHead);
	...
	private List<Edge> createNewEdges(Edge edgeA, Edge edgeB, boolean collectCombos,
		...
		if (gluingFragments)
			fragCompletion = completesChunk(edgeA, edgeB);

		// A B combos
		List<Symbol> results;
		if (gluingFragments)
			results = generalRules.applyGlueRule(edgeA.sign, edgeB.sign);
		else
			results = generalRules.applyBinaryRules(edgeA.sign, edgeB.sign);
		binaryRuleApps++;
		...
		if (bothDirections) {
			if (gluingFragments)
				reversedResults = generalRules.applyGlueRule(edgeB.sign, edgeA.sign);
			else
		...
		if (numResults + numReversedResults > 0) {
			...
			if (gluingFragments && union.cardinality() > cardBefore)
				fragCompletion = true;
	...
	/**
	 * Returns all edges that can be created by applying a unary rule to the
	 * given edge or by combining it with a purely syntactic edge, without
	 * collecting combos.
	 */
	...
	/**
	 * Returns all edges that can be created by applying a unary rule to the
	 * given edge, or by combining it with a purely syntactic edge, or by
	 * completing a realization/chunk/alt with an optional part, while updating
	 * the given edge with collected combos, if the collectCombos flag is true.
	...
	public List<Edge> createNewEdges(Edge edge, boolean collectCombos) {
		...
		if (!gluingFragments) {
			...
			if (genResults.size() > 0) {
				...
				for (int i = 0; i < genResults.size(); i++) {
					...
					if (sign.getDerivationHistory().containsCycle())
						continue;
			...
			for (int i = 0; i < ruleInstances.size(); i++) {
				...
				// check sem overlap
				if (edge.intersects(ruleInst))
					continue;
				// check for indices in common
				if (useIndexing && !edge.indicesIntersect(ruleInst))
					continue;
				// check LF chunk constraints
				if (useChunks && !edge.meetsLfChunkConstraints(ruleInst))
					continue;
				// get combined alts, checking compatibility
				List<List<Alt>> combinedLfAlts = getCombinedLfAlts(edge.activeLfAlts,
						ruleInst.activeLfAlts);
				if (combinedLfAlts == null)
					continue;
		...
		if (!lfOpts.isEmpty() && !edge.complete()) {
			// get completed bitsets for each completed active alt or chunk, and
			// for whole thing
	...
	/**
	 * Returns the edges that can be made by constructing alternative edges from
	 * the given edge and the collected combos in its representative edge.
	 */
	...
	private void addAltsFromCombos(Edge edge, List<EdgeCombos.CatCombo> combos, boolean rightward,
			List<Edge> results) {
		for (EdgeCombos.CatCombo combo : combos) {
			...
			for (Edge comboEdge : comboEdges) {
				Symbol[] signs = (rightward) ? new Symbol[] { edge.sign, comboEdge.sign }
						: new Symbol[] { comboEdge.sign, edge.sign };
				Symbol lexHead = (rightward == lefthead) ? edge.sign.getLexHead() : comboEdge.sign
						.getLexHead();
	...
	private void addAltsFromOptionalResults(Edge edge, List<Edge> optionalResults,
			List<Edge> results) {
	...
	public int ruleApps() {
		return unaryRuleApps * generalRules.getUnaryRules().size() + unaryRuleInstApps
	...
	// nb: could consider adding feature licensing for type changing rules with
	// no semantics
