...
/**
 * The realizer manages the realization process. Realization options may be set
 * for use across calls to the realizer.
 *
...
public class Realizer {
	...
	/**
	 * Retrieves an input LF from the given XML doc, processing any LF chunks
	 * along the way.
	 */
	...
	/**
	 * Retrieves an input LF from the given XML element, processing any LF
	...
	/**
	 * Realizes the input LF, returning the best edge found (or null if none).
	 */
	...
	/**
	 * Realizes the input LF relative to the given sign scorer, returning the
	 * best edge found (or null if none).
	 */
	public Edge realize(LF lf, SymbolScorer signScorer) {
		Preferences prefs = Preferences.userNodeForPackage(TextCCG.class);
		int timeLimitToUse = (timeLimitMS != -1) ? timeLimitMS : prefs.getInt(Chart.TIME_LIMIT,
				Chart.NO_TIME_LIMIT);
		return realize(lf, signScorer, timeLimitToUse, waitForCompleteEdge);
	}

	/**
	 * Realizes the input LF relative to given sign scorer, returning the best
	 * edge found (or null if none) in the given time limit (in ms), potentially
	 * waiting longer for a complete edge according to the given flag. If a
	 * hypertagger is employed, realization proceeds iteratively through the
	 * available beta-best values within the overall time or edge limit.
	 */
	public Edge realize(LF lf, SymbolScorer signScorer, int timeLimitMS, boolean waitForCompleteEdge) {
		...
		SymbolScorer scorerToUse = (signScorer != null) ? signScorer : SymbolScorer.nullScorer;
		PruningStrategy strategyToUse = (pruningStrategy != null) ? pruningStrategy
				: new NBestPruningStrategy();
	...
	private Edge realizeWithHypertagger(List<SatOp> preds, SymbolScorer signScorer,
			PruningStrategy pruningStrategy, int timeLimitMS) {
		// get start time
		long startTime = System.currentTimeMillis();
		// get edge limit
		Preferences prefs = Preferences.userNodeForPackage(TextCCG.class);
		...
		while (retval == null && !outOfBetas && !pastTimeLimit && !exceededEdgeLimit) {
			...
			chart = new Chart(new EdgeFactory(grammar, preds, signScorer, hypertagger),
					pruningStrategy);
			...
			chart.usePacking = true;
			chart.collectCombos = false;
			chart.doUnpacking = false;
			chart.joinFragments = false;
			// run request
			chart.initialize();
			...
			if (timeLimitMS != Chart.NO_TIME_LIMIT && iterTime >= timeLimitMS) {
				pastTimeLimit = true;
				// System.out.println("Went past time limit with ht beta: " +
				// hypertagger.getCurrentBetaValue());
			}
			// check edge limit
			if (edgeLimit != Chart.NO_EDGE_LIMIT && chart.numEdges >= edgeLimit) {
				exceededEdgeLimit = true;
				// System.out.println("Exceeded edge limit with ht beta: " +
				// hypertagger.getCurrentBetaValue());
			}
			// if complete, unpack and return best edge
			if (chart.bestEdge.complete()) {
				chart.doUnpacking = true;
				chart.doUnpacking();
			...
			// otherwise check beta level if still within limits
				...
				if (hypertagger.hasMoreBetas()) {
					...
					outOfBetas = true;
					// System.out.println("Ran out of betas with ht beta: " +
					// hypertagger.getCurrentBetaValue());
				}
			}
		}
		...
		if (retval == null) {
			...
			if (useGluing) {
				// System.out.println("Num edges for final iteration: " +
				// chart.numEdges);
				// System.out.println("Trying gluing option after iterTime: " +
				// iterTime);
				chart.reInitForGluing();
				...
				// System.out.println("Num edges after gluing: " +
				// chart.numEdges);
				currentTime = System.currentTimeMillis();
				iterTime = (int) (currentTime - iterStartTime);
				// if complete, unpack and return best edge
				if (chart.bestEdge.complete()) {
					// System.out.println("Unpacking in final iteration after iterTime: "
					// + iterTime);
					chart.doUnpacking = true;
					chart.doUnpacking();
					retval = chart.bestEdge;
			...
			// otherwise try a final iteration in an iteration in anytime mode,
			// possibly resorting to joining fragments
			if (retval == null) {
				// System.out.println("Trying a final iteration in anytime mode after iterTime: "
				// + iterTime);
				// instantiate chart and set start time for this iteration
				chart = new Chart(new EdgeFactory(grammar, preds, signScorer, hypertagger),
						pruningStrategy);
				iterStartTime = System.currentTimeMillis();
				// run request
				chart.usePacking = false;
				chart.joinFragments = true;
				chart.initialize();
				chart.combine(timeLimitMS, waitForCompleteEdge);
				// System.out.println("Num edges after anytime iteration: " +
				// chart.numEdges);
				currentTime = System.currentTimeMillis();
				iterTime = (int) (currentTime - iterStartTime);
				// if (chart.bestEdge.complete())
				// System.out.println("Found complete edge after iterTime: " +
				// iterTime);
				// else
				// System.out.println("Resorting to joined fragments after iterTime: "
				// + iterTime);
				// return best edge
				retval = chart.bestEdge;
			}
