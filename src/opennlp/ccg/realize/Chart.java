 ...
 * (see Chart.combine). In the packing mode, completing the chart produces a
 * packed representation, which may then be unpacked if the DO_UNPACKING
 * preference is set.
 *
...
public class Chart {
	 ...
	 * If between o-1, the time limit is interpreted as a percentage of the time
	 * until the first realization is found.
	 */
	...
	/**
	 * Preference key for whether to create a packed generation forest in the
	 * first of two realization stages (deferring unpacking to the later stage).
	 * This option pre-empts the use of collected combos in a single, anytime
	 * realization stage.
	 */
	 ...
	 * two realization stages. This option is only relevant if packing is used
	 * in the first stage.
	 */
	...
	/**
	 * New best time limit, as a percentage of time from the first complete
	 * realization. Set from prefs in constructor.
	 */
	...
	/**
	 * Flag for whether to collect category combos. Set from prefs in
	 * constructor.
	 */
	...
	/**
	 * Flag for whether to join best fragments if no complete realization found.
	 * Defaults to false.
	 */
	...
	/**
	 * The best edge found so far (or null), where a complete edge is always
	 * given preference to an incomplete one.
	 */
	...
	/**
	 * Returns the number of unpruned (and unpacked, if apropos) edges in the
	 * chart.
	 */
	...
	/**
	 * Returns whether there were no uncovered lexical or featural preds after
	 * lex lookup.
	 */
	public boolean noUncoveredPreds() {
		return !edgeFactory.hasUncoveredPreds;
	}

	/**
	 * Reinitializes the agenda for gluing fragments. A runtime exception is
	 * thrown if not in packing mode.
	 */
	public void reInitForGluing() {
		...
		// add opt for uncovered preds, unless already done for relaxed relation
		// matching
	...
	/**
	 * Adds to the chart by iteratively moving an edge from the agenda to the
	 * chart, creating new edges for the agenda by applying available rules,
	 * while updating the best edge found so far, until a stopping criterion is
	 * reached. The basic stopping criterion is when the agenda becomes empty,
	 * and thus the search is done. Otherwise, the search is stopped either when
	 * the edge limit (if any) is reached, or the time limit (if any) is
	 * reached, or the first complete edge is found (if beyond the edge/time
	 * limit, and according to the given flag), or until the new best time limit
	 * (if any; anytime case only) beyond the first complete realization is
	 * exceeded. In the packing case, unpacking is then performed according to
	 * the preference setting. In the anytime case, if the collect combos option
	 * is set, then the combinatory rules are only invoked when an edge with a
	 * new category is moved to the chart, in which case any successful
	 * combinations are collected in the edge's combos data structure; if the
	 * edge instead has an already seen category, new edges are created as
	 * alternatives to the collected combos in its representative, much as with
	 * unpacking.
	 */
	public void combine(int timeLimitMS, boolean waitForCompleteEdge) {
		...
		while (!agenda.isEmpty()) {
			...
			(edgeLimit != NO_EDGE_LIMIT && numEdges > edgeLimit && (!waitForCompleteEdge || bestEdgeComplete))
					...
					(!usePacking && bestEdgeComplete && ((newBestTimeLimit != NO_TIME_LIMIT && timeSinceFirst > newBestTimeLimit) || (newBestTimeLimitPct != NO_TIME_LIMIT && (double) timeSinceFirst
							/ timeTilFirst > newBestTimeLimitPct)))) {
		...
		if (joinFragments && !bestEdge.complete())
			joinBestFragments();
	}
	...
	private void doEdgeCombos(Edge next) {
		...
		if (gluingFragments && next.bitset.isEmpty())
			return;
		...
		for (Edge edge : edgesToUse) {
			...
			// skip fragment gluing if semantically null or if result cell
			// non-empty
			if (gluingFragments) {
				if (edge.bitset.isEmpty())
					continue;
				tmpBitSet.clear();
				tmpBitSet.or(edge.bitset);
				tmpBitSet.or(next.bitset);
				if (nonEmptyCells.contains(tmpBitSet))
					continue;
			}
	...
	protected void joinBestFragments() {
		...
		while (true) {
			...
			for (Edge edge : allEdges)
				bestFrag = chooseBestFrag(bitset, bestFrag, edge);
	...
	protected void doUnpacking() {
		...
		for (Edge edge : edges) {
			if (foundComplete && !edge.complete())
				continue;
	...
	private void unpackAlt(Edge alt, Set<Edge> unpacked, EdgeHash merged) {
		...
		if (alt.optCompletes != null) {
			...
			for (Edge inputAlt : inputEdge.altEdges) {
				Edge edgeToAdd = (inputAlt.sign == alt.sign) ? alt // use this
																	// alt for
																	// same sign
						: edgeFactory.makeAltEdge(inputAlt.sign, alt); // otherwise
																		// make
																		// edge
																		// for
																		// new
		...
		for (int i = 0; i < inputSigns.length; i++) {
			inputEdges[i] = signMap.get(inputSigns[i]); // get input edge using
														// signMap
		...
		for (Symbol[] combo : altCombos) {
			...
			Symbol sign = Symbol.createDerivedSignWithNewLF(resultCat, combo, history.getRule(),
					lexHead);
			Edge edgeToAdd = (sign.equals(alt.sign)) ? alt // use this alt for
															// equiv sign
					: edgeFactory.makeAltEdge(sign, alt); // otherwise make edge
															// for new alt
			merged.insert(edgeToAdd);
		}
	}
	...
	/**
	 * Returns the best complete edges, sorted by their score and pruned by the
	 * pruning strategy.
	 */
	...
	/**
	 * Returns the oracle best edge for the given string, together with a flag
	 * indicating whether the target string was found. At present, returns the
	 * edge from bestEdges() whose orthography equals the given string,
	 * otherwise the one with the highest 4-gram precision score (breaking ties
	 * using the list order). TODO: It would be better to return the forest
	 * oracle.
	 */
	...
	/**
	 * The PrintWriter to use with the printing routines. Default wraps
	 * System.out.
	 */
	...
	public void printTiming() {
		...
		if (!usePacking) {
			out.println("# pruned:    " + numPrunedRemoved + " removed, " + numPrunedNeverAdded
					+ " never added");
		}
		if (doUnpacking) {
	...
	/**
	 * Prints chart edges unsorted, using the complete edges filter according to
	 * the given flag.
	 */
	public void printEdges(boolean complete) {
		printEdges(complete, false);
	}

	/**
	 * Prints chart edges using the complete edges filter according to the given
	 * flag and sorting according to the given flag. In the packing only case,
	 * the representative edges are shown, otherwise the unpruned (and possibly
	 * unpacked) edges are shown.
	 */
	...
	/**
	 * Prints the licensed, instantiated purely syntactic (semantically null)
	 * edges.
	 */
	...
	/**
	 * Prints the licensed, uninstantiated purely syntactic (semantically null)
	 * edges.
	 */
	...
	// if the edge replaces an (essentially) equivalent edge of lower score or
	// higher
	...
	private void addEdgeToAgenda(Edge edge) {
		numEdges++;
		if (!usePacking) {
			// update edgeHash, checking for equivalent edge of equal or lower
			// complexity
			...
			if (oldEdge != null) {
				...
				if (!onAgenda) {
					...
					if (repEdge != null) {
						...
						if (inChart)
							supercededEdgesPendingRemoval.add(oldEdge);
					}
				}
			}
		}
	...
	// first one added for its equiv class; otherwise it's added as an
	// alternative;
	// in the anytime case, prunes the edges listed as alts for the
	// representative edge,
	// and adds the edge to the list of all unpruned edges, if it survives
	// pruning;
	// returns true if the edge is actually added, and false if it doesn't
	// survive
	// the pruning (anytime case), or is folded into an existing edge (packing
	// case);
	...
	private boolean addEdgeToChart(Edge edge) {
		...
		// anytime case: if not pruning, just add edge to all edges list, and
		// return
	...
	private void addSorted(List<Edge> list, Edge edge) {
		...
		if (index >= 0) {
			...
			while (index < list.size()) {
				...
				if (comparator.compare(existingEdge, edge) == 0)
					index++;
				else
					break;
			}
	...
	/**
	 * Compares edges based on their relative size then score, in descending
	 * order.
	 */
