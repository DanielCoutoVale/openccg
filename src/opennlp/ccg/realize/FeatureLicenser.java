...
/**
 * The feature licenser is a helper class for the edge factory, responsible for
 * managing features which license the use and instantiation of semantically
 * null or marked categories.
 *
...
public class FeatureLicenser {
	...
	/**
	 * Adds new nominal atoms for subcategorized semantically null words. A
	 * check is made for atomic categories with a value for the 'lex' feature
	 * but with a null or uninstantiated index feature. If found, a new nominal
	 * atom is created as the value of the index feature, and the nominal is
	 * added to the edge factory's nominals map, for indexing purposes. The same
	 * nominal is reused for repeated occurrences of a 'lex' value.
	 */
	...
	/**
	 * Updates the licensed feature map with the info from the given initial
	 * category.
	 */
	...
	private CategoryFcn featureMapUpdater = new CategoryFcnAdapter() {
		public void forall(Category c) {
			if (!(c instanceof AtomCat))
				return;
			if (currentFeatureMap == featureMap)
				allInitialAtomCats.add(c);
			FeatureStructure fs = c.getFeatureStructure();
			if (fs == null)
				return;
			// for each feature
			for (int i = 0; i < licensingFeatures.length; i++) {
				...
				if (val != null && !(val instanceof Variable)) {
					...
					if (fVal != null && !fVal.equals(valStr) && !alsoList.contains(valStr))
						continue;
					// add to feature map
	...
	/**
	 * Returns whether the given category contains a feature indicating that it
	...
	/**
	 * Returns whether the given category is licensed according to the current
	 * feature map.
	 */
	...
	// records the licensing feature which succeeded in licensing the last cat
	// (or null if none)
	...
	private boolean checkLicensing(Category cat, boolean needsLicensing) {
		...
		for (int i = 0; i < licensingFeatures.length; i++) {
			...
			if (emptyCat && !licensingFeatures[i].licenseEmptyCats)
				continue;
			if (!emptyCat && !licensingFeatures[i].licenseMarkedCats)
				continue;
			String attr = licensingFeatures[i].attr;
	...
	/**
	 * Determines whether the given semantically null category is licensed
	 * according to the licensed feature map, and if so, returns appropriately
	 * (un-)instantiated versions of the category. The licensing features are
	 * checked in priority order. NB: Instantiation is limited to the case where
	 * there is a single value for the operative licensing feature.
	 */
	public void licenseEmptyCat(Category cat, Set<Category> instantiatedCats,
			Set<Category> uninstantiatedCats) {
		// reindex
		...
		if (currentLicensingFeature == null) {
			for (int i = 0; i < licensingFeatures.length; i++) {
				if (!catFeatureMap.containsKey(licensingFeatures[i].attr))
					continue;
				Map<String, Set<Category>> valMap = catFeatureMap.get(licensingFeatures[i].attr);
				String fVal = licensingFeatures[i].val;
				if (fVal != null && !valMap.containsKey(fVal))
					continue;
				currentLicensingFeature = licensingFeatures[i];
				break;
			}
		...
		for (Iterator<Category> acIt = atomCats.iterator(); acIt.hasNext();) {
			...
			if (!currentLicensingFeature.licenseEmptyCats)
				...
				if (alsoList.size() > 0) {
					if (initialCats != null)
						...
						initialCats = new HashSet<Category>();
			...
			if (initialCats == null) {
				System.out.println("Warning, unable to find initial cats for feature " + attr + "="
						+ val);
				uninstantiatedCats.add(cat);
				return;
			}
			...
			for (Iterator<Category> it = initialCats.iterator(); it.hasNext();) {
				...
				if (!(index instanceof NominalAtom))
					continue;
