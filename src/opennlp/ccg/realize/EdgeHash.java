...
/**
 * A set of edges, unique up to surface words. Edges with higher scores or whose
 * signs have lower derivational complexity are kept during insertion.
 *
...
public class EdgeHash extends THashSet {
	...
	public Set<Edge> asEdgeSet() {
		return (Set<Edge>) this;
	}

	/**
	 * Adds an edge, keeping the one with a higher score or whose sign has lower
	 * derivational complexity if there is an equivalent one there already;
	 * returns the old edge if it was displaced, the new edge if there was no
	 * equivalent old edge, or null if the edge was not actually added.
	 */
