 ...
 */
// NB: It might make sense to eventually make this a subclass of
// TypeChangingRule,
...
public abstract class AbstractTypeRaisingRule extends AbstractRule {
	...
	/**
	 * The argument of the ComplexCat argument of the type raised category, eg
	 * the "np" in s/(s\np). Defaults to "np".
	 */
	protected Category _arg;

	/**
	 * Creates a new type raising rule with the given name; upper and lower
	 * slashes; use dollar switch; arg category; and result category. Defaults
	 * are used for the arg and result categories if null.
	 */
	protected AbstractTypeRaisingRule(String name, Slash uslash, Slash eslash, boolean useDollar,
			Category arg, Category result) {
		_name = name;
		_upperSlash = uslash;
		...
		if (arg != null) {
			...
			_arg = new AtomCat("np", new GFeatStruc());
		}
	...
	public Element toXml(String dir) {
		...
		boolean usesDollar = (_result instanceof ComplexCat)
				&& ((ComplexCat) _result).containsDollarArg();
	...
	protected List<Category> apply(Category input) throws UnifyFailure {
		...
		if (result instanceof ComplexCat) {
			...
			range = new ComplexCat((TargetCat) result.copy(), new BasicArg(_embeddedSlash, arg));
			result = new ComplexCat((TargetCat) result.copy(), new BasicArg(_upperSlash, range));
		}
