...
public abstract class NgramScorer implements SymbolScorer, Reversible, FeatureExtractor {
	...
	public static class NgramFloats {
		...
		public String toString() {
			return "logprob: " + logprob + ", bow: " + bow;
		}
	}
	...
	/**
	 * Flag for open vocabulary, ie whether the unknown word &lt;unk&gt; is in
	 * the model.
	 */
	...
	public void addFilter(NgramFilter filter) {
		if (ngramFilters == null) {
			ngramFilters = new ArrayList<NgramFilter>();
		}
	...
	/**
	 * Sets the alphabet, so that features can be filtered to the relevant ones
	 * (when present).
	 */
	...
	protected void putCachedLogProb(List<Association> words, Float logprob) {
		if (cachedLogProbs == null)
			cachedLogProbs = new WeakHashMap<List<Association>, Float>();
		cachedLogProbs.put(words, logprob);
	}

	/**
	 * Returns a score between 0 (worst) and 1 (best) for the given sign and
	 * completeness flag, based on the n-gram score of the sign's words. If the
	 * sign is complete, sentence delimiters are added before scoring the words,
	 * if not already present. Returns 0 if any filter flags the n-gram for
	 * filtering, or if the sign has no words. Otherwise, sets
	 * <code>signToScore</code>, calls <code>prepareToScoreWords</code>, and
	 * then returns the result of <code>logprob()</code> converted to a
	 * probability.
	 */
	...
	/**
	 * Returns a log prob for the given sign and completeness flag, based on the
	 * n-gram log prob of the sign's words. If the sign is complete, sentence
	 * delimiters are added before scoring the words, if not already present.
	 * Returns the log prob for zero probability if any filter flags the n-gram
	 * for filtering, or if the sign has no words. Otherwise, sets
	 * <code>signToScore</code>, calls <code>prepareToScoreWords</code>, and
	...
	public synchronized double logprob(Symbol sign, boolean complete) {
		...
		if (ngramFilters != null) {
			for (int i = 0; i < ngramFilters.size(); i++) {
				NgramFilter filter = ngramFilters.get(i);
				if (filter.filterOut(wordsToScore))
					return convertToLogProb(0);
			}
		}
		prepareToScoreWords();
	...
	/**
	 * Returns an n-gram probability from the given list of words, by converting
	...
	/**
	 * Returns an n-gram log prob for the given list of words. This method is a
	 * simplified version of scoring a sign's words that does not cache results,
	 * filter n-grams or ever add sentence delimiters.
	 */
	...
	protected void setWordsToScore(List<Association> words, boolean complete) {
		...
		if (complete && (reverse || words.get(0).getForm() != "<s>")) {
			wordsToScore.add(AssociationPool.createMuster("<s>"));
			tagsAdded = true;
		}
		if (reverse) {
	...
	/**
	 * Returns a feature map with counts of each ngram for the given sign and
	 * completness flag.
	 */
	...
	/**
	 * Returns a feature vector with the log prob features for the given sign
	 * and completeness flag. The default implementation returns the log prob as
	...
	public FeatureVector extractLogProbs(Symbol sign, boolean complete) {
		...
		if (f != null)
			retval.add(f, (float) logprob(sign, complete));
		return retval;
	}

	/**
	 * Returns the features for the given sign and completeness flag. The
	 * default implementation returns the log prob as the value of a feature
	 * named '$ngram' plus the counts of each ngram.
	 */
	 ...
	 * wordsToScore and with the given order. The default implementation uses
	 * ngram(i, order).
	 */
	 ...
	 * this operation does not succeed normally.
	 */
	...
	/**
	 * Returns a log prob for the words in wordsToScore. The default method
	 * returns the log prob of the word sequence as determined by this language
	 * model's <code>logProbFromNgram</code> method. The probabilities for the
	 * first n-1 words are backed off to the lower order probabilities. If the
	 * tagsAdded flag is false, the cache is checked to see whether the log prob
	 * of the words of signToScore's initial sign has already been calculated,
	 * and at the end the log prob of signToScore's words is stored in the
	 * cache.
	 */
	protected double logprob() {
		float logProbTotal = 0;
		int numCached = 0;
		if (!tagsAdded && signToScore != null) { // check cache for initial
													// words
	...
	/**
	 * Sets the keys in keysList to hold the ngram starting at the given index
	 * in wordsToScore and with the given order; returns true if the operation
	 * succeeds normally. The default implementation invokes logProbFromNgram,
	 * and returns false if the log prob is zero.
	 */
	...
	private Tokenizer getTokenizer() {
		if (tokenizer != null)
			return tokenizer;
		if (Grammar.theGrammar != null)
			tokenizer = Grammar.theGrammar.lexicon.tokenizer;
		else
	 ...
	 * resets the lists.
	 */
	protected void addTrieMapChildren(List<Object> prefix, List<Object> keys,
			List<TrieMap<Object, NgramFloats>> children) {
		if (!keys.isEmpty()) {
	...
	protected float logProb(int pos, int len) {
		...
		if (retval > -99)
			retval += backoffWeight(pos, len - 1);
		return retval;
	}
	...
	/**
	 * Returns the rank order centroid weights for a ranked list of the given
	 * length. The weights go from highest to lowest, and sum to 1.
	 */
	// ex:
	...
	/**
	 * Converts a base 10 log prob to an actual probability, checking for -99
	 * (not found).
	 */
	...
	public static double convertToLogProb(double prob) {
		if (prob == 0)
			...
			return Math.log(prob) / Math.log(10);
	}
