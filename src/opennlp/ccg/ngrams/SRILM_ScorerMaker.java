 ...
 * SRILM toolkit, which must be separately installed. This class may be
 * subclassed to set different parameters in the constructor, for use in
 * cross-validation tests with the realizer.
 *
...
public class SRILM_ScorerMaker implements ScorerMaker {
	 ...
	 * ngram-count tool with the current options, and computes perplexity on the
	 * test data. The training/test data are written to foldN-train.txt and
	 * foldN-test.txt, if not already present. The LM file's name is determined
	 * by filename(N).
	 */
	...
	protected void writeTrainingAndTestFiles(File tmpDir, int foldNum, File trainFile, File testFile)
			throws IOException {
	...
	protected String countNgrams(int foldNum) {
		...
		String cmd = "ngram-count -order " + getOrder() + " " + ((unk) ? "-unk " : "")
				+ minCountOptions + " " + discountOptions + " " + "-text " + trainingfile(foldNum)
				+ " " + "-lm " + lmfile + " " + "-debug " + debugLevel;
		return cmd;
	}
	...
	protected String scoreNgrams(int foldNum) {
		String lmfile = filename(foldNum);
		String cmd2 = "ngram -order " + getOrder() + " " + ((unk) ? "-unk " : "") + "-ppl "
				+ testfile(foldNum) + " " + "-lm " + lmfile;
		return cmd2;
	}
