...
/**
 * Linear combination of n-gram probability models, interpolated at the word
 * level; can also be used as a feature extractor in a perceptron model. The
 * models must have the same direction.
 *
...
public class LinearNgramScorerCombo extends NgramScorer implements FeatureExtractor {
	...
	/**
	 * Creates a new linear combo model with the given component models and with
	 * the combination weights determined by the rank order centroid method. The
	 * models are assumed to be ordered from most to least important.
	 */
	...
	/**
	 * Creates a new linear combo model with the given component models and
	 * combination weights. The weights are assumed to sum to 1, and the number
	 * of weights is assumed to match the number of models. The wordsToScore
	 * list is shared across the component models.
	 */
	 ...
	 * wordsToScore and with the given order, with backoff. In particular,
	 * returns the linear combination using the established weights of the
	 * probabilities given by the component models, converted back to a log prob
	 * (base 10).
	 */
	 ...
	 * wordsToScore and with the given order. The implementation delegates to
	 * the component models.
	 */
	protected void incNgrams(FeatureMap featmap, int i, int order) {
		for (int j = 0; j < models.length; j++) {
			if (models[j].useNgramFeatures)
				models[j].incNgrams(featmap, i, order);
		}
	}
	 ...
	 * and completeness flag. In particular, returns the log prob from each
	 * model as the value of a feature named '$ngramN'. The interpolated log
	 * prob is also returned as a feature '$ngram' if the flag for including the
	 * interpolated log prob as a feature is set (the default).
	 */
	public FeatureVector extractLogProbs(Symbol sign, boolean complete) {
		...
		for (int i = 0; i < models.length; i++) {
			...
			if (f != null)
				retval.add(f, (float) models[i].logprob(sign, complete));
		}
		return retval;
	}
