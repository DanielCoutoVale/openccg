...
/**
 * A scorer consisting of a family of factored n-gram backoff models. The family
 * of models is specified using the factored language model specification file
 * format given as input to the SRILM version 1.4.1 fngram-count tool. Each
 * individual model is loaded as a FactoredNgramModel instance, and thus only
 * static backoff orders are supported at present, with the most distant parent
 * variable dropped at each backoff point. The first model in the specification
 * file should be the primary one. It may be followed by any number of models
 * for the same child variable but with lower history orders; these models will
 * be used when the full history is unavailable, if their order matches the
 * available number of words in the history. For example, if the primary model
 * is a trigram model, then a bigram model may also be given (with a potentially
 * different backoff order) for scoring bigram word sequences. Following these
 * models, there may also be further (sequences of) models for scoring different
 * child variables. If present, the scores calculated for these child variables
 * will be multipled with the score calculated for the primary model's child
 * variable (typically the word form). For example, one or more models may be
 * given to calculate the probability of the word's pitch accent, independently
 * of the word form (though potentially looking at some of the same history).
 * With each individual model, the parents are assumed to be listed in backoff
 * order. Unknown words are mapped to &lt;unk&gt; if the latter is present in
 * the first model.
 *
...
public class FactoredNgramModelFamily extends NgramScorer {
	...
	/**
	 * A factored n-gram model and any secondary ones for the same child
	 * variable.
	 */
	public class ModelGroup {
		...
		public FactoredNgramModel getModel(int order) {
			...
			for (int i = 0; i < secondaryModels.length; i++) {
				if (secondaryModels[i].order == order)
					return secondaryModels[i];
			}
			return primaryModel;
		}
	...
	/**
	 * Loads a family of factored n-gram models from the file with the given
	 * name, in the SRILM format. The flag for using sem classes is defaulted to
	 * true.
	 */
	...
	private List<ModelGroup> readModel(String filename) throws IOException {
		...
		while (tokenizer.ttype != StreamTokenizer.TT_EOF) {
			...
			// line format is <child> : <numParents> <parent1> ... <parentN>
			// <countfile> <lmfile> <numbognodes>
		...
		if (actualNumModels != numModels) {
			System.err.println("Warning: Only found " + actualNumModels + "/" + numModels + " in "
					+ filename);
		...
		while (modelIndex < numModels) {
			...
			FactoredNgramModel[] secondaryModels = new FactoredNgramModel[secondaryModelsList
					.size()];
