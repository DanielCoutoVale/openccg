...
/**
 * A language model that uses the <acronym
 * title="Stanford Research Institute">SRI</acronym> language modeling toolkit.
 * 
...
public class SRILMNgramModel extends AbstractStandardNgramModel {
	 ...
	 *             These include non-existent or unreadable files, file format
	 *             problems, etc.
	 */
	public SRILMNgramModel(int order, File lmFile, boolean useSemClasses,
			SRILMNgramModelType modelType) throws IOException {
	 ...
	 *      SRILMNgramModelType)
	 */
	public SRILMNgramModel(int order, File lmFile, SRILMNgramModelType modelType)
			throws IOException {
	 ...
	 *             <code>null</code>, or if a problem occurs loading or parsing
	 *             the file.
	 */
	protected void loadLMFromFile(int ngramOrder, File lmFile, SRILMNgramModelType lmType)
			throws IOException {
		if (lmFile == null) {
	...
	/**
	 * Calculates a log probability of a delineated substring of the strings to
	 * score using SRILM. This method reverses the context before passing the
	 * string to SRILM, as this is the format SRILM expects.
	 * 
	 * @param pos The start position (inclusive) within the strings to score.
	 * @param len The length, starting from <null>pos</null>, of the string that
	 *            should be used.
	 */
	@Override
	public float logProb(int pos, int len) {
		try {
			...
			String[] context = (rangeSize > 1) ? new String[rangeSize - 1] : null;

			if (context != null) { // reverse for SRILM
				ListIterator<Object> contextIterator = range.listIterator(rangeSize - 1);
				int i = 0;
	 ...
	 * @throws IOException If a problem happens with SRILM while trying to load
	 *             the language model.
	 */
	private native void loadLM(int ngramOrder, String fileAbsolutePath, int lmType)
			throws IOException;

	/**
	 * Invokes SRILM to calculate the log probability of a string in the given
	 * context. SRILM will make its calculations based on the language model
	 * loaded in {@link #loadLM(int, String, int)}.
	 * 
	 * @param word The word to calculate a probability for.
	 * @param context The context, in reverse order. For example, to calculate
	 *            the probability of the word &quot;rain&quot; in the context of
	 *            the string &quot;in the rain&quot;, the context should be the
	 *            array <code>{the, in}</code>. If the context is
	 *            <code>null</code> or zero-length, SRILM will assume this means
	 *            no context should be used.
	...
	public static void main(String[] args) throws Exception {
		...
		System.out.println("Loading " + reversed + "n-gram model with order " + order + " from: "
				+ lmfile);
		SRILMNgramModel lm = new SRILMNgramModel(Integer.parseInt(order), new File(lmfile),
				SRILMNgramModelType.valueOf(lmType));
