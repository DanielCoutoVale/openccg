...
public class KenNgramModel extends AbstractStandardNgramModel {
	...
	// Map of hash of word to integer representation (integerized word used by
	// KenLM).
	...
	// Map from hashes to tokens (to see whether we have encountered a new
	// token).
	private Map<Long, String> hash2String = new HashMap<Long, String>();
	// List of vocabulary tokens. New items are added and assigned their index
	// as a representation.
	private List<String> vocabList = new ArrayList<String>();
	// Whether to lowercase text before querying to the language model (e.g.,
	// "Pierre Vinken" => "pierre vinken").
	private boolean lowercaseText = false;
	// Whether to split up named entities before querying the language model
	// (e.g., "Pierre_Vinken" => "Pierre Vinken")
	...
	public KenNgramModel(int order, String lmFile, boolean useSemClasses, boolean lowercaseText,
			boolean splitNEs, char neDelim, boolean useNgramFeatures) throws IOException {
	...
	/**
	 * Construct with order and filename. (Delegates to superclass for these
	 * flags).
	 */
	...
	/**
	 * Construct with order and filename and an indication of whether to use
	 * semantic classes. (Delegates to superclass for these flags).
	 */
	...
	/**
	 * Lowercase each token, if desired, and split each token into a list of
	 * tokens (splitting on NE delim token), if desired.
	 */
	protected List<Association> splitAndLowercase(List<Association> words) {
		...
		if (!(lowercaseText || splitNEs)) {
			...
			for (Association w : words) {
				...
				// ArrayList<String> subTmp = new
				// ArrayList<String>(parts.length);
				for (String part : parts) {
					String newWdForm = (lowercaseText) ? part.toLowerCase() : part;
					// add null attr/val list, since it is not accessible.
					tmp.add(AssociationPool.createAssociation(newWdForm, w.getTone(), newWdForm, w.getFunctions(),
	...
	public float logProb(int pos, int len) {
		try {
			...
			for (Object wts : keysList.subList(pos, pos + len))
	...
	public static void main(String[] args) throws IOException {
		...
		// we want to prove that there are NEs to split and that there are
		// uppercase chars to preserve.
		...
		KenNgramModel lm = new KenNgramModel(Integer.parseInt(order), lmfile, false, lowercase,
				splitNEs, '_', false);
		lm.debugScore = true;
