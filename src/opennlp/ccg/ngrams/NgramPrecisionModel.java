...
/**
 * N-gram precision scoring model, using a linear combination of n-grams with
 * rank order centroid weights, and optionally replacing word forms with their
 * semantic classes. Words in the target strings are assumed to contain any
 * desired delimiters. With the exact matches flag set, only exact matches
 * count.
 *
...
public class NgramPrecisionModel extends NgramScorer implements SelfParaphraseBiaser {
	...
	/**
	 * Creates a new 4-gram precision model with no initial target strings and
	 * with the combination weights determined by the rank order centroid
	 * method. Word forms are not replaced by their semantic classes.
	 */
	...
	/**
	 * Creates a new 4-gram precision model from the given target strings and
	 * with the combination weights determined by the rank order centroid
	 * method. Word forms are not replaced by their semantic classes.
	 */
	...
	/**
	 * Creates a new n-gram precision model of the given order from the given
	 * target strings and with the combination weights determined by the rank
	 * order centroid method. Word forms are not replaced by their semantic
	 * classes.
	 */
	...
	/**
	 * Creates a new 4-gram precision model from the given target strings, with
	 * the given flag controlling whether word forms are replaced by their
	 * semantic classes, and with the combination weights determined by the rank
	...
	/**
	 * Creates a new n-gram precision model of the given order from the given
	 * target strings, with the given flag controlling whether word forms are
	 * replaced by their semantic classes, and with the combination weights
	 * determined by the rank order centroid method.
	 */
	 ...
	 * Creates a new n-gram precision model of the given order from the given
	 * target strings, with the given flag controlling whether word forms are
	 * replaced by their semantic classes, and with the given combination
	 * weights, beginning with the highest-order weight and ending with the
	 ...
	 * then calculates and returns the n-gram precision score. In particular,
	 * returns the linear combination using the established weights of the
	 * various n-gram precision scores (from unigram up to the configured
	 * order), where the n-gram precision is the number of n-grams with a match
	 * in the target strings divided by the number of n-grams in the word
	 * sequence. With the exact matches flag set, only exact matches count. With
	 * short sequences (less than the order), the score is adjusted
	...
	/**
	 * Returns the features as counts of each ngram for the given sign and
	 * completeness flag. This method returns the feature map as a feature
	 * vector.
	 */
	...
	/**
	 * Sets wordList to be the n-gram of the given order using words starting at
	 * pos i.
	 */
	...
	/**
	 * Makes a canonical n-gram of the given order using words starting at pos
	 * i. Sublists are shared, a la a trie data structure.
	 */
	@SuppressWarnings("unchecked")
	protected List<Association> makeNgram(List<Association> words, int i, int order) {
		...
		List<Association> alreadyInterned = (List<Association>) Interner
				.getGlobalInterned(wordList);
		if (alreadyInterned != null)
			return alreadyInterned;
		// if order is 1, intern new singleton list
		if (order == 1) {
			return (List<Association>) Interner.globalIntern(new SingletonList<Association>(words
					.get(i)));
		}
		...
		return (List<Association>) Interner.globalIntern(new StructureSharingList<Association>(
				firstOneList, suffixList));
	}

	// initializes the n-grams from the target phrases
	private void initTargetNgrams(String[] targets) {
		for (int j = 0; j < targets.length; j++) {
			if (targets[j].length() == 0)
				continue;
			...
			// reduce each word to a surface word, using the sem class if
			// apropos
			int numWords = wordsToScore.size();
			for (int i = 0; i < numWords; i++) {
				Association w = wordsToScore.get(i);
