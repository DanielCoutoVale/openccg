 ...
 * SRILM toolkit, which must be separately installed. Most parameters are set in
 * the FLM spec file.
 *
...
public class SRILM_FactoredScorerMaker extends SRILM_ScorerMaker {
	...
	/**
	 * Returns the model FLM spec filename, extending flmSpecFileBase with the
	 * discount options string and n-gram order, eg "test.flm.n4".
	 */
	...
	protected String pplFileRoot() {
		return flmSpecFilename();
	}

	/**
	 * Creates fold-specific FLM spec filenames from fold numbers, eg
	 * "test.flm.n4.fold1".
	 */
	...
	/**
	 * Writes a fold-specific FLM spec file, given the tmp dir and fold num. The
	 * fold-specific file is created by simply replacing ".count" with
	 * ".foldN.count" and ".lm" with ".foldN.lm", where N is the fold num, in the
	 * model FLM spec file.
	 */
	...
	/**
	 * Prepares a scoring model from the training data, by exec-ing the SRILM
	 * fngram-count tool with the FLM spec file, and computes perplexity on the
	 * test data. The training/test data are written to foldN-train.txt and
	 * foldN-test.txt, if not already present. The fold-specific FLM spec file's
	 * name is determined by filename(N).
	 */
	...
	protected String countNgrams(int foldNum) {
		String cmd = "fngram-count -nonull -write-counts -lm " + ((unk) ? "-unk " : "")
				+ "-factor-file " + filename(foldNum) + " " + "-text " + trainingfile(foldNum)
				+ " " + "-debug " + debugLevel;
		return cmd;
	}

	/**
	 * Returns the command for calculating perplexity. NB: At present, only the
	 * perplexity from the first model is collected.
	 */
	protected String scoreNgrams(int foldNum) {
		String lmfile = filename(foldNum);
		String cmd2 = "fngram -nonull " + ((unk) ? "-unk " : "") + "-factor-file " + lmfile + " "
