 ...
 * notCompellinglyDifferent if the n-1 initial and final words are the same,
 * where n is the n-gram order of interest. The single arg constructor defaults
 * the singleBestPerGroup flag to true, which can increase efficiency with no
 * loss in quality when only the single best output is of interest (as long as
 * the reduction in the search space outweighs the extra time necessary to check
 * for the same initial and final words).
 *
...
public class NgramDiversityPruningStrategy extends DiversityPruningStrategy {
	 ...
	 * particular, returns true iff the n-1 initial and final words are the
	 * same.
	 */
