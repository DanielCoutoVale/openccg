...
/**
 * Scores a sign according to how repetitive its words are given the observed
 * context. Relevant repeated items (eg stems) are counted, with full counts
 * given to items in the previous words or recent context, and fractional counts
 * to older items. The score is then assigned according to the number of
 * repeated items and the configured penalty, as 10 to the minus (penalty times
 * repeated items).
 *
...
public class RepetitionScorer implements SymbolScorer {
	...
	/**
	 * Default constructor. Adds "NNP", "N", "V", "Adj" and "Adv" to
	 * posValsToUse, and "do" and "not" to stemsToIgnore.
	 */
	...
	public void resetContext() {
		contextItems.clear();
		olderContextItems.clear();
		evenOlderContextItems.clear();
		oldestContextItems.clear();
	}

	/** Ages the context items, clearing the recent ones. */
	public void ageContext() {
		oldestContextItems.clear();
		oldestContextItems.addAll(evenOlderContextItems);
		evenOlderContextItems.clear();
		evenOlderContextItems.addAll(olderContextItems);
		olderContextItems.clear();
		olderContextItems.addAll(contextItems);
		contextItems.clear();
	}

	/**
	 * Adds the items (eg stems) from the given sign's words to the context
	 * items.
	 */
	...
	/**
	 * Adds the items (eg stems) from the given word to the given set. By
	 * default, adds the relevant stems, per the relevantStem method.
	 */
	...
	/**
	 * Returns the stem of the given word if its POS is in posValsToUse, unless
	 * the stem is in stemsToIgnore; otherwise returns null.
	 */
	protected String relevantStem(Association word) {
		if (!(posValsToUse.contains(word.getFunctions())))
			return null;
		String stem = word.getTerm();
		if (!(stemsToIgnore.contains(stem)))
			return stem;
		return null;
	}
	 ...
	 * completeness flag, according to how repetitive its word are compared to
	 * the observed context. In particular, returns 10 to the minus (penalty
	 ...
	 * using fractional counts for repetitions of older items. The previous
	 * items set is cleared, and then the repeated items are summed for each
	 * word, updating the previous items along the way.
	 */
	...
	/**
	 * Returns the number of repeated items (eg stems) in the given word, using
	 * fractional counts for repetitions of older items. By default, returns 1
	 * (or a fractional count) if the stem is relevant, per the relevantStem
	 * method.
	 */
	protected double repeatedItems(Association word) {
		...
		if (stem == null)
			return 0;
		if (contextItems.contains(stem) || previousItems.contains(stem))
			return 1;
		if (olderContextItems.contains(stem))
			return olderCount;
		if (evenOlderContextItems.contains(stem))
			return evenOlderCount;
		if (oldestContextItems.contains(stem))
			return oldestCount;
		return 0;
	}
}
