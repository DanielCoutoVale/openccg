 ...
 * or "an" followed by a word beginning with a consonant. Note that this filter
 * only makes an approximate check, which may be augmented with a set of
 * exceptions. Exceptions may be culled from a file of bigrams using
 * cullAAnExceptions, which may be accessed from the command line using the -c
 * option. An appropriate bigrams file can be produced using the SRILM
 * ngram-count tool, with the -text and -write2 options.
 *
...
public class AAnFilter implements NgramFilter, Reversible {
	...
	/**
	 * Returns whether to filter out the given word bigram (reversed if
	 * apropos).
	 */
	...
	// words for a/an
	private static final Association A_WORD = AssociationPool.createMuster("a");
	private static final Association AN_WORD = AssociationPool.createMuster("an");
	...
	private boolean isException(String w1, String w2) {
		...
		keyList.add(AssociationPool.createMuster(w2));
		return exceptions.contains(keyList);
	}

	// singletons for a/an
	@SuppressWarnings("unchecked")
	private static final List<Association> A_SINGLETON = (List<Association>) Interner
			.globalIntern(new SingletonList<Association>(A_WORD));
	@SuppressWarnings("unchecked")
	private static final List<Association> AN_SINGLETON = (List<Association>) Interner
			.globalIntern(new SingletonList<Association>(AN_WORD));
	...
	public void addException(String w1, String w2) {
		...
		w1 = w1.intern();
		if (w1 != "a" && w1 != "an") {
			System.err.println("Warning: ignoring exception not starting with a/an: " + w1 + " "
					+ w2);
			return;
		}
		// ensure exceptions initialized
		if (exceptions == null)
			exceptions = new THashSet();
		...
		List<Association> w2Singleton = (List<Association>) Interner
				.globalIntern(new SingletonList<Association>(AssociationPool.createMuster(w2)));
	...
	/**
	 * Culls a/an exceptions from the given infile of bigrams, writing them to
	 * the given outfile.
	 */
