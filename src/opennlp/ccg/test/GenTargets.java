...
/**
 * Generates initial target sentences from a list of logical forms. The input is
 * an XML file with a list of top-level LF elements. The output is a testbed
 * file, with the number of parses just set to 1 (rather than actually being
 * computed). The best realization is determined using n-grams from an existing
 * testbed file.
 *
...
public class GenTargets {
	...
	/**
	 * Constructor: loads grammar, instantiates realizer, and sets up n-gram
	 * scorer using targets with sem class replacement from the testbed.
	 */
	...
	private void realize(Element lfElt, Element outRoot) throws IOException {
		...
		// make test item (w/o trying to figure out the correct number of
		// parses)
