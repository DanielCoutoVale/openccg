...
public class CrossValidateRealizer implements ScorerMaker {
	...
	public void setupInputs(URL testbedURL) throws IOException {
		...
		try {
			...
			shuffledItems.toArray(shuffledItemsArray); // need a non-live
														// listing
	...
	/**
	 * Default implementation of ScorerMaker.loadScorer. Loads an n-gram
	 * precision model with semantic class replacement, using targets from the
	 * training data.
	 */
	public SymbolScorer loadScorer(File tmpDir, int foldNum, File trainFile) throws IOException {
		...
		NgramPrecisionModel retval = (tester.ngramOrder > 0) ? new NgramPrecisionModel(targets,
				tester.ngramOrder, true) : new NgramPrecisionModel(targets, true);
		return retval;
	}
	...
	public static void main(String[] args) throws IOException {
		...
		String usage = "java opennlp.ccg.test.CrossValidateRealizer "
				+ "(-folds N) (-tmp <tmpdir>) " + "(-setuponly) (-skipsetup) "
				+ "(-preponly) (-skipprep) " + "(-ngramorder N) "
		...
		if (pruningStrategyClass != null) {
			try {
				cvr.pruningStrategy = (PruningStrategy) Class.forName(pruningStrategyClass)
						.newInstance();
			} catch (Exception exc) {
				throw (RuntimeException) new RuntimeException().initCause(exc);
			}
