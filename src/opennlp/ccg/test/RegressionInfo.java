...
public class RegressionInfo {
	...
	public class TestItem {
		...
		/**
		 * The full words for the sentence/phrase, or null if none, formatted by
		...
		/**
		 * The gold std nominal id name & supertag for LF predicates. @deprecated
		 * Should use fullWords.
		 */
	...
	/**
	 * Reads in the given regression test file and corresponding .ser file, if
	 * any.
	 */
	...
	/**
	 * Reads in a regression test from the given input stream and object input
	 * stream.
	 */
	...
	public RegressionInfo(Grammar grammar, InputStream istr, ObjectInputStream serStream) {
		...
		try {
			...
			for (int i = 0; i < items.size(); i++) {
				...
				testItem.knownFailure = ("true".equals(item.getAttributeValue("known"))) ? true
						: false;
				...
				if (signMap != null && testItem.info != null)
	...
	/**
	 * Reads in a map of info keys and gold standard signs from the given
	 * stream, or returns null if the stream is null.
	 */
	@SuppressWarnings("unchecked")
	...
	/**
	 * Writes the map of info keys and gold standard signs to the corresponding
	 * .ser file.
	 */
	public static void writeSerFile(Map<String, Symbol> signMap, File regressionFile)
	 ...
	 * applying the configured to-XML transformations.
	 */
	public static Element makeTestItem(Grammar grammar, String target, int numParses, LF lf)
			throws IOException {
		return makeTestItem(grammar, target, numParses, lf, null);
	}

	/**
	 * Makes an XML test item with the given string, number of parses, LF and
	 * info attribute, applying the configured to-XML transformations.
	 */
	public static Element makeTestItem(Grammar grammar, String target, int numParses, LF lf,
	...
	/**
	 * Adds the given sign with its string, number of parses and converted LF as
	 * a test item to the testbed with the given filename, applying the
	 * configured to-XML transformations.
	 */
	public static void addToTestbed(Grammar grammar, Symbol sign, int numParses, LF lf,
		...
		if (signMap == null)
			signMap = new HashMap<String, Symbol>();
		...
		// append new item
		if (!newDoc)
			root.addContent("  "); // nb: for some reason, this gets the
									// indenting right
