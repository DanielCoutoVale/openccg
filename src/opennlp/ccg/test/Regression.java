...
public class Regression {
	...
	/**
	 * Flag for whether to normalize strings as for BLEU scoring in n-best
	 * output.
	 */
	...
	private XMLOutputter xmlOutputter = new XMLOutputter(); // for xml-escaping
															// strings
	...
	public void resetTotals() {
		...
		totalDepsF = 0.0;
		totalDepsRecall = 0.0;
		totalDepsPrecision = 0.0;
		totalUnlabeledDepsF = 0.0;
		totalUnlabeledDepsRecall = 0.0;
		totalUnlabeledDepsPrecision = 0.0;
		totalFComplete = 0.0;
		totalRecallComplete = 0.0;
		totalPrecisionComplete = 0.0;
		totalDepsFComplete = 0.0;
		totalDepsRecallComplete = 0.0;
		totalDepsPrecisionComplete = 0.0;
		totalUnlabeledDepsFComplete = 0.0;
		totalUnlabeledDepsRecallComplete = 0.0;
		totalUnlabeledDepsPrecisionComplete = 0.0;
		pTotalEdges = 0;
		pTotalEdgesGood = 0;
		pMaxEdges = 0;
		pMaxEdgesGood = 0;
		pTotalUnpackingEdges = 0;
		pMaxUnpackingEdges = 0;
		pTotalCellMax = 0;
		pTotalCellMaxGood = 0;
		pMaxCellMax = 0;
		pMaxCellMaxGood = 0;
	...
	private void bleuSetup() throws IOException {
		...
		if (bleufileprefix != null && doRealization) {
			bleuGen = new PrintWriter(new BufferedWriter(
					new FileWriter(bleufileprefix + "-gen.sgm")));
			bleuRef = new PrintWriter(new BufferedWriter(
					new FileWriter(bleufileprefix + "-ref.sgm")));
			bleuSrc = new PrintWriter(new BufferedWriter(
					new FileWriter(bleufileprefix + "-src.sgm")));
	...
	private String xmlEscape(String s) {
		return xmlOutputter.outputString(new Text(s));
	}

	// normalizes realizations for BLEU scoring
	// at present, this means replacing underscores with spaces and escaping for
	// xml output
	private String norm_bleu(String s) {
		return xmlEscape(s.replace('_', ' '));
	}
	...
	public void runTest(File regressionFile) throws IOException {
		...
		if (eventfile != null)
			events = EventFile.openWriter(new File(eventfile));
		// set up bleu output, n-best realizations, rescoring, n-best parses (if
		// apropos)
		bleuSetup();
		nbestrealSetup();
		rescoreSetup();
		...
		for (File f : getXMLFiles(regressionFile))
			runSingleTest(f);
		// finish bleu, n-best realization output, rescoring, n-best parses (if
		// apropos)
		bleuFinish();
		nbestrealFinish();
		rescoreFinish();
	...
	private void runSingleTest(File regressionFile) throws IOException {
		...
		for (int i = 0; i < numItems; i++) {
			...
			if (testItem.lfElt != null
					&& (doRealization || (showParseStats && parsed && testItem.sign == null))) {
			...
			if (nbestparsePW != null) {
				...
				if (id == null)
					id = "" + i;
				nbestparsePW.println("<seg id=\"" + id + "\" str=\"" + xmlEscape(testItem.sentence)
						+ "\"" + extras + ">");
				String tagend = (nbestincludelfs) ? ">" : "/>";
				// add best parse
				if (parseScore != null) {
					double edgeScore = parseScores.get(0);
					String scores = "score=\"" + nf.format(parseScore.fscore) + "\" edge-score=\""
							+ nfE.format(edgeScore) + "\"";
				...
				for (int k = 1; k < parses.size(); k++) {
					...
					String scores = "score=\"" + nf.format(parseScoreK.fscore) + "\" edge-score=\""
							+ nfE.format(edgeScore) + "\"";
			...
			// set up n-gram precision scorer for default scoring and/or scoring
			// results
			String[] targets = (testItem.alt == null) ? new String[] { testItem.sentence }
			...
			if (scorerToUse == null) {
				if (ngramOrder > 0 || exactMatches) {
					if (ngramOrder > 0)
						scorerToUse = new NgramPrecisionModel(targets, ngramOrder);
					else
						scorerToUse = new NgramPrecisionModel(targets);
			...
			double score = defaultNgramScorer.score(bestEdge.getSign(), false); // nb:
																				// use
																				// default
																				// n-gram
																				// precision
			...
			if (bleufileprefix != null) {
				String extras = " time=\"";
				if (chart.done)
					extras += chart.timeTilDone;
				else
					extras += chart.timeTilStopped;
				extras += "\"";
				...
				if (id == null)
					id = "" + i;
				bleuGen.println("<seg id=\"" + id + "\"" + extras + ">"
						+ norm_bleu(bestRealization) + "</seg>");
			...
			if (nbestrealPW != null) {
				...
				// add ref sentence
				String ref = (nbestnormbleu) ? norm_bleu(testItem.sentence)
						: xmlEscape(testItem.sentence);
				nbestrealPW.println("<ref>" + ref + "</ref>");
				// add best realization
				String scores = "score=\"" + nf.format(score) + "\" edge-score=\""
						+ nfE.format(bestEdge.score) + "\"";
				String best = (nbestnormbleu) ? norm_bleu(bestRealization)
						: xmlEscape(bestRealization);
				if (!nbestincludelfs)
				...
				if (bestEdge.complete()) {
					...
					for (int j = 1; j < bestEdges.size(); j++) {
						...
						double eScore = defaultNgramScorer.score(e.getSign(), false); // nb:
																						// use
																						// default
																						// n-gram
																						// precision
																						...
																						// for
																						// reporting
						String eScores = " score=\"" + nf.format(eScore) + "\" edge-score=\""
								+ nfE.format(e.score) + "\"";
			...
			// store best realization, if apropos and grammatically complete,
			// keyed by info string or item position
			...
			totalTokens += tokens;
			if (tokens < minTokens || minTokens == 0)
				minTokens = tokens;
			if (tokens > maxTokens)
				maxTokens = tokens;
	...
	public void showParseStats() {
		...
		System.out.println("Strings parsed completely (in fragments, failed): " + pComplete + " ("
				+ pFragCount + ", " + pFailedCount + ") " + pCompletePct);
		String pInexact = "" + (pCount - pExactCount);
		String pExactPct = "" + nf.format(100.0 * pExactCount / pCount) + "%";
		System.out.println("Strings parsed exactly (inexactly): " + pExactCount + " (" + pInexact
				+ ") " + pExactPct);
		...
		System.out.println("Average edge count (before unpacking) for complete parses: "
				+ avgEdgesGood);
		...
		System.out.println("Average max cell size (before unpacking) for complete parses: "
				+ avgCellMaxGood);
		...
		System.out
				.println("Labeled recall deps only for complete parses: " + avgDepsRecallComplete);
		String avgDepsPrecisionComplete = nf.format(100.0 * totalDepsPrecisionComplete
				/ pCompleteCount);
		System.out.println("Labeled precision deps only for complete parses: "
				+ avgDepsPrecisionComplete);
		String avgUnlabeledDepsFComplete = nf.format(100.0 * totalUnlabeledDepsFComplete
				/ pCompleteCount);
		System.out.println("Unlabeled deps f-score for complete parses: "
				+ avgUnlabeledDepsFComplete);
		String avgUnlabeledDepsRecallComplete = nf.format(100.0 * totalUnlabeledDepsRecallComplete
				/ pCompleteCount);
		System.out.println("Unlabeled deps recall for complete parses: "
				+ avgUnlabeledDepsRecallComplete);
		String avgUnlabeledDepsPrecisionComplete = nf.format(100.0
				* totalUnlabeledDepsPrecisionComplete / pCompleteCount);
		System.out.println("Unlabeled deps precision for complete parses: "
				+ avgUnlabeledDepsPrecisionComplete);
		if (oracleBetter > 0)
			System.out.println("Oracle better: " + oracleBetter);
		if (goldMissing > 0)
			System.out.println("Gold missing: " + goldMissing);
		System.out.println();
	}
	...
	public void showStats() {
		...
		System.out.println("Strings realized completely (in fragments): " + rComplete + " ("
				+ rBadCount + ") " + rCompletePct);
		...
		String residualMRR = (rCount == rExactCount) ? "n/a" : nf
				.format((totalReciprocalRank - rExactCount) / (rCount - rExactCount));
		...
		System.out.println("Avg num pruned edges removed, never added: " + avgRemoved + ", "
				+ avgNeverAdded);
		...
		System.out.println("Total, avg num new best realizations: " + totalNewBest + ", "
				+ avgNewBest);
		...
		System.out.println("Max time 'til new best realization: " + maxNewBest + " ("
				+ maxNewBestStr + ")");
		...
		System.out.println("Avg (std) time 'til stopped/done with realizations: "
				+ avgStoppedOrDone + " (" + stdStoppedOrDone + ")");
		System.out.println("Max time 'til stopped/done with realizations: " + maxStoppedOrDone
				+ " (" + maxStoppedOrDoneStr + ")");
		if (oracleBetter > 0)
			System.out.println("Oracle better: " + oracleBetter);
		if (goldMissing > 0)
		...
		if (statsfile != null) {
			...
			if (oracleBetter > 0)
				edges.setAttribute("oracle-better", "" + oracleBetter);
			if (goldMissing > 0)
				edges.setAttribute("gold-missing", "" + goldMissing);
	...
	private static void showOutcome(String parseResult, String realizeResult,
			String starForBadSentence, String str) {
		showOutcome(parseResult, realizeResult, starForBadSentence, str, null);
	}

	// show outcome including best realization
	private static void showOutcome(String parseResult, String realizeResult,
			String starForBadSentence, String str, String bestRealization) {
	...
	private static void simpleWrap(String str) {
		...
		for (int i = 0; i <= (str.length() - 1) / TEXTWIDTH; i++) {
			...
			System.out.println(str.substring(i * TEXTWIDTH,
					Math.min(i * TEXTWIDTH + TEXTWIDTH, str.length())));
		}
	}
	...
	static void showRealizerSettings() {
		...
		int timeLimit = prefs.getInt(opennlp.ccg.realize.Chart.TIME_LIMIT,
				opennlp.ccg.realize.Chart.NO_TIME_LIMIT);
		double nbTimeLimit = prefs.getDouble(opennlp.ccg.realize.Chart.NEW_BEST_TIME_LIMIT,
				opennlp.ccg.realize.Chart.NO_TIME_LIMIT);
		int pruningVal = prefs.getInt(opennlp.ccg.realize.Chart.PRUNING_VALUE,
				opennlp.ccg.realize.Chart.NO_PRUNING);
		int cellPruningVal = prefs.getInt(opennlp.ccg.realize.Chart.CELL_PRUNING_VALUE,
				opennlp.ccg.realize.Chart.NO_PRUNING);
		...
		if (usePacking)
			...
			if (timeLimit == opennlp.ccg.realize.Chart.NO_TIME_LIMIT)
				msg += "no time limit, ";
			else
				msg += "a time limit of " + timeLimit + " ms, ";
			if (nbTimeLimit == opennlp.ccg.realize.Chart.NO_TIME_LIMIT)
				msg += "no new best time limit, ";
			else {
				msg += "a new best time limit of ";
				if (nbTimeLimit >= 1)
					msg += ((int) nbTimeLimit) + " ms, ";
				else
					msg += nbTimeLimit + " of first, ";
			}
		}
		if (pruningVal == opennlp.ccg.realize.Chart.NO_PRUNING)
			msg += "no pruning, ";
		else
			msg += "a pruning value of " + pruningVal + ", ";
		msg += "and ";
		if (cellPruningVal == opennlp.ccg.realize.Chart.NO_PRUNING)
			msg += "no cell pruning";
		else
			msg += "a cell pruning value of " + cellPruningVal;
	...
	/**
	 * Writes the target strings with semantic class replacement from the given
	 * testbed to the given textfile.
	 */
	...
	/**
	 * Writes the target strings with all associated factors from the given
	 * testbed to the given textfile.
	 */
	...
	/**
	 * Writes the target strings with all associated factors with semantic class
	 * replacement from the given testbed to the given textfile.
	 */
	...
	// and optionally reversing the words; ungrammatical options are filtered
	// out
	private void writeTargets(File tbFile, String filename, boolean semClassReplacement,
			boolean withFactors, boolean reverse) throws IOException {
		// open text file
		String option = "";
		if (withFactors)
			option = " with factors";
		if (semClassReplacement)
			option += " with semantic class replacement";
		if (reverse)
			option += ", reversed";
			...
			for (int i = 0; i < numItems; i++) {
				...
				if (semClassReplacement || withFactors) {
					...
					// otherwise parse
						...
						words = grammar.getParsedWords(s);
				} else
				...
				if (reverse) {
					...
					words.add(AssociationPool.createMuster("<s>"));
	...
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i < args.length; i++) {
			...
			if (args[i].equals("-nullscorer")) {
				tester.scorer = SymbolScorer.nullScorer;
				tester.parseScorer = SymbolScorer.nullScorer;
				continue;
			}
			if (args[i].equals("-randomscorer")) {
				tester.scorer = SymbolScorer.randomScorer;
				tester.parseScorer = SymbolScorer.randomScorer;
				continue;
			}
			...
			if (args[i].equals("-srilm")) {
				...
				if (i < (args.length - 1)) {
					...
					try {
						...
						System.err.println("Warning: unknown SRILM n-gram model type " + type
								+ " specified, using default (" + srilmModelType + ")");
					}
				}
				continue;
			}
		...
		if (lmfile != null) {
			...
			System.out.println("Loading " + reversedStr + order + "-gram model from file: "
					+ lmfile);
			NgramScorer lmScorer = (srilm) ? new SRILMNgramModel(order, new File(lmfile),
					useSemClasses, srilmModelType) : new StandardNgramModel(order, lmfile,
					useSemClasses);
			if (reverse)
		...
		if (flmfile != null) {
			String reversedStr = (reverse) ? "reversed " : "";
			System.out.println("Loading " + reversedStr
					+ "factored n-gram model family from file: " + flmfile);
			NgramScorer flmScorer = new FactoredNgramModelFamily(flmfile, useSemClasses);
			if (reverse)
				flmScorer.setReverse(true);
			if (aanfilter)
		...
		// with -text|-textsc|-textf|-textfsc options, just write text file and
		// exit
		if (textfile != null) {
		...
		if (tester.doParsing) {
			...
			if (parseScorerClass != null) {
				try {
					System.out.println("Instantiating parsing sign scorer from class: "
							+ parseScorerClass);
					tester.parseScorer = (SymbolScorer) Class.forName(parseScorerClass)
							.newInstance();
			...
			if (tester.parseScorer != null)
				tester.parser.setSymbolScorer(tester.parseScorer);
			...
			if (supertaggerClass != null || stconfig != null) {
				try {
					...
					if (supertaggerClass != null) {
						System.out.println("Instantiating supertagger from class: "
								+ supertaggerClass);
						supertagger = (Supertagger) Class.forName(supertaggerClass).newInstance();
					} else {
						System.out.println("Instantiating supertagger from config file: "
								+ stconfig);
						supertagger = WordAndPOSDictionaryLabellingStrategy
								.supertaggerFactory(stconfig);
					}
					tester.parser.setSupertagger(supertagger);
					if (tester.eventfile != null) {
						supertagger.setIncludeGold(true); // use gold tags
															// during training
						tester.parser.setSupertaggerMostToLeastRestrictiveDirection(false); // reverse
																							// direction
																							// to
																							// start
																							// with
		...
		if (tester.doRealization) {
			...
			if (ngrampruningstrategy) {
				int order = (tester.ngramOrder > 0) ? tester.ngramOrder : 3;
				System.out.println("Instantiating n-gram diversity pruning strategy with order "
						+ order);
			...
			if (pruningStrategyClass != null) {
				try {
					System.out.println("Instantiating pruning strategy from class: "
							+ pruningStrategyClass);
					tester.realizer.pruningStrategy = (PruningStrategy) Class.forName(
							pruningStrategyClass).newInstance();
					System.out.println();
				} catch (Exception exc) {
					throw (RuntimeException) new RuntimeException().initCause(exc);
				}
			...
			if (hypertaggerClass != null || htconfig != null) {
				try {
					...
					if (hypertaggerClass != null) {
						System.out.println("Instantiating hypertagger from class: "
								+ hypertaggerClass);
						hypertagger = (Hypertagger) Class.forName(hypertaggerClass).newInstance();
					} else {
						System.out.println("Instantiating hypertagger from config file: "
								+ htconfig);
						hypertagger = ZLMaxentHypertagger.ZLMaxentHypertaggerFactory(htconfig);
					}
					tester.realizer.hypertagger = hypertagger;
					if (tester.eventfile != null) {
						hypertagger.setIncludeGold(true); // use gold tags
															// during training
						// NB: could investigate most-to-least-restrictive
						// direction
					}
					if (htgold)
						hypertagger.setIncludeGold(true); // use gold tags, eg
															// for paraphrasing
					System.out.println();
				} catch (Exception exc) {
					throw (RuntimeException) new RuntimeException().initCause(exc);
		...
		if (tester.eventfile != null) {
			...
			if (tester.doParsing && tester.doRealization) {
				throw new RuntimeException(
						"Events can't be generated for parsing and realization at the same time.");
			}
			FeatureExtractor extractor = null;
			if (extractorClass != null) {
				try {
					System.out.println("Instantiating feature extractor from class: "
							+ extractorClass);
			...
			if (tester.scorer instanceof FeatureExtractor) {
				if (extractor != null)
					tester.featureExtractor = new ComposedFeatureExtractor(new FeatureExtractor[] {
							(FeatureExtractor) tester.scorer, extractor });
				else
					tester.featureExtractor = (FeatureExtractor) tester.scorer;
			} else if (tester.parseScorer instanceof FeatureExtractor) {
				if (extractor != null)
					tester.featureExtractor = new ComposedFeatureExtractor(new FeatureExtractor[] {
							(FeatureExtractor) tester.parseScorer, extractor });
				else
					tester.featureExtractor = (FeatureExtractor) tester.parseScorer;
			}
		...
		if (tester.realserdir != null) {
			...
			System.out.println("Writing best realization serialization files to dir: "
					+ tester.realserdir);
			System.out.println();
		}
