...
public class TextCCG {
	...
	public static void main(String[] args) throws IOException, LexException {
		...
		String[] completions = { ":sh", ":v", ":reset", ":feats", ":nofeats", ":foff", ":sem",
				":nosem", ":all", ":notall", ":derivs", ":noderivs", ":doff", ":vison", ":visoff",
				":wordpos", ":nowordpos", ":eisner", ":noeisner", ":ptl", ":noptl", ":pel",
				":nopel", ":ppv", ":noppv", ":pcpv", ":nopcpv", ":plazy", ":noplazy", ":r", ":sel",
				":2xml", ":2tb", ":2apml", ":tl", ":notl", ":el", ":noel", ":nbtl", ":nonbtl",
				":pv", ":nopv", ":cpv", ":nocpv", ":upon", ":upoff", ":t", ":toff", ":inc",
				":noinc", ":ion", ":ioff", ":con", ":coff", ":flon", ":floff", ":ccon", ":ccoff",
				":pon", ":poff", ":q", ":h" };
		...
		while (true) {
			...
			if (input.equals(":show settings") || input.equals(":sh")) {
				...
				prefs.putInt(opennlp.ccg.realize.Chart.TIME_LIMIT,
						opennlp.ccg.realize.Chart.NO_TIME_LIMIT);
				prefs.putDouble(opennlp.ccg.realize.Chart.NEW_BEST_TIME_LIMIT,
						opennlp.ccg.realize.Chart.NO_TIME_LIMIT);
				prefs.putInt(opennlp.ccg.realize.Chart.EDGE_LIMIT,
						opennlp.ccg.realize.Chart.NO_EDGE_LIMIT);
				prefs.putInt(opennlp.ccg.realize.Chart.PRUNING_VALUE,
						opennlp.ccg.realize.Chart.NO_PRUNING);
				prefs.putInt(opennlp.ccg.realize.Chart.CELL_PRUNING_VALUE,
						opennlp.ccg.realize.Chart.NO_PRUNING);
			...
			} else if (input.startsWith(":show feats ") || input.startsWith(":feats ")
					|| input.startsWith(":f ")) {
				prefs.putBoolean(SHOW_FEATURES, true);
				...
				Grammar.theGrammar.prefs.featsToShow = "";
			} else if (input.equals(":show semantics") || input.equals(":sem")
					|| input.equals(":s")) {
				prefs.putBoolean(SHOW_SEMANTICS, true);
				...
				prefs.putBoolean(SHOW_ALL_RESULTS, false);
			} else if (input.equals(":show derivs") || input.equals(":derivs")
					|| input.equals(":d")) {
				prefs.putBoolean(SHOW_DERIVATIONS, true);
				...
				try {
					...
					System.out.println("Expecting an integer cell pruning value, rather than: "
							+ last);
				}
				...
				// nb: need to upgrade, consolidate :r FN option with
				// Realize.java ...
				...
				RegressionInfo.addToTestbed(grammar, lastResults[lastReading], lastResults.length,
						lastLFs[lastReading], filename);
				...
				System.out
						.println("Wrote \"" + lastSentence + "\" to \"" + filename + "\" as APML");
				...
				if ((input.startsWith(":vison ")) && (input.length() >= 8)) {
					...
					if (fname.lastIndexOf('.') != -1) {
						System.out
								.println("Filename should not contain a suffix. Suffixes .tex and .dvi are assumed.");
						prefs.put("VISFNAME", "");
					} else
			...
			} else if (input.startsWith(":no parse cell pruning value")
					|| input.startsWith(":nopcpv")) {
				prefs.putInt(Parser.FORM_PRUNE_LIMIT, Parser.NO_PRUNE_LIMIT);
				...
				try {
					...
					for (int i = 0; i < numToShow; i++) {
						...
						if (cat.getLF() != null) {
							...
							convertedLF = HyloHelper.getInstance().compactAndConvertNominals(
									cat.getLF(), index, rootSign);
							lastLFs[i] = convertedLF;
							cat.setLF(null);
						}
						String parseNum = (resLength == 1) ? "Parse: "
								: ("Parse " + (i + 1) + ": ");
					...
					if (visualize) {
						...
						if (prefs.get("VISFNAME", "").equals("")) // If
																	// temporary
																	// file,
							vis.cleanFiles(baseFileName); // clean it
						else {
							vis.cleanAuxFiles(baseFileName);
							System.out.println("Saved to files " + baseFileName + ".tex and "
									+ baseFileName + ".dvi");
						}
						vis = null;
					}
	...
	public static void showHelp() {
		...
		System.out
				.println("  :2tb (FN)\t\tadd last input and LF as a test item (to file with name FN)");
		...
		System.out
				.println("  :nbtl N\t\tset realization new best time limit to N ms | N < 1 of first");
	...
	public static void showSettings(Preferences prefs) {
		...
		System.out.println("  parse time limit:\t"
				+ ((ptl == Parser.NO_TIME_LIMIT) ? "none" : "" + ptl + " ms"));
		int pel = prefs.getInt(Parser.PARSE_SCORED_SYMBOL_LIMIT, Parser.NO_SCORED_SYMBOL_LIMIT);
		System.out.println("  parse edge limit:\t"
				+ ((pel == Parser.NO_SCORED_SYMBOL_LIMIT) ? "none" : "" + pel));
		int ppv = prefs.getInt(Parser.PARSE_PRUNE_LIMIT, Parser.NO_PRUNE_LIMIT);
		System.out.println("  parse pruning value:\t"
				+ ((ppv == Parser.NO_PRUNE_LIMIT) ? "none" : "" + ppv));
		int pcpv = prefs.getInt(Parser.FORM_PRUNE_LIMIT, Parser.NO_PRUNE_LIMIT);
		System.out.println("  parse cell prune val:\t"
				+ ((pcpv == Parser.NO_PRUNE_LIMIT) ? "none" : "" + pcpv));
		boolean plazy = prefs.getBoolean(Parser.LAZY_UNPACKING, true);
		System.out.println("  lazy unpacking:\t" + ((plazy) ? "on" : "off"));
		System.out.println();
		int tl = prefs.getInt(opennlp.ccg.realize.Chart.TIME_LIMIT,
				opennlp.ccg.realize.Chart.NO_TIME_LIMIT);
		System.out.println("  time limit:\t\t"
				+ ((tl == opennlp.ccg.realize.Chart.NO_TIME_LIMIT) ? "none" : "" + tl + " ms"));
		double nbtl = prefs.getDouble(opennlp.ccg.realize.Chart.NEW_BEST_TIME_LIMIT,
				opennlp.ccg.realize.Chart.NO_TIME_LIMIT);
		String nbtlStr = (nbtl >= 1) ? (((int) nbtl) + " ms") : (nbtl + " of first");
		System.out.println("  new best time limit:\t"
				+ ((nbtl == opennlp.ccg.realize.Chart.NO_TIME_LIMIT) ? "none" : nbtlStr));
		int el = prefs.getInt(opennlp.ccg.realize.Chart.EDGE_LIMIT,
				opennlp.ccg.realize.Chart.NO_EDGE_LIMIT);
		System.out.println("  edge limit:\t\t"
				+ ((el == opennlp.ccg.realize.Chart.NO_EDGE_LIMIT) ? "none" : "" + el));
		int pv = prefs.getInt(opennlp.ccg.realize.Chart.PRUNING_VALUE,
				opennlp.ccg.realize.Chart.NO_PRUNING);
		System.out.println("  pruning value:\t"
				+ ((pv == opennlp.ccg.realize.Chart.NO_PRUNING) ? "none" : "" + pv));
		int cpv = prefs.getInt(opennlp.ccg.realize.Chart.CELL_PRUNING_VALUE,
				opennlp.ccg.realize.Chart.NO_PRUNING);
		System.out.println("  cell pruning value:\t"
				+ ((cpv == opennlp.ccg.realize.Chart.NO_PRUNING) ? "none" : "" + cpv));
