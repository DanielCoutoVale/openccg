...
/**
 * Class for composing feature extractors. Features from the component feature
 * extractors are assumed to be independent.
 * 
...
public class ComposedFeatureExtractor implements FeatureExtractor {
	...
	/** Binary constructor. */
	public ComposedFeatureExtractor(FeatureExtractor featureExtractor1,
	...
	public ComposedFeatureExtractor(SymbolScorer[] models) {
		...
		for (int i = 0; i < models.length; i++) {
			if (models[i] instanceof FeatureExtractor)
				feList.add((FeatureExtractor) models[i]);
		}
