 ...
 * 
 * An alphabet can be read from either an alphabet file or a model file. An
 * alphabet file starts with the number of features on one line, followed by one
 * line per feature pairing the feature name with its frequency (which is
 * ignored).
 * 
 * An alphabet can be open or closed. An closed alphabet does not accept new
 * features, and thus can be used to filter out features not already in the
 * alphabet.
 * 
...
public class Alphabet {
	...
	/**
	 * Constructor to load an alphabet from a file. The alphabet is set to
	 * closed.
	 */
	...
	/**
	 * Add feature with given name, if not already present, returning added
	 * feature.
	 */
	...
	/**
	 * Add feature with given keys, if not already present, returning added
	 * feature.
	 */
	...
	/**
	 * Get or add index of feature with given key extractors (null if none when
	 * closed).
	 */
	public Feature indexLazy(List<TrieMap.KeyExtractor<String>> keyExtractors) {
		if (!closed)
			return addLazy(keyExtractors);
	...
	public Feature feature(int index) {
		return dictInv.get(index);
	}

	/**
	 * Creates an alphabet file from an event file, saving it to a file with
	 * features sorted by frequency, using the given table size and pruning
	 * threshold.
	 **/
	public static void createAlphabet(String eventfile, String alphabetfile, int tablesize,
			int pruningthreshold) throws IOException {
		// open files
		...
		List<Map.Entry<Feature, Integer>> tallies = new ArrayList<Map.Entry<Feature, Integer>>(
				freqTally.size());
		...
		Collections.sort(tallies, new Comparator<Map.Entry<Feature, Integer>>() {
			public int compare(Map.Entry<Feature, Integer> entry1,
					Map.Entry<Feature, Integer> entry2) {
				...
				return 0;
				// return
				// entry1.getKey().name().compareTo(entry2.getKey().name());
			}
		});
	...
	public static void main(String[] args) throws IOException {
		if (args.length < 2) {
			System.out
					.println("Usage: java perceptron.Alphabet <eventfile> <alphabetfile> (-s <tablesize>) (-p <pruningthreshold>");
			System.exit(0);
		}
