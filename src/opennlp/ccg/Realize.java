...
/**
 * Sample front-end to the realizer, showing the intermediate steps of
 * realization.
 *
...
public class Realize {
	...
	public static void main(String[] args) throws Exception {
		...
		if (ngramModelElt == null) {
			...
			int order = 3; // order can only be changed for standard n-gram
							// models
			...
			if (ngramOrder > 0)
				order = ngramOrder; // preference given to command-line value
			out.println();
			String msg = "Loading ";
			if (reverse)
				msg += "reversed ";
			...
			if (!factored)
				msg += "of order " + order + " ";
			if (useSemClasses)
				msg += "with semantic class replacement ";
			msg += "from: " + filename;
			out.println(msg);
			if (factored)
		...
		if (pruningStrategyElt != null) {
			...
			realizer.pruningStrategy = (PruningStrategy) Class.forName(pruningStrategyClass)
					.newInstance();
		}
