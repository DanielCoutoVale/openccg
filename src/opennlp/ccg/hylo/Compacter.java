...
public class Compacter {
	...
	static LF compact(LF lf, Nominal root) {
		...
		// compact preds with multiple parents, using parent that is closest to
		// a root
		prevSize = -1;
		while (multipleParentPreds.size() != prevSize) {
			...
			for (Iterator<SatOp> it = multipleParentPreds.iterator(); it.hasNext();) {
				SatOp pred = it.next();
				Nominal nom = pred.getNominal();
				// find parent closest to root, but checking for a parent not
				// below a root
				Set<Nominal> nomParents = multipleParents.get(nom);
				...
				for (Iterator<Nominal> it2 = nomParents.iterator(); it2.hasNext();) {
					...
					// otherwise set closest dist to -1, to indicate that not
					// all ancestors are roots
					else {
						closestDist = -1;
					}
				}
				// check for a parent not below a root, or no closest root, and
				// skip this nom if so
				if (closestDist == -1 || closestRootIndex == -1) {
					continue;
				}
		...
		if (retPreds.size() == 1) {
			retval = retPreds.get(0);
		} else {
			retval = new Op(Op.CONJ, retPreds);
		}
	...
	// returns whether the substitution has been made
	private static boolean subst(LF lf, Nominal currentParent, SatOp satOp2, Nominal nom2,
			Nominal requiredParent) {
		...
		if (lf instanceof SatOp) {
			...
			if (arg.equals(nom2)
					&& (requiredParent == null || requiredParent.equals(currentParent))) {
				// make substitution
			...
			for (int i = 0; i < args.size(); i++) {
				...
				if (arg instanceof Nominal) {
					...
					if (arg.equals(nom2)
							&& (requiredParent == null || requiredParent.equals(currentParent))) {
						// make substitution
						// nb: this (rarely used) operation doesn't nec.
						// preserve the sort order, unfortunately
	...
	private static Nominal findDupParent(LF lf, Nominal currentParent, SatOp dup, Nominal dupNom) {
		...
		if (lf instanceof SatOp) {
			...
			if (arg.equals(dupNom))
				...
				return findDupParent(arg, currentParent, dup, dupNom);
		} else if (lf instanceof Op) {
			Op op = (Op) lf;
			List<LF> args = op.getArguments();
			for (int i = 0; i < args.size(); i++) {
