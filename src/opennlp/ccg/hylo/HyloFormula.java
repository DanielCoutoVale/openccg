...
public abstract class HyloFormula implements LF, Serializable {
	...
	/**
	 * Applies a ModFcn to this LF and then applies it to all fields which are
	 * themselves Mutables.
	 *
	...
	/**
	 * Unify this Unfiable with another Object. This default implementation will
	 * reverse the direction of unification for a variable, otherwise it fails.
	 * <b>NB:</b> The implementation of unification in the hylo package is not
	 * complete; a particular limitation is that no attempt is made to unify
	 * lists of terms connected by an Op instance.
	 *
	...
	public Object unify(Object u, Substitution s) throws UnifyFailure {
		if (u instanceof Variable)
			return ((Unifiable) u).unify(this, s);
		else
			throw new UnifyFailure(this.toString(), u.toString());
	}
