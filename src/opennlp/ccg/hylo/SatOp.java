...
/**
 * A hybrid logic satifaction operator, which tests whether a formula is true a
 * particular point named by a nominal.
 *
...
public class SatOp extends HyloFormula {
	...
	/**
	 * The LF alts to which this LF belongs. Null represents no alts.
	 */
	...
	/**
	 * The LF opts (optional parts) to which this LF belongs. LF opts are used
	 * during realization to represent optional parts of the input. The opts are
	 * numbered starting with 0, and null represents no opts.
	 */
	...
	/**
	 * Gets the sign or unary rule which introduced this predication (or null if
	 * none).
	 */
	public EntityRealizer getOrigin() {
		return _origin;
	}
	 ...
	 */
	public void setOrigin(EntityRealizer origin) {
		_origin = origin;
	}
	...
	public SatOp(Element e) {
		...
		if (nom != null) {
			_nominal = new NominalAtom(HyloHelper.getInstance().prefix(nom), HyloHelper
					.getInstance().type(nom), shared);
		} else {
			nom = e.getAttributeValue("nomvar");
			if (nom != null) {
				_nominal = new NominalVar(HyloHelper.getInstance().prefix(nom), HyloHelper
						.getInstance().type(nom), shared);
			} else {
	...
	public boolean equals(Object o) {
		if (o instanceof SatOp && _nominal.equals(((SatOp) o)._nominal)
	...
	public int hashCode() {
		return _nominal.hashCode() + _arg.hashCode();
	}
	...
	/**
	 * Returns whether this sat op equals the given object up to variable names,
