...
/**
 * A class for variables over NominalAtom objects. Nominal vars take precedence
 * over generic hylo vars. Types are unified with other hylo vars and nominal
 * atoms.
 *
...
public class NominalVar extends HyloVar implements Nominal {
	...
	public int compareTo(Nominal nom) {
		...
		if (retval == 0) {
			retval = 1;
		} // atom precedes var if names equal
		return retval;
	}
	...
	public Object unify(Object u, Substitution sub) throws UnifyFailure {
		...
		// with nominal vars, substitute according to type specificity then
		// comparison order,
		...
		if (u instanceof NominalVar) {
			...
			String name = (super.compareTo(u_nv) >= 0) ? (u_nv._name + u_nv._index)
					: (_name + _index);
