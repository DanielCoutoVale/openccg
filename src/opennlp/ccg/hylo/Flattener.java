...
public class Flattener {
	...
	// map from nominals to highest pred for that nominal from original
	// expression
	...
	// map from nominal to highest parent nominal in original expression, or
	// null if a root
	...
	/**
	 * Returns a map from a nominal to its highest parent nominal in the
	 * original expression, after flattening, or null if none.
	 */
	 ...
	 * and opts (optional parts). Chunks, alts and opts are propagated through
	 * shared nominals. A runtime exception is thrown if the LF cannot be
	 * flattened.
	 */
	public List<SatOp> flatten(LF lf) {
		flatten(lf, null, null, 0, new Stack<Alt>(), new TIntArrayList());
		if (altCount > 0 || optCount > 0)
			propAltsOptsChunks();
		return preds;
	}

	// recursive flattening, with conversion of alts and opts
	private void flatten(LF lf, Nominal currentNominal, SatOp parent, int depth, Stack<Alt> alts,
			TIntArrayList opts) {
		if (lf instanceof SatOp) {
			...
			if (arg instanceof Proposition || arg instanceof Nominal || arg instanceof HyloVar) {
				...
				// and flatten the rest of the args with the first nominal arg
				// as the
				// new current nominal
				Op argOp = (Op) arg;
				...
				// add SatOp for diamond
				SatOp satOp = new SatOp(currentNominal, new Diamond(diamond.getMode(),
						firstNominalArg));
				addSatOp(satOp, parent, depth, alts, opts, lf);
				...
				SatOp dummyParent = makeDummySatOp(currentNominal);
				addSatOp(dummyParent, parent, depth, alts, opts, lf);
				// as before, process xor by introducing new alt set and adding
				// alt for each disjunct;
				// this time, also assume each disjunct is a conj op or nominal,
				// and add a diamond satop
				// to the disjunct nominal
				int altSet = altCount++;
				for (int i = 0; i < argOp._args.size(); i++) {
					alts.push(new Alt(altSet, i));
					LF disjunct = argOp._args.get(i);
					if (!(disjunct instanceof Op && ((Op) disjunct)._name.equals(Op.CONJ))
							&& !(disjunct instanceof Nominal)) {
						throw new RuntimeException(
								"Disjunct of diamond is not a conj op or nominal: " + disjunct);
					}
					// conj op case
					if (disjunct instanceof Op) {
						...
						if (!(firstArg instanceof Nominal)) {
							throw new RuntimeException(
									"First arg of conj op under xor op is not a nominal: "
											+ firstArg);
						}
						// add SatOp for diamond
				...
				throw new RuntimeException("Arg of diamond is not a proposition, nominal or list: "
						+ arg);
			}
		} else
			throw new RuntimeException("Unable to flatten " + lf);
	}
	...
	// handles new preds
	private void addSatOp(SatOp satOp, SatOp parent, int depth, Stack<Alt> alts,
			TIntArrayList opts, LF lf) {
		...
		if (!nom.isShared()) {
			if (!nomMap.containsKey(nom) || depth < depthMap.get(nom)) {
				...
				parentMap.put(nom, (parent != null && parent._nominal != nullNom) ? parent._nominal
						: null);
			}
		}
	...
	// prop alts, opts & chunks, recursing through preds in child map and shared
	// nom refs in nomMap
	private void propAltsOptsChunks(SatOp satOp, List<Alt> alts, TIntArrayList opts,
			TIntArrayList chunks) {
		// prop alts and opts
		if (!alts.isEmpty()) {
			if (satOp.alts == null)
				satOp.alts = new ArrayList<Alt>(3);
			for (Alt alt : alts) {
				if (!satOp.alts.contains(alt))
					satOp.alts.add(alt);
			}
		...
		if (!chunks.isEmpty()) {
			...
			for (int i = 0; i < chunks.size(); i++) {
				...
				if (!satOp.chunks.contains(chunk))
