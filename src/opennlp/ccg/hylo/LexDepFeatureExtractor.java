...
/**
 * A class for extracting lexical dependency features. This class implements the
 * features in White and Rajkumar's EMNLP-12 paper <a
 * href="http://aclweb.org/anthology-new/D/D12/D12-1023.pdf"> Minimal Dependency
 * Length in Realization Ranking</a>.
 * 
 * Features are extracted lazily for efficiency, using a prefix of "ld" for
 * "lexical dependency".
 * 
 * Features potentially involve the word, POS tag, word class, definiteness,
 * syntactic complexity, short-long order, and total dependency length, where
 * the latter three can be controlled by the appropriate include* flags; of
 * these, only dependency length is included by default.
 *
 * Word class is based on the semantic class, a check for color terms, common
 * suffixes, and the presence of a hyphen or capitalization.
 * 
 * The checks for definite NPs, color terms and common suffixes are done by
 * methods with defaults for the English CCGbank, which can be overridden in
 * subclasses or reconfigured in the case of color terms and suffixes.
 * 
...
public class LexDepFeatureExtractor implements FeatureExtractor {
	...
	/**
	 * Returns the appropriate siblings precedence constant for the head-dep
	 * order constant.
	 */
	...
	/**
	 * Flag for whether to include syntactic complexity ordering features
	 * (defaults to false).
	 */
	...
	/**
	 * Flag for whether to include global dependency length features (defaults
	 * to true).
	 */
	...
	/**
	 * Conditional lazy extractor, for lazily extracting a feature subject to a
	 * test.
	 */
	public static abstract class ConditionalLazyExtractor {
		abstract boolean test();

		List<TrieMap.KeyExtractor<String>> lazyExtractor = new ArrayList<TrieMap.KeyExtractor<String>>(
				5);
	}

	/**
	 * Conditional lazy evaluator, for lazily extracting a feature and its
	 * value, subject to a test.
	 */
	...
	/**
	 * Current difference in lengths between second and first siblings (for
	 * extracting features).
	 */
	protected PhraseLengths currentLengthsDiff = null;

	// /** Lexical feature extractors. */
	// protected List<List<TrieMap.KeyExtractor<String>>> lexExtractors = new
	// ArrayList<List<TrieMap.KeyExtractor<String>>>();
				...
				for (LexDependency sib : sign.getSiblingFilledDeps()) {
					...
					if (includeComplexityFeats || includeShortLong) {
						...
						if (currentLengthsDiff != null) {
							if (includeComplexityFeats)
								incCond(sibsComplexityOrderCondExtractors);
							if (includeShortLong)
								addCond(shortLongCondEvaluators);
						}
					}
				}
			}
	...
	protected void addCond(List<ConditionalLazyEvaluator> condEvaluators) {
		for (ConditionalLazyEvaluator condEvaluator : condEvaluators) {
			if (condEvaluator.test()) {
				...
				if (f != null)
					currentMap.add(f, condEvaluator.eval());
			}
		}
	}
	...
	/**
	 * Class for storing whether a sign is or immediately contains a definite
	 * NP.
	 */
	...
	/**
	 * Returns the given sign if it's an NP sign; otherwise returns the first
	 * child sign that's an NP sign; otherwise returns null.
	 */
	...
	/**
	 * Returns whether the given sign is an NP. The default implementation tests
	 * for a category type of "np".
	 */
	...
	/**
	 * Returns whether the given semantic features signals indefiniteness. The
	 * default implementation tests for &lt:det&gt;nil.
	 */
	protected boolean isIndefFeat(SatOp feat) {
		return HyloHelper.getInstance().getRel(feat).equals("det")
				&& "nil".equals(HyloHelper.getInstance().getVal(feat));
	}

	/**
	 * Returns whether the given lexical dependency signals indefiniteness. The
	 * default implementation checks for &lt;Det&gt;a|an|any|some.
	 */
	...
	/**
	 * Class for storing length of phrase in words, puncts and (finite) verbs,
	 * for unique retrieval from a sign's data objects.
	 */
	public static class PhraseLengths {
		...
		public String toString() {
			return " wordlen: " + wordlen + " punctlen: " + punctlen + " verblen: " + verblen;
		}
	}
	...
	/**
	 * Returns whether a word is a punctuation mark that typically signals
	 * sentence-internal complexity. The default implementation tests for
	 * commas, dashes (--), semi-colons and colons.
	 */
	protected boolean isPunct(Association word) {
		// NB: in principle could use POS, but sometimes punctuation marks seem
		// to end up with IN as the POS tag
		String form = word.getForm();
		return (form == "," || form == "--" || form == ";" || form == ":");
	}

	/**
	 * Returns whether a word is a verb that indicates a substantial clause. The
	 * default implementation tests for the finite verb POS tags VBD, VBP and
	 * VBZ.
	 */
	...
	/**
	 * Sets the differences in length between the signs headed by the current
	 * dep and sib, or null if none; also sets the current dep and sib lengths.
	 * The lengths are set to the lengths of the second sign minus those of the
	 * first sign.
	 */
	protected void setLengthsDiff() {
		// reset
		currentLengthsDiff = null;
		currentDepLengths = null;
		currentSibLengths = null;
		// ensure both there
		if (currentDepSign == null || currentSibSign == null)
			return;
		// get phrase lengths
		...
		// set diff to 2nd - 1st
		currentLengthsDiff = new PhraseLengths(pl2.wordlen - pl1.wordlen, pl2.punctlen
				- pl1.punctlen, pl2.verblen - pl1.verblen);
	}
	...
	/**
	 * Returns the dependency length between the current head and the current
	 * dependent. The default implementation returns the number of intervening
	 * words excluding punctuation (as determined by isPunct), and doesn't count
	...
	/**
	 * Returns a class for the word, or CLASS_NIL if none. The default
	 * implementation returns one of the following, in this order: the semantic
	 * class of the word; CLASS_PRO, if a pronoun; CLASS_COLOR, if a color word;
	 * the suffix, if getSuffix returns a value; CLASS_HYPH, if the word is
	 * hyphenated; CLASS_CAP, if capitalized; or CLASS_NIL, otherwise. The word
	 * class is cached using cachedWordClasses.
	 */
	protected String getWordClass(Association word) {
		...
		if (wClass != null)
			return updateCachedWordClasses(word, wClass);
		if (isPro(word))
			return updateCachedWordClasses(word, CLASS_PRO);
		String form = word.getForm();
		if (colors.contains(form))
			return updateCachedWordClasses(word, CLASS_COLOR);
		String suffix = getSuffix(form);
		if (suffix != null)
			return updateCachedWordClasses(word, suffix);
		if (form.indexOf('-') >= 0)
			return updateCachedWordClasses(word, CLASS_HYPH);
		if (Character.isUpperCase(form.charAt(0)))
			return updateCachedWordClasses(word, CLASS_CAP);
		return updateCachedWordClasses(word, CLASS_NIL);
	}

	/**
	 * Returns whether a word is a pronoun. The default implementation returns
	 * whether the POS tag starts with "PR".
	 */
	protected boolean isPro(Association word) {
		return word.getFunctions().startsWith("PR");
	}
	...
	/**
	 * Returns the default set of color words: 11 common English colors, with
	 * two spellings of gray/grey.
	 */
	protected Set<String> defaultColors() {
		String[] colors = { "black", "blue", "brown", "gray", "grey", "green", "orange", "pink",
				"purple", "red", "white", "yellow" };
		return new HashSet<String>(Arrays.asList(colors));
	}

	/**
	 * A sequence of suffixes to check for in determining the word class,
	 * ordered by specificity.
	 */
	protected String[] suffixClasses = defaultSuffixClasses();

	/**
	 * Returns the default suffix classes: 61 common English suffixes from
	 * various lists on the web.
	 */
	protected String[] defaultSuffixClasses() {
		return new String[] { "ancy", "aphy", "arch", "crat", "gram", "less", "logy", "ness",
				"nomy", "ship", "some", "sque", "tude", "ade", "age", "ant", "aph", "ary", "ast",
				"ate", "ble", "dom", "ent", "est", "ful", "ian", "ile", "ion", "ing", "ish", "ism",
				"ist", "ise", "ite", "ium", "ive", "ize", "nce", "oid", "ory", "ose", "ote", "ous",
				"sig", "ure", "ac", "al", "an", "cy", "ed", "en", "er", "fy", "ic", "le", "ly",
				"or", "se", "sy", "ty", "y" };
	}
	...
	protected String getSuffix(String form) {
		for (int i = 0; i < suffixClasses.length; i++) {
			...
			if (form.length() > suff.length() && form.endsWith(suff))
				return suff;
		}
		return null;
	}
	...
	/**
	 * Updates the cached word classes with the given word and word class, and
	 * returns the word class.
	 */
	protected String updateCachedWordClasses(Association word, String wordClass) {
		cachedWordClasses.put(word, wordClass);
		return wordClass;
	}
	...
	private void add_head_word(List<TrieMap.KeyExtractor<String>> retval) {
		retval.add(new TrieMap.KeyExtractor<String>() {
			public String getKey() {
				return currentDep.lexHead.getWordForm();
			}
		});
	}
	...
	private void add_head_pos(List<TrieMap.KeyExtractor<String>> retval) {
		retval.add(new TrieMap.KeyExtractor<String>() {
			public String getKey() {
				return currentDep.lexHead.getPOS();
			}
		});
	}
	...
	private void add_dep_word(List<TrieMap.KeyExtractor<String>> retval) {
		retval.add(new TrieMap.KeyExtractor<String>() {
			public String getKey() {
				return currentDep.lexDep.getWordForm();
			}
		});
	}
	...
	private void add_dep_pos(List<TrieMap.KeyExtractor<String>> retval) {
		retval.add(new TrieMap.KeyExtractor<String>() {
			public String getKey() {
				return currentDep.lexDep.getPOS();
			}
		});
	}
	...
	private void add_sibs_word1(List<TrieMap.KeyExtractor<String>> retval) {
		retval.add(new TrieMap.KeyExtractor<String>() {
			public String getKey() {
				return (currentDepPrecedesSib) ? currentDep.lexDep.getWordForm()
						: currentSib.lexDep.getWordForm();
			}
		});
	}
	...
	private void add_sibs_word2(List<TrieMap.KeyExtractor<String>> retval) {
		retval.add(new TrieMap.KeyExtractor<String>() {
			public String getKey() {
				return (currentDepPrecedesSib) ? currentSib.lexDep.getWordForm()
						: currentDep.lexDep.getWordForm();
			}
		});
	}
	...
	private void add_sibs_pos1(List<TrieMap.KeyExtractor<String>> retval) {
		retval.add(new TrieMap.KeyExtractor<String>() {
			public String getKey() {
				return (currentDepPrecedesSib) ? currentDep.lexDep.getPOS() : currentSib.lexDep
						.getPOS();
			}
		});
	}
	...
	private void add_sibs_pos2(List<TrieMap.KeyExtractor<String>> retval) {
		retval.add(new TrieMap.KeyExtractor<String>() {
			public String getKey() {
				return (currentDepPrecedesSib) ? currentSib.lexDep.getPOS() : currentDep.lexDep
						.getPOS();
			}
		});
	}
	...
	private ConditionalLazyExtractor sibs_precedence_pos() {
		ConditionalLazyExtractor retval = new ConditionalLazyExtractor() {
			boolean test() {
				return currentDep.lexDep.getPOS() != currentSib.lexDep.getPOS();
			}
		};
	...
	private ConditionalLazyExtractor sibs_precedence_rels() {
		ConditionalLazyExtractor retval = new ConditionalLazyExtractor() {
			boolean test() {
				return !currentDep.rel.equals(currentSib.rel);
			}
		};
		add_sibs_precedence_common(retval.lazyExtractor);
	...
	private ConditionalLazyExtractor sibs_precedence_defs() {
		...
		retval.lazyExtractor.add(new TrieMap.KeyExtractor<String>() {
			public String getKey() {
				return (currentDepPrecedesSib) ? defConstant(getDefiniteNP(currentDepSign))
						: defConstant(getDefiniteNP(currentSibSign));
			}
		});
		retval.lazyExtractor.add(new TrieMap.KeyExtractor<String>() {
			public String getKey() {
				return (currentDepPrecedesSib) ? defConstant(getDefiniteNP(currentSibSign))
						: defConstant(getDefiniteNP(currentDepSign));
			}
		});
		return retval;
	}
	...
	private ConditionalLazyEvaluator short_long_words() {
		ConditionalLazyEvaluator retval = new ConditionalLazyEvaluator() {
			boolean test() {
				return currentLengthsDiff.wordlen != 0;
			}

			float eval() {
				return (float) Math.abs(currentLengthsDiff.wordlen);
			}
		};
		add_sibs_precedence_common(retval.lazyExtractor);
		retval.lazyExtractor.add(new TrieMap.KeyExtractor<String>() {
