...
public class Converter {
	...
	static Nominal convertNominals(LF lf, Symbol root, Nominal nominalRoot) {
		// check preference for naming with word positions; set root to null if
		// false
		...
		if (lf instanceof SatOp) {
			...
			if (root != null) {
				...
				if (origin instanceof Symbol) {
					...
					// nb: also need to check for special pred 'elem', which
					// isn't
	...
	private Nominal convertNominal(Nominal oldNom, Proposition prop, int wordIndex) {
		...
		if (wordIndex >= 0)
			return convertNominal(oldNom, "w" + wordIndex);
		// skip absent props according to flag
		if (prop == null && skipAbsentProp)
			return oldNom;
		...
		if (prop != null) {
			...
			if (!Character.isLetter(nameBase.charAt(0)))
				nameBase = "n";
		}
		int ext = 1;
		Integer baseCount = nameMap.get(nameBase);
		if (baseCount != null) {
			ext = baseCount.intValue() + 1;
		}
	...
	private static boolean lexDominated(String lexPred, Symbol lexSign) {
		...
		for (SatOp pred : otherLexPreds) {
			...
			if (dominates(otherNom, lexNom, preds, seen))
				return true;
		}
		// otherwise false
		return false;
	...
	private static boolean dominates(Nominal a, Nominal b, List<SatOp> preds, Stack<Nominal> seen) {
		...
		for (SatOp pred : preds) {
			if (a.equals(HyloHelper.getInstance().getPrincipalNominal(pred))) {
				...
				if (b.equals(c))
					return true; // found dominance!
				// check seen
				if (seen.contains(c))
					continue;
				// recurse
				if (dominates(c, b, preds, seen))
					return true;
			}
		}
