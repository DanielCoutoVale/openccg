...
/**
 * A generic operator, such as conjunction, disjunction, exclusive-or, negation
 * or optionality (^, v, v_, ~, ?).
 *
...
public class Op extends HyloFormula {
	...
	/**
	 * Appends the args if the given lf is a CONJ op, otherwise just adds it.
	 */
	...
	public boolean equals(Object o) {
		...
		if (_args.size() != opArgs.size())
			return false;
		if (!opArgs.containsAll(_args))
			return false;
		return true;
	}
	...
	/**
	 * Returns whether this op equals the given object up to variable names,
	 * using the given maps from vars to ints (where args must be in the same
	 * order).
	 */
	public boolean equals(Object obj, TObjectIntHashMap varMap, TObjectIntHashMap varMap2) {
		...
		Op op = (Op) obj;
		if (!_name.equals(op._name))
			return false;
		if (_args.size() != op._args.size())
			return false;
		for (int i = 0; i < _args.size(); i++) {
			LF arg = (LF) _args.get(i);
			LF arg2 = (LF) op._args.get(i);
			if (!arg.equals(arg2, varMap, varMap2))
				return false;
		}
		return true;
	}
