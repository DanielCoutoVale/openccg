 ...
 * <p>
 * Since certain modes reoccur frequently, this class maintains a cache of modes
 * mapped to edge labels, accessible via {@link #forMode(Mode)}.
 * 
...
public class LFEdgeLabel {
	 ...
	 * 
	 * @param mode The mode representing the type of relation between the
	 *            vertices.
	...
	/**
	 * Gets a cached LF edge label for the specified mode, if one is available.
	 * If none has been created yet, a new LF edge label is created and cached
	 * for later use. Access to the cached LF edge labels is synchronized to
	 * avoid threading isues.
	 * 
	 * @param mode The mode to create a label for.
	 * @return Either a cached LF edge label corresponding to the specified
	 *         mode, if one is available, or a newly created one.
	 */
	...
	/**
	 * Tests whether this LF edge label is equivalent to another by comparing
	 * their modes, using their {@link Mode#equals(Object)} methods.
	 */
	@Override
