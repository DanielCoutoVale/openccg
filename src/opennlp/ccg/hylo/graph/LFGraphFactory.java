...
public class LFGraphFactory {
	...
	/**
	 * Builds a new LF graph based on the representation provided in the
	 * specified w3c XML element.
	 * 
	 * @param lfElement The XML element with root element tagged <tt>lf</tt>.
	 * @return The value of {@link #newGraphFrom(Element)} after using a
	 *         {@link DOMBuilder} to create a jdom document.
	 * 
	...
	/**
	 * Creates a new LF graph from the corresponding representation contained in
	 * the specified jdom XML element.
	 * 
	 * @param lfElement The jdom element containing the representation of the
	 *            LF.
	...
	/**
	 * Creates a new LF graph based on the specified LF object. The LF object is
	 * first flattened, and then its LF ancestry structure is obtained by
	 * calling {@link Flattener#getHighestParentMap()}.
	 * <p>
	 * This method makes two passes over the list of {@link SatOp}s obtained by
	 * flattening the specified LF. The first pass adds vertices to the graph
	 * for every lexical predication, as determined by calling
	 * {@link HyloHelper#isLexPred(LF)} on the {@linkplain SatOp#getNominal()
	 * SatOp's nominal} and {@linkplain SatOp#getArg() proposition argument}.
	 * <p>
	 * The second pass proceeds by cases, depending on the nature of the SatOp
	 * in question:
	 * <dl>
	 * <dt>Lexical predications</dt>
	 * <dd>cause the new LF graph to be updated with the corresponding LF
	 * ancestry, as determined by {@link Flattener#getHighestParentMap()}.</dd>
	 * <dt>Relation predications</dt>
	 * <dd>cause a new {@link LFEdge} to be added to the LF graph based on the
	 * {@linkplain SatOp#getArg() SatOp's argument} and
	 * {@linkplain Diamond#getMode() the argument's mode}.</dd>
	 * <dt>Attribute-value predications</dt>
	 * <dd>cause the vertex corresponding to the {@linkplain SatOp#getNominal()
	 * SatOp's nominal} to have attributes
	 * {@linkplain LFVertex#setAttribute(opennlp.ccg.hylo.Mode, Proposition)
	 * added} based on the {@linkplain SatOp#getArg() SatOp's argument}.</dd>
	 * </dl>
	 * where the nature of the SatOp in question is determined using
	 * {@link HyloHelper#isLexPred(LF)}, {@link HyloHelper#isRelPred(LF)}, and
	 * {@link HyloHelper#isAttrPred(LF)}.
	 * 
	 * @param lf The LF object to build an LF graph for.
	 * @return A new LF graph whose vertices represent the nominals in the LF's
	 *         flattened representation and whose edges represent its relation
	 *         predications.
	...
	public static LFGraph newGraphFrom(LF lf) {
		...
		for (SatOp so : satOps) { // second pass adds edges and attributes, sets
									// highest parent (if any)
