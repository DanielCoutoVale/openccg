...
/**
 * A class for tracking semantic dependencies between lexical items. A lex
 * dependency is a triple consisting of a lexical head, a relation and a lexical
 * dependent. Either the head or dependent can be null, in which case the
 * dependency is considered unfilled. To be equal, a lex dependency must have
 * identical heads and dependents and equal relations. Relations can involve
 * chains of individual relations (concatenated with dots) when there are
 * intervening nominals for the same lexical item.
 * 
...
public class LexDependency {
	...
	public int hashCode() {
		...
		if (lexHead != null)
			retval += 31 * lexHead.hashCode();
		if (lexDep != null)
			retval += 7 * lexDep.hashCode();
		return retval;
	}
	...
	public String toString() {
		...
		if (lexHead == null)
			...
			retval.append(lexHead.getOrthography());
		retval.append('<').append(rel).append('>');
		if (lexDep == null)
			retval.append("null");
		else
			retval.append(lexDep.getOrthography());
	...
	/**
	 * Returns a list of unfilled dependencies for a list of EPs for a lexical
	 * item.
	 */
	public static List<LexDependency> unfilledLexDeps(List<SatOp> preds) {
		...
		// otherwise, starting with each root nominal, enumerate paths to leaf
		// nominals
		for (Nominal root : nominals) {
			// check for root nominal
			if (!HyloHelper.getInstance().isRoot(root, preds))
				continue;
			// set lex head
			Symbol lexHead = null;
			...
			for (int i = rootIndex; i < preds.size()
					&& HyloHelper.getInstance().getPrincipalNominal(preds.get(i)).equals(root); i++) {
	...
	private static void addUnfilledLexDep(Nominal dep, Symbol lexHead, String rel,
			List<SatOp> preds, Map<Nominal, Integer> nomIndex, List<LexDependency> retval) {
		// if dep not in nom index, then just add unfilled dep for the current
		// rel
		...
		for (int i = depIndex; i < preds.size()
				&& HyloHelper.getInstance().getPrincipalNominal(preds.get(i)).equals(dep); i++) {
			depPred = preds.get(i);
			Nominal depdep = HyloHelper.getInstance().getSecondaryNominal(depPred);
			if (depdep == null)
				continue;
			String relrel = (rel == null) ? HyloHelper.getInstance().getRel(depPred) : rel + "."
	 ...
	 * by checking the list of EPs for ones that have become filled, removing
	 * the corresponding no longer unfilled deps.
	 */
	public static List<LexDependency> filledLexDeps(List<LexDependency> unfilled, List<SatOp> preds) {
		...
		for (Iterator<LexDependency> it = unfilled.iterator(); it.hasNext();) {
			...
			if (udep.lexDep == null) {
				...
				if (!nomIndex.containsKey(depnom))
					continue;
			...
			// head missing case
				...
				if (!nomIndex.containsKey(headnom))
					continue;
	...
	private static SatOp findDescendantPred(SatOp current, int index, String[] rels,
			List<SatOp> preds, Map<Nominal, Integer> nomIndex) {
		...
		for (int i = nomIndex.get(depnom); i < preds.size()
				&& HyloHelper.getInstance().getPrincipalNominal(preds.get(i)).equals(depnom); i++) {
	...
	private static SatOp findAncestorPred(SatOp current, int index, String[] rels, List<SatOp> preds) {
		...
		for (int i = 0; i < preds.size(); i++) {
			SatOp pred = preds.get(i);
			if (headnom.equals(HyloHelper.getInstance().getSecondaryNominal(pred))
					&& rel.equals(HyloHelper.getInstance().getRel(pred))) {
				prev = pred;
				break;
			}
		}
	...
	/**
	 * Filters the first list of dependencies to those sharing a head with a
	 * dependency in the second list.
	 */
	public static List<LexDependency> filterSameHead(List<LexDependency> deps1,
			List<LexDependency> deps2) {
