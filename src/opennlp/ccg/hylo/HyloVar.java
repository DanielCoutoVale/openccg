...
/**
 * A class for objects which can stand for any HyloFormula object. Types are
 * unified with other hylo vars, and with other hylo formulas when present.
 *
...
public class HyloVar extends HyloFormula implements Variable, Indexed {
	...
	public Object unify(Object u, Substitution sub) throws UnifyFailure {
		...
		SimpleType st = null;
		if (lf.getType() != null)
			st = (SimpleType) type.unify(lf.getType(), sub);
		// with hylo vars, substitute according to type specificity then
		// comparison order,
		...
		if (u instanceof HyloVar) {
			...
			String name = (compareTo(u_hv) >= 0) ? (u_hv._name + u_hv._index)
					: (_name + this._index);
	...
	public String toString() {
		...
		if (!type.getName().equals(Types.TOP_TYPE))
			retval += ":" + type.getName();
		return retval;
	}

	/**
	 * Returns the name with the type separated by a colon if the type is not
	 * the top type.
	 */
	 ...
	 * using the given maps from vars to ints. (Note that the name and index may
	 * differ, but the types must be equal.)
	 */
	public boolean equals(Object obj, TObjectIntHashMap varMap, TObjectIntHashMap varMap2) {
		...
		if (varMap.get(this) != varMap2.get(hv))
			return false;
		if (!this.type.equals(hv.type))
			return false;
		return true;
	}
