...
public class GFeatStruc extends HashMap<String, Object> implements FeatureStructure {
	...
	public Element toXml() {
		...
		if (_index > 0)
			retval.setAttribute("id", Integer.toString(_index));
		if (_inheritsFrom > 0)
			retval.setAttribute("inheritsFrom", Integer.toString(_inheritsFrom));
		List<String> keys = new ArrayList<String>(keySet());
		Collections.sort(keys);
		if (size() == 1 && get(keys.get(0)) instanceof SimpleType) {
			String attr = keys.get(0);
			SimpleType val = (SimpleType) get(attr);
			...
			for (String attr : keys) {
				...
				if (val instanceof SimpleType)
					...
					if (val instanceof GFeatVar) {
						...
						if (!typeName.equals(Types.TOP_TYPE))
							name += ":" + typeName;
						...
						throw new RuntimeException(
								"Unsupported feature value type in constructing XML: " + val);
				}
			}
		}
		return retval;
	...
	/**
	 * Returns true iff this feature structure has the same index and set of
	 * attr-val pairs.
	 */
	public boolean equals(FeatureStructure fs) {
		...
		for (Iterator<String> it = atts1.iterator(); it.hasNext();) {
			String att = it.next();
			if (!getValue(att).equals(bfs.getValue(att)))
				return false;
		}
	...
	public void unifyCheck(Object u) throws UnifyFailure {
		...
		for (Iterator<String> i1 = keys1.iterator(); i1.hasNext();) {
			...
			if (!(val1 instanceof SimpleType))
				continue;
			Object val2 = fs2.getValue(k1);
			if (!(val2 instanceof SimpleType))
				continue;
	...
	public Object fill(Substitution sub) throws UnifyFailure {
		...
		if (_index > 0 && sub instanceof GSubstitution) {
			FeatureStructure otherVals = (FeatureStructure) ((GSubstitution) sub)
					.getIndexedObject(_index);
			if (null != otherVals) {
	...
	/**
	 * Returns the supertag info for this feature structure. In particular,
	 * returns the values of any non-variable features of interest, within
	 * square brackets. The features of interest are configurable at the grammar
	 * level.
	 */
	public String getSupertagInfo() {
		...
		for (int i = 0; i < attrs.size(); i++) {
			...
			if (!supertagFeatures.contains(attr))
				continue;
	...
	/**
	 * Returns a hash code using the given map from vars to ints, to allow for
	 * equivalence up to variable names.
	 */
	public int hashCode(TObjectIntHashMap varMap) {
		...
		// otherwise treat missing index as unique, keyed to negative identity
		// hash
		else {
			int next = varMap.size() + 1;
	...
	/**
	 * Returns whether this feature structure equals the given object up to
	 * variable names, using the given maps from vars to ints.
	 */
	public boolean equals(Object obj, TObjectIntHashMap varMap, TObjectIntHashMap varMap2) {
		...
		int mappedIndex = (_index != 0) ? varMap.get(_index) : varMap.get(-1
				* Math.abs(System.identityHashCode(this)));
		int fsMappedIndex = (fs._index != 0) ? varMap2.get(fs._index) : varMap2.get(-1
				* Math.abs(System.identityHashCode(fs)));
		...
		for (Iterator<String> it = atts1.iterator(); it.hasNext();) {
			...
			if (val instanceof Variable && val2 instanceof Variable) {
				if (!((Variable) val).equals(val2, varMap, varMap2))
					return false;
			} else {
				if (!val.equals(val2))
					return false;
