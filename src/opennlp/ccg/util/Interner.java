...
/**
 * A utility class for interning (canonicalizing) objects. A WeakHashMap is used
 * as the backing store, so that interned objects can be garbage collected.
 * Normally, it is easier to use the globalIntern method (sharing a global
 * backing store) than to allocate separate interners. Individual interners can
 * be constructed to use soft references to the interned objects, so that they
 * are kept around longer than is the case with weak references (the default).
 *
...
public class Interner<T> {
	...
	/**
	 * Returns a canonical version of the given object. The returned object is
	 * .equals() to the given one. If the given object is not equal to one
	 * already seen, then the returned object will be == to the given one.
	 */
	public T intern(T obj) {
		...
		Reference<T> ref = (softRefs) ? new SoftReference<T>(obj) : new WeakReference<T>(obj);
		weakMap.put(obj, ref);
		return obj;
	}

	/**
	 * Returns the canonical version of the given object, if any, otherwise
	 * returns null.
	 */
	 ...
	 * The returned object is .equals() to the given one. If the given object is
	 * not equal to one already seen, then the returned object will be == to the
	 * given one.
	 */
	public static Object globalIntern(Object obj) {
		if (globalInterner == null)
			globalInterner = new Interner<Object>();
		return globalInterner.intern(obj);
	}

	/**
	 * Returns the canonical version of the given object using the global
	 * interner, if any, otherwise returns null.
	 */
	...
	public static void main(String[] args) {
		...
		System.out.println("interner.size(): " + interner.size()); // should be
																	// SIZE
