...
/**
 * An unmodifiable list formed by sequencing two sublists. The sublists are
 * assumed to remain unchanged as well. Equality is checked using == on the list
 * elements, which are assumed to be canonical.
 *
...
public class StructureSharingList<T> extends AbstractList<T> implements Serializable {
	...
	public boolean equals(Object obj) {
		...
		if (list instanceof StructureSharingList<?>) {
			...
			if (first == ssl.first && second == ssl.second)
				return true;
		}
		if (size() != list.size())
			return false;
