...
/**
 * Quick-and-dirty class for using JDOM elements in parsing XML without building
 * a document for the whole XML file. The <code>parse</code> method parses an
 * XML file with a SAX parser, constructing JDOM elements for root and the
 * top-level elements, without attaching the top-level elements to the root.
 * Along the way, the <code>handleRoot</code> and <code>handleElement</code>
 * methods are invoked, to handle these elements incrementally. To use the
 * class, just implement <code>handleElement</code>, and optionally implement
 * <code>handleRoot</code>, and call <code>parse</code> on the input XML via its
 * URL. Note that with Java's incremental garbage collection, an XmlScanner
 * should be reasonably efficient, but not as efficient as a pure SAX parser
 * (which however requires considerably more work to implement). At present, the
 * parser only handles elements and attributes without namespaces; all
 * attributes with namespaces, text nodes, comments, etc. are ignored.
 *
...
public abstract class XmlScanner extends DefaultHandler {
	...
	/**
	 * Method for handling the root element, sans children. The default method
	 * does nothing.
	 */
	...
	/**
	 * For the root, a childless element is created and handled, via
	 * <code>handleRoot</code>; for all other elements, a new current element is
	 * created and added as a child of the current element, if any.
	 */
	...
	/**
	 * Constructs and returns a new element from the given info. This
	 * implementation ignores the <code>uri</code> and <code>localName</code>,
	...
	public static class MyScanner extends XmlScanner {
		...
		public void handleElement(Element e) {
			count++;
			second += e.getContentSize();
		}
	}

	/**
	 * The main method shows an example of using a scanner, by invoking an
	 * instance of <code>MyScanner</code> on the file whose name is given by the
	 * first arg.
	 */
