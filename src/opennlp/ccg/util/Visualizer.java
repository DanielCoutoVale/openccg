...
public class Visualizer {
	...
	public boolean show(String fileName) {
		...
		try {
			...
			// The process will wait indefinitely unless we close each of the
			// related streams:/
			...
			// p.waitFor();
			// p = null;
			// p = java.lang.Runtime.getRuntime().exec(viewerName + " " +
			// fileName);
			// p.getInputStream().close();
			// p.getOutputStream().close();
			// p.getErrorStream().close();
	...
	public class auxFileNameFilter implements FilenameFilter {
		...
		public boolean accept(File dir, String name) {
			if ((name.startsWith(fn))
					&& (!((name.toUpperCase().endsWith(".TEX")) || (name.toUpperCase()
							.endsWith(".DVI")))))
				return true;
			else
				return false;
	...
	/**
	 * Writes a derivation in fileName in TeX format; returns false in case it
	 * fails.
	 */
	 ...
	 *
	 * @param cmd The string containing the command to execute
	 * @param wait True if the caller should wait for this thread to finish
